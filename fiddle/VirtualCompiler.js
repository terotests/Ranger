...
class RangerFlowParser  {
  ...
  async WalkNode (node, ctx, wr) {
    ...
    if ( (node.children.length) > 0 ) {
      ...
      let b_found = true;
      const opFn = await ctx.getOpFns(fc_2.vref);
      if ( (opFn.length) > 0 ) {
    ...
    await operatorsOf.forEach_15(opFnList, (async (item, index) => { 
        ...
        if ( errDelta > 0 ) {
          if ( errDelta < least_err_cnt ) {
            ...
            while (i < currentErrCnt) {
              const tmp = rootCtx.compilerErrors[i];
              least_errs.push(tmp);
              i = i + 1;
            };
          }
      ...
      if ( false ) {
        if ( false == ctx.isTestCompile() ) {
          ...
          const codeStr = node.getCode();
          subCtx.setTestCompile();
          const results = await this.testCompile(itemCopy, node, subCtx, wr);
          await operatorsOf_13.forEach_25(results, ((item, index) => { 
              ...
              if ( false == (( typeof(regParams[item.vref] ) != "undefined" && regParams.hasOwnProperty(item.vref) )) ) {
                ...
                if ( (realArg.register_name.length) > 0 ) {
                  ...
                  if ( use_delta && (opCntDelta > 0) ) {
                    ...
                    let i_2 = cnt;
                    while (i_2 < opCnt) {
                      const tmp_1 = BlockOP.register_expressions[i_2];
    ...
    if ( errDelta_3 > 0 ) {
      if ( errDelta_3 < least_err_cnt ) {
        ...
        while (i_3 < currentErrCnt_1) {
          const tmp_4 = rootCtx.compilerErrors[i_3];
...
class VirtualCompiler  {
  ...
  async run (env) {
    ...
    operatorsOf_13.forEach_54(params.flags, ((item, index) => { 
      const n = index;
...
operatorsOf.clone_55 = function(__self) {
  let res_14 = [];
...
operatorsOf.forEach_56 = function(__self, cb) {
...
operatorsOf_13.forEach_54 = function(__self, cb) {
