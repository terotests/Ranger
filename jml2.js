...
;(function (_glob) {
    ...
    function create_ctx(parent, autoDefine) {
        var parentCtxBase = function parentCtxBase() {};
        if (parent) {
        ...
        if (autoDefine) {
            Object.keys(autoDefine).forEach(function (k) {
                o.__def(k);
                o[k] = autoDefine[k];
            });
        }
...
function init_lisp_core() {
    ...
    window.lisp_fns = {
        ...
        "typeof": function _typeof(list, ctx) {

            var v = list[0];
            if ((typeof v === "undefined" ? "undefined" : _typeof2(v)) == "object") {
                if (v.vref) {
            ...
            return typeof v === "undefined" ? "undefined" : _typeof2(v);
        },

        // (def foo "bar")
        "def": function def(list, ctx) {
            var var_list = list[0];

            if ((typeof var_list === "undefined" ? "undefined" : _typeof2(var_list)) == "object" && var_list.children) {
        ...
        "concatenate": function concatenate(list, ctx) {
            return expand_sequence(list, ctx).map(function (x) {
                return lisp_eval(x, ctx);
            }).join("");
        },

        // can you expand a list 
        "expand": function expand(list, ctx) {
            /*
        ...
        "list": function list(_list, ctx) {
            return _list;
        },

        "apply": function apply(list, ctx) {
        ...
        "uppercase": function uppercase(list, ctx) {
        ...
        "join_array": function join_array(list, ctx) {

            // is it possible to expand list ? 
            var the_list = lisp_eval(list[0], ctx);

            var val = expand_sequence(the_list, ctx).map(function (x) {
                return lisp_eval(x, ctx);
            }).join("");
            return val;
        },

        "join": function join(list, ctx) {

            // is it possible to expand list ? 
            var the_list = list;

            if (list.length == 1) {
                if (_typeof2(list[0]) == "object") {
        ...
        "join_with": function join_with(list, ctx) {
        ...
        "array": function array(list, ctx) {
            return expand_sequence(list, ctx).map(function (x) {
                return lisp_eval(x, ctx);
            });
        },

        "smap": function smap(list, ctx) {
            var op = list[0]; // lisp operator to apply to array

            if (list.length == 2) {
                if (_typeof2(list[1]) == "object") {
        ...
        "filter": function filter(list, ctx) {
            list = expand_sequence(list, ctx);
            var op = list[0]; // lisp operator to apply to array
            var value_list = lisp_eval(list[1], ctx); // evaluate
        ...
        "reduce": function reduce(list, ctx) {
            var op = list[0]; // lisp operator to apply to array
        ...
        "add_to_dom": function add_to_dom(list, ctx) {
            var id = list[0];
            var dom = document.getElementById(id);
            if (dom) {
                var t = dom.innerHTML;
                var elems = list.slice(1);
                dom.innerHTML = t + expand_sequence(elems, ctx).map(function (x) {
                    return lisp_eval(x, ctx);
                }).join("");
            }
        }
...
function run_lisp(node, ctx) {
    ...
    if ((typeof node === "undefined" ? "undefined" : _typeof2(node)) == "object" && node.vref) {
    ...
    if ((typeof fn_name === "undefined" ? "undefined" : _typeof2(fn_name)) == "object" && fn_name.lisp) {
    ...
    if ((typeof fn_name === "undefined" ? "undefined" : _typeof2(fn_name)) == "object" && fn_name.vref) {
...
function JMLParser(s, obj) {
    ...
    function tag_attr_parser() {
        ...
        while (i < len && s.charCodeAt(i) <= 32) {
            ...
            while (i < len) {
                ...
                var ep = i;
                var c;
                while (i < len && (c = s.charCodeAt(i)) >= 65 && c <= 122) {
                    i++;
                }ep = i;
                ...
                if (i < len && ep > sp) {
                    ...
                    // then find the : 
                    while (i < len && (c = s.charCodeAt(i)) != 58) {
                        i++;
                    } // skip :
                    if (c == 58) i++;
                    ...
                    if (fc == 34) {
                        ...
                        if (fc == 40) {
                            ...
                            var ep = i;
                            var c;
                            while (i < len && ((c = s.charCodeAt(i)) >= 65 && c <= 90 || c >= 48 && c <= 57 || c == 95 || c == 46 || c == 64 || c >= 97 && c <= 122)) {
                                i++;
                            }ep = i;
                            if (i < len && ep > sp) {
                                curr_node.attrs[s.substring(an_sp, an_ep)] = s.substring(sp, ep);
    ...
    function tag_body_parser() {
        ...
        var c;
        while (i < len && (c = s.charCodeAt(i)) <= 32) {
            i++;
        } // --==(
        if (s.charCodeAt(i) == 45 && s.charCodeAt(i + 1) == 45 && s.charCodeAt(i + 2) == 61 && s.charCodeAt(i + 3) == 61 && s.charCodeAt(i + 4) == 40) {
            ...
            while (i < len && !(s.charCodeAt(i) != 40 && s.charCodeAt(i + 1) == 61 && s.charCodeAt(i + 2) == 61 && s.charCodeAt(i + 3) == 45 && s.charCodeAt(i + 4) == 45)) {
                i++;
            }ep = i;
    ...
    function parse_lisp_expression() {
        ...
        while (i < len) {
            ...
            if ( (c == 34) || (c == 96) || (c == 39)) {
                ...
                var sp = i;
                var ep = i;
                while (i < len && s.charCodeAt(i) != end_d) {
                    i++;
                }ep = i;
                if (i < len) {
                    // string ... 
            ...
            if (fc == 45 && s.charCodeAt(i + 1) >= 46 && s.charCodeAt(i + 1) <= 57 || fc >= 48 && fc <= 57) {
                ...
                while (i < len && (c = s.charCodeAt(i)) > 32 && c != 40 && c != 41) {
                    i++;
                }ep = i;
                if (i < len && ep > sp) {
                ...
                // spaces, ) ( are not allowed
                while (i < len && (c = s.charCodeAt(i)) > 32 && c != 58 && c != 40 && c != 41) {
                    i++;
                }ep = i;
    ...
    function xml_tag_attr_parser() {
        ...
        while (i < len) {
            ...
            while (i < len && ((c = s.charCodeAt(i)) >= 65 && c <= 122 || c == 45)) {
                i++;
            }ep = i;
            ...
            if (i < len && ep > sp) {
                ...
                // then find the = 
                while (i < len && (c = s.charCodeAt(i)) != 61) {
                    i++;
                } // skip :
                if (c == 61) i++;
    ...
    function xml_tag_expression_parser() {
        ...
        while (i < len) {
            ...
            if (cc1 == 60 && cc2 == 47) {
                ...
                var c;
                // approximately legal name chars
                while (i < len && ((c = s.charCodeAt(i)) >= 65 && c <= 90 || // upper case alpha
                c >= 48 && c <= 57 // numbers
                || c == 95 // _
                || c == 46 // .
                || c == 64 // @
            ...
            if (cc1 == 60 && cc2 == 33 && s.charCodeAt(i + 2) == 45 && s.charCodeAt(i + 3) == 45) {
                ...
                while (i < len && !(s.charCodeAt(i) == 45 && s.charCodeAt(i + 1) == 45 && s.charCodeAt(i + 2) == 62)) {
                    i++;
                }
            }
            ...
            if (s.charCodeAt(i) == 60 && s.charCodeAt(i + 1) == 33 && s.charCodeAt(i + 2) == 91 && s.charCodeAt(i + 3) == 67 && s.charCodeAt(i + 4) == 68 && s.charCodeAt(i + 5) == 65 && s.charCodeAt(i + 6) == 84 && s.charCodeAt(i + 7) == 65 && s.charCodeAt(i + 8) == 91) {
                ...
                // "]]>"
                while (i < len && !(s.charCodeAt(i) == 93 && s.charCodeAt(i + 1) == 93 && s.charCodeAt(i + 2) == 62)) {
                    i++;
                }ep = i;
            ...
            if (s.charCodeAt(i) == 60) {
                ...
                var ep = i;
                // read text content...
                while (i < len && s.charCodeAt(i) != 60 && s.charCodeAt(i) != 40) {
                    i++;
                }ep = i;
