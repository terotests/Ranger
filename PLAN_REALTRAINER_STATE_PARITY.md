...
Status: `S0 tehty: referenssi ajettu ja nauhoitettu · S1–S5: koneet ja näkymät tehty, tekstikentät hostattu · puhelimen kuori ja osiot piirretty · joka ruutu ≥ 97 % referenssistä, velho ja vuosilakana 100 %` · 2026-09-03
...
| **S3 — vuosisuunnitelma** *(näkymä tehty)* | vuosi | Tehty: `src/RtYearSheet.rgr` — lista, yksi lakana ruudukkona (rivit × kaudet, `count`/`duration` seedin merkinnöistä), kausi- ja rivieditorit (`SelectCtl` yksikölle ja tyypille, `NumberCtl` kestolle), jaksot malliviikkoineen, paluu dashboardille. Selvitys `app-ranger/lib/yearsheet`ista: se laskee kausien päivämäärät (myös lukitut segmentit) ja metriikat snapshotteista; tähän portattiin yksinkertainen tapaus (alku + kestot peräkkäin) ja kaksi metriikkaa, lukittuja päivämääriä ei. `yearsheet` ja `yearsheet-detail` täsmäävät 100 % järjestyksessä. Jäljellä: "Lisää kausi"- ja AI-modaalit, leikepöytä, poisto | vuosinäkymän ruudukko ja valinnat täsmäävät ✓ |
...
liittäminen, poisto ja "Featuret"-sivu — kaikki nappeja jotka nyt vastaavat
tyhjää. Rivieditorin alarivit (`addRow(parentId)`) ja rivien laajennus
ruudukossa puuttuvat myös; seedin lakanalla on yksi rivi ilman lapsia.
S6:sta on jo se mitä näkymät ovat vaatineet: seedissä on kalenterit
omistajineen, valinta (`selectCalendar`) ja lisäys (`addCalendar`), sekä
lakanan rivit — `appStore`n `calendars`/`selectCalendar`/`addCalendar` ja
`yearsheet-storage`n oletusrivi sen verran kuin S1–S4 tarvitsivat.
