...
//
//   node lib/evg/gl/shift-check.mjs
//
...
// The draws that are NOT in a layer, each with the reason. A post-pass runs
// over the finished surface: the layer's shift is in those pixels already, and
// a pass that applied it again would move the effect twice — once with the
// page and once more on top of it. Each one is named here so that leaving a
// draw out of the rule stays a decision rather than an omission.
const EXPECTED_SHIFTLESS = {
  drawLegacyRipple: "a post-pass over the finished surface — the shift is in the texture",
};

console.log("--- the painter's draws ---");
ok("there are draws to check", draws.length >= 3, draws.length + " found");
for (const d of draws) {
  if (d.name in EXPECTED_SHIFTLESS) {
    ok(`${d.name} is deliberately outside a layer`,
       !d.body.includes("curShift"),
       `evg-webgl.js:${d.line} mentions curShift, but it is listed as ` +
       `shiftless: ${EXPECTED_SHIFTLESS[d.name]}`);
    continue;
  }
