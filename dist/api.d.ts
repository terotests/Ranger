export declare enum RangerAnnType {
    None = 0,
    Expression = 1,
    Type = 2,
    VRef = 3
}
export declare enum RangerMethodCallType {
    None = 0,
    Method = 1,
    Static = 2
}
export declare enum RangerWrap {
    None = 0,
    Optional = 1,
    Unwrap = 2
}
export declare enum RangerVForce {
    None = 0,
    Weak = 1,
    Strong = 2
}
export declare enum RangerNodeRefType {
    NoType = 0,
    Weak = 1,
    Strong = 2,
    Shared = 3,
...
export declare enum RangerContextVarType {
    NoType = 0,
    This = 1,
    ThisProperty = 2,
    NewObject = 3,
...
export declare enum DictNodeType {
    NoType = 0,
    Double = 1,
    Integer = 2,
    String = 3,
...
export declare class InputFileResolver {
    ...
    tryRead(path: string, name: string): string | undefined;
    hostHas(name: string): boolean;
    hostRead(name: string): string | undefined;
    toDictionary(): Record<string, any>;
...
export declare class RangerAppParamDesc {
    ...
    varType: RangerContextVarType;
    refType: RangerNodeRefType;
    initRefType: RangerNodeRefType;
    isParam?: boolean;
    paramIndex: number;
    is_optional: boolean;
    is_mutating: boolean;
...
export declare class RangerAppFunctionDesc extends RangerAppParamDesc {
    ...
    forOtherVersions(ctx: RangerAppWriterContext, cb: (item: RangerAppFunctionDesc) => void): void;
    isFunction(): boolean;
    isClass(): boolean;
    isProperty(): boolean;
}
...
export declare class RangerAppClassDesc extends RangerAppParamDesc {
    ...
    createVariable(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
...
export declare class CodeNode {
    ...
    rg_moved_here: boolean;
    rg_init_fold: boolean;
    rg_init_fields: Array<string>;
    rg_init_values: Array<CodeNode>;
    rg_init_stmts: Array<CodeNode>;
    rg_init_covers_all: boolean;
    rg_init_folded: boolean;
    ...
    chlen(): number;
    forTree(callback: (item: CodeNode, i: number) => void): void;
...
export declare class RangerAppWriterContext {
    ...
    cachedRoot?: RangerAppWriterContext;
    ...
    getOpFns(name: string): Array<CodeNode>;
    ...
    getTypedNodes(name: string): Array<CodeNode>;
    addTypedNode(name: string, op: CodeNode): void;
    getPluginNodes(name: string): Array<CodeNode>;
    addPluginNode(name: string, op: CodeNode): void;
    addOperator(op: RangerAppOperatorDesc): void;
    getAllOperators(): Array<RangerAppOperatorDesc>;
    ...
    pushAndCollectAst(rootNode: CodeNode, wr: CodeWriter): void;
    pushAndCompileAst(rootNode: CodeNode, wr: CodeWriter): void;
    pushAst(source_code: string, node: CodeNode, wr: CodeWriter): void;
    pushAndCollectCode(source_code: string, wr: CodeWriter): void;
    pushCode(source_code: string, wr: CodeWriter): void;
    ...
    inLambda(): boolean;
    variableTypeUsage(): Array<string>;
    writeContextVars(wr: CodeWriter): void;
    writeContextInfo(wr: CodeWriter): void;
    getContextInfo(): string;
    isCapturing(): boolean;
    forkWithOps(opNode: CodeNode): RangerAppWriterContext;
    getOperatorDef(): RangerActiveOperators;
    getOperators(name: string): Array<CodeNode>;
    ...
    findOperatorsWithName(name: string): Array<CodeNode>;
    findClassWithSign(node: CodeNode): RangerAppClassDesc;
    createSignature(origClass: string, classSig: string): string;
    createStaticMethod(withName: string, currC: RangerAppClassDesc, nameNode: CodeNode, argsNode: CodeNode, fnBody: CodeNode, parser: RangerFlowParser, wr: CodeWriter): RangerAppFunctionDesc;
    canUseTypeInference(nameNode: CodeNode): boolean;
    createOpStaticClass(name: string): RangerAppClassDesc;
    createTraitInstanceClass(traitName: string, instanceName: string, initParams: CodeNode, flowParser: RangerFlowParser, wr: CodeWriter): RangerAppClassDesc | undefined;
    createGenericClassInstance(templateName: string, instanceName: string, typeArgs: CodeNode, flowParser: RangerFlowParser, wr: CodeWriter): RangerAppClassDesc | undefined;
...
export declare class CodeWriter {
    ...
    lineMark(): number;
    lineMarkIsParenGroup(mark: number): boolean;
    stripLineMarkParens(mark: number): boolean;
...
export declare class RangerLispParser {
    ...
    skip_space(s: Uint8Array, is_block_parent: boolean): boolean;
    attachParenMember(closed: CodeNode, listNode: CodeNode): void;
    end_expression(consumeCurrent: boolean): boolean;
    getOperator(s: Uint8Array, disabled: boolean): number;
    isOperator(s: Uint8Array, disabled: boolean): number;
    ...
    parse_attributes(s: Uint8Array): boolean;
    parseXML(s: Uint8Array): void;
    parse(disable_ops: boolean): void;
    parseBuf(s: Uint8Array, disable_ops: boolean): void;
...
export declare class RangerProcessProcSend {
    ...
    static resolveTargetClass(parser: RangerFlowParser, target: CodeNode, processClasses: Array<RangerAppClassDesc>, ctx: RangerAppWriterContext, wr: CodeWriter, errNode: CodeNode): RangerAppClassDesc | undefined;
    static matchHandler(parser: RangerFlowParser, recvClass: RangerAppClassDesc, methodName: string, argNodes: Array<CodeNode>, ctx: RangerAppWriterContext, wr: CodeWriter, errNode: CodeNode): RangerAppFunctionDesc | undefined;
    ...
    static transform(parser: RangerFlowParser, node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): boolean;
}
export declare class RangerProcessProcStartCheck {
    constructor();
    static validate(parser: RangerFlowParser, node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): boolean;
}
export declare class RangerProcessCodegen {
    ...
    writeNewArgs(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter, writer: RangerGenericClassWriter): void;
    ...
    writeWrappedNewCallEs6(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter, writer: RangerGenericClassWriter): void;
    writeWrappedNewCallKotlin(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter, writer: RangerGenericClassWriter): void;
    writeWrappedNewCallSwift6(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter, writer: RangerGenericClassWriter): void;
    writeWrappedNewCall(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter, lang: string, writer: RangerGenericClassWriter): void;
...
export declare class RangerServiceBuilder {
    ...
    CreateServices(parser: RangerFlowParser, ctx: RangerAppWriterContext, orig_wr: CodeWriter): void;
}
...
export declare class PkgImport {
    ...
    static readText(env: InputEnv, dir: string, name: string): string;
    ...
    static resolve(env: InputEnv, startDir: string, spec: string, libraryPaths: Array<string>): PkgHit;
    static resolveIn(env: InputEnv, manDir: string, spec: string): PkgHit;
...
export declare class RangerFlowParser {
    ...
    hoistParenthesisedReceivers(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    WalkNodeChildren(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    WalkNode(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): boolean;
    getVoidNameSignature(): string;
    getNameSignature(node: CodeNode): string;
    getArgsSignature(node: CodeNode): string;
    getThisName(): string;
    GetProperty(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    WriteVRef(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    EnterFnParts(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): RangerFnParts;
    Constructor(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    WriteScalarValue(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    normalizeNewArgList(node: CodeNode): void;
    cmdNew(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    ...
    CreateCTTI(node: CodeNode, ctx: RangerAppWriterContext, orig_wr: CodeWriter): void;
    CreateRTTI(node: CodeNode, ctx: RangerAppWriterContext, orig_wr: CodeWriter): void;
    walkXmlCreate(xmlNode: CodeNode, regName: string, in_currCnt: number, opBody: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): number;
    markAsyncFrom(f: RangerAppFunctionDesc, visited: Array<RangerAppFunctionDesc>): void;
    markAsyncFromVariant(f: RangerAppFunctionDesc, visited: Array<RangerAppFunctionDesc>, ctx: RangerAppWriterContext): void;
    markCalledFromMain(f: RangerAppFunctionDesc, ctx: RangerAppWriterContext): void;
    SolveAsyncFuncs(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    matchMethodCall(cl: RangerAppClassDesc, methodName: string, callArgs: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter, errNode: CodeNode): RangerAppFunctionDesc | undefined;
    cmdCall(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): boolean;
    matchLambdaArgs(n1: CodeNode, n2: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): boolean;
    testLambdaCallArgs(lambda_expression: CodeNode, callParams: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): boolean;
    cmdLocalCall(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): boolean;
    transformImmutableAssigment(node: CodeNode): CodeNode;
    transformDotMethodCallExpr(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): boolean;
    ...
    cmdAssign(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    EnterTemplateClass(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    EnterClass(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    walkFunctionBody(m: RangerAppFunctionDesc, fnBody: CodeNode, ctx: RangerAppWriterContext, subCtx: RangerAppWriterContext, wr: CodeWriter): void;
    EnterMethod(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    EnterStaticMethod(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    DefineArrowOpFn(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    DefineOpFn(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    testCompile(opFn: CodeNode, node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): {
        [key: string]: CodeNode;
    };
    inlineStaticEligible(m: RangerAppFunctionDesc, fnNode: CodeNode, ctx: RangerAppWriterContext): boolean;
    tryInlineTrivialStatic(node: CodeNode, fnNode: CodeNode, m: RangerAppFunctionDesc, ctx: RangerAppWriterContext, wr: CodeWriter): boolean;
    TransformOpFn(opFnList: Array<CodeNode>, origNode: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    cmdArray(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    EnterLambdaMethod(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    annotationIsTypeArgs(tAnn: CodeNode, ctx: RangerAppWriterContext): boolean;
    normalizeTypeArg(ch: CodeNode): void;
    genericInstanceName(baseName: string, tAnn: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): string;
    typeArgKey(spelling: string): string;
    CheckVRefTypeAnnotationOf(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): boolean;
    CheckTypeAnnotationOf(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): boolean;
    matchNode(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): boolean;
    StartWalk(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    ...
    findLanguageOper(details: CodeNode, ctx: RangerAppWriterContext, opDef: CodeNode): CodeNode | undefined;
    buildMacro(langOper: CodeNode, args: CodeNode, ctx: RangerAppWriterContext): CodeNode;
    operandIsNonOptionalForNullCheck(node: CodeNode): boolean;
    stdParamMatch(callArgs: CodeNode, inCtx: RangerAppWriterContext, wr: CodeWriter, require_all_match: boolean): boolean;
    ...
    rewriteToGroupFieldGet(node: CodeNode, receiver: CodeNode, fieldName: string, groupCls: string, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    rewriteToGroupWiden(node: CodeNode, targetGroupCls: string, sourceGroupCls: string, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    ...
    CheckTargetSupport(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    CheckNoLossyTry(node: CodeNode, ctx: RangerAppWriterContext, allowed: boolean): void;
    CollectMethods(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    defineFunctionParam(method: RangerAppFunctionDesc, arg: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    spliceFunctionBody(startIndex: number, node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): CodeNode;
    CreateFunctionObject(orig_node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): RangerAppFunctionDesc;
    WalkCollectMethods(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    ...
    convertToUnion(unionName: string, node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    transformMethodToLambda(node: CodeNode, vFnDef: RangerAppFunctionDesc, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    areEqualTypes(n1: CodeNode, n2: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): boolean;
    ...
    finalizeRecordClasses(ctx: RangerAppWriterContext, wr: CodeWriter): void;
    expandRecordCtorArgsIfNeeded(cl: RangerAppClassDesc, fnDescr: RangerAppFunctionDesc, params: CodeNode, node: CodeNode): void;
    buildRecordConstructor(cl: RangerAppClassDesc, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    prepareImport(ctx: RangerAppWriterContext, spec: string): PkgHit;
    mergeImports(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
}
...
export declare class RangerGenericClassWriter {
    ...
    writeCallReceiver(obj: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    ...
    addSystemImport(cl: RangerAppClassDesc, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    EncodeString(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): string;
    CustomOperator(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    beforeOperatorStatement(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    WriteSetterVRef(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    writeArrayTypeDef(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    WriteEnum(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    ...
    renderDocExample(fd: RangerAppFunctionDesc, ctx: RangerAppWriterContext, outer: CodeWriter): string;
    trimTrailingBlank(text: string): string;
    WalkNode(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    writeTypeDef(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    writeRawTypeDef(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    adjustType(tn: string): string;
    defValueHasSideEffects(value: CodeNode): boolean;
    writeSideEffectOnlyStmt(value: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    WriteVRef(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    writeVarDef(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    CreateCallExpression(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    CreateMethodCall(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    CreatePropertyGet(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    isPackaged(ctx: RangerAppWriterContext): boolean;
    CreateUnions(parser: RangerFlowParser, ctx: RangerAppWriterContext, orig_wr: CodeWriter): void;
    CreateServices(parser: RangerFlowParser, ctx: RangerAppWriterContext, orig_wr: CodeWriter): void;
    CreatePages(parser: RangerFlowParser, ctx: RangerAppWriterContext, orig_wr: CodeWriter): void;
    CreatePage(parser: RangerFlowParser, node: CodeNode, ctx: RangerAppWriterContext, orig_wr: CodeWriter): void;
    CreateLambdaCall(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    CreateLambda(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    writeFnCall(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    walkNewArgForProcess(n: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    tryWriteProcessNewCall(procNewNode: CodeNode, procNewCtx: RangerAppWriterContext, outWr: CodeWriter): boolean;
    writeNewCall(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    writeInterface(cl: RangerAppClassDesc, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    disabledVarDef(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    writeArrayLiteral(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    writeClass(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
}
export declare class TraitInterfaceAnalysis {
    ifaceTraits: {
        [key: string]: boolean;
    };
    computed: boolean;
    constructor();
    nodeNamesType(node: CodeNode, tName: string): boolean;
    fnsNameType(fns: Array<RangerAppFunctionDesc>, tName: string): boolean;
    compute(ctx: RangerAppWriterContext): void;
    isInterface(tName: string, ctx: RangerAppWriterContext): boolean;
    basesOf(cl: RangerAppClassDesc, ctx: RangerAppWriterContext): Array<string>;
}
export declare class ForLoopShape {
    constructor();
    treeMentions(node: CodeNode, name: string): boolean;
    bodyTouchesCollection(body: CodeNode, coll: CodeNode): boolean;
    bodyReadsIndex(node: CodeNode): boolean;
    canIterate(node: CodeNode): boolean;
    canIterateWithIndex(node: CodeNode): boolean;
    bodyAssignsElement(node: CodeNode): boolean;
}
export declare class EnumNativeAnalysis {
    nativeEnums: {
        [key: string]: boolean;
    };
    computed: boolean;
    ...
    fallbackType: string;
    constructor();
    nodeIsEnumOf(node: CodeNode, enumName: string, ctx: RangerAppWriterContext): boolean;
    enumNameOf(node: CodeNode, ctx: RangerAppWriterContext): string;
    collectCaseEnums(node: CodeNode, found: {
        [key: string]: boolean;
    }): void;
    operandIsSafe(headVref: string): boolean;
    unsafeReason(node: CodeNode, enumName: string, ctx: RangerAppWriterContext): string;
    compute(ctx: RangerAppWriterContext): void;
    scanFns(fns: Array<RangerAppFunctionDesc>, enumName: string, ctx: RangerAppWriterContext, reasonIn: string): string;
    isNative(enumName: string, ctx: RangerAppWriterContext): boolean;
}
...
export declare class RangerJava7ClassWriter extends RangerGenericClassWriter {
    ...
    javaTraits: TraitInterfaceAnalysis;
    javaLoops: ForLoopShape;
    java_traits_written: boolean;
    javaEnums: EnumNativeAnalysis;
    java_enums_written: boolean;
    java_json_written: boolean;
    java_args_written: boolean;
    constructor();
    getSignatureInterface(s: string): string;
    adjustType(tn: string): string;
    javaEnumIsNative(enumName: string, ctx: RangerAppWriterContext): boolean;
    writeJavaNativeEnums(ctx: RangerAppWriterContext, wr: CodeWriter): void;
    writeJavaTraitInterfaces(ctx: RangerAppWriterContext, wr: CodeWriter): void;
    javaProgramUsesJson(ctx: RangerAppWriterContext): boolean;
    writeJavaArgsHolder(ctx: RangerAppWriterContext, wr: CodeWriter): void;
    writeJavaJsonSupport(ctx: RangerAppWriterContext, wr: CodeWriter): void;
    getObjectTypeString2(type_string: string, ctx: RangerAppWriterContext, wr: CodeWriter): string;
    getTypeString(type_string: string): string;
    writeTypeDef(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    WriteVRef(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    disabledVarDef(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    ...
    javaWriteOperand(arg: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    javaWriteForLoop(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    CustomOperator(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    ...
    writeClass(node: CodeNode, ctx: RangerAppWriterContext, orig_wr: CodeWriter): void;
    CreateServices(parser: RangerFlowParser, ctx: RangerAppWriterContext, orig_wr: CodeWriter): void;
    CreatePages(parser: RangerFlowParser, ctx: RangerAppWriterContext, orig_wr: CodeWriter): void;
    CreatePage(parser: RangerFlowParser, node: CodeNode, ctx: RangerAppWriterContext, orig_wr: CodeWriter): void;
}
export declare class RangerSwift3ClassWriter extends RangerGenericClassWriter {
    ...
    sw3Enums: EnumNativeAnalysis;
    swift3_enums_written: boolean;
    swift_unions_written: boolean;
    constructor();
    sw3EnumIsNative(enumName: string, ctx: RangerAppWriterContext): boolean;
    writeSwiftNativeEnums(ctx: RangerAppWriterContext, wr: CodeWriter): void;
    ...
    swiftWriteUnionValue(targetTypeName: string, nVal: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): boolean;
    swiftWriteUnionArg(arg: RangerAppParamDesc, nVal: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): boolean;
    adjustType(tn: string): string;
    getObjectTypeString(type_string: string, ctx: RangerAppWriterContext): string;
    getTypeString(type_string: string): string;
    writeTypeDef(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    ...
    writeVarDef(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    writeArgsDef(fnDesc: RangerAppFunctionDesc, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    writeArgsDefWithLocals(fnDesc: RangerAppFunctionDesc, localFnDesc: RangerAppFunctionDesc, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    CreateCallExpression(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    writeFnCall(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    CreateLambdaCall(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    CreateLambda(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
...
export declare class RangerSwift6ClassWriter extends RangerGenericClassWriter {
    ...
    header_created: boolean;
    swift_unions_written: boolean;
    swiftTraits: TraitInterfaceAnalysis;
    swiftLoopLoops: ForLoopShape;
    swift_traits_written: boolean;
    swEnums: EnumNativeAnalysis;
    swift_enums_written: boolean;
    constructor();
    swiftThrowsMark(variant: RangerAppFunctionDesc): string;
    adjustType(tn: string): string;
    ...
    swiftWriteMutableParamCopies(fnDesc: RangerAppFunctionDesc, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    paramNeedsInout(arg: RangerAppParamDesc): boolean;
    swEnumIsNative(enumName: string, ctx: RangerAppWriterContext): boolean;
    writeSwiftNativeEnums(ctx: RangerAppWriterContext, wr: CodeWriter): void;
    writeSwiftTraitProtocols(ctx: RangerAppWriterContext, wr: CodeWriter): void;
    writeSwiftUnionEnums(ctx: RangerAppWriterContext, wr: CodeWriter): void;
    swiftDeclaredClassOf(nVal: CodeNode): string;
    swiftUnionHasMember(ucl: RangerAppClassDesc, memberName: string): boolean;
    swiftWriteUnionValue(targetTypeName: string, nVal: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): boolean;
    ...
    writeArrayLiteral(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    haveSameSig(fn1: RangerAppFunctionDesc, fn2: RangerAppFunctionDesc, ctx: RangerAppWriterContext): boolean;
    swiftLoopWriteOperand(arg: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    swiftLoopWriteBinding(arg: CodeNode, body: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    swiftLoopWriteForLoop(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    CustomOperator(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    writeClass(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    resolveMethodFnDesc(methodNode: CodeNode, ctx: RangerAppWriterContext): RangerAppFunctionDesc | undefined;
    CreateMethodCall(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
}
export declare class CppValueClassAnalysis {
    valueClasses: {
        [key: string]: boolean;
    };
    computed: boolean;
    constructor();
    nodeIsClassOf(node: CodeNode, className: string): boolean;
    optionalOf(node: CodeNode, className: string): boolean;
    unsafeReason(node: CodeNode, className: string, ctx: RangerAppWriterContext): string;
    scanFns(fns: Array<RangerAppFunctionDesc>, className: string, ownName: string, ctx: RangerAppWriterContext, reasonIn: string): string;
    mentionsBareThis(node: CodeNode): boolean;
    fieldReason(cl: RangerAppClassDesc, className: string): string;
    classReason(cl: RangerAppClassDesc, ctx: RangerAppWriterContext): string;
    compute(ctx: RangerAppWriterContext): void;
    fieldOfOtherReason(ocl: RangerAppClassDesc, className: string): string;
    isValue(className: string, ctx: RangerAppWriterContext): boolean;
}
export declare class RangerCppClassWriter extends RangerGenericClassWriter {
    ...
    cppEnums: EnumNativeAnalysis;
    cppTraits: TraitInterfaceAnalysis;
    cppValues: CppValueClassAnalysis;
    cppLoops: ForLoopShape;
    constructor();
    cppEnumIsNative(enumName: string, ctx: RangerAppWriterContext): boolean;
    cppTraitIsInterface(tName: string, ctx: RangerAppWriterContext): boolean;
    cppClassIfaceBases(cl: RangerAppClassDesc, ctx: RangerAppWriterContext): Array<string>;
    cppIsValueClass(cl: RangerAppClassDesc, ctx: RangerAppWriterContext): boolean;
    writeCppTraitInterfaces(ctx: RangerAppWriterContext, wr: CodeWriter): void;
    ...
    cppEmitOptionalPrimitive(wr: CodeWriter): void;
    cppEmitArgRef(wr: CodeWriter): void;
    writeTypeDef(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    ...
    writeReturnTypeDef(variant: RangerAppFunctionDesc, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    cppSegmentIsValueCase(node: CodeNode, idx: number, ctx: RangerAppWriterContext): boolean;
    WriteVRef(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    writeVarDef(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    disabledVarDef(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    CreateCallExpression(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    cppWriteCmpOperand(o: CodeNode, otherIsLiteral: boolean, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    cppWriteOperand(arg: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    cppForItemIsScalar(item: CodeNode): boolean;
    cppWriteForItemType(item: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    cppWriteForLoop(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    CustomOperator(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    CreateMethodCall(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    CreatePropertyGet(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    CreateLambdaCall(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    ...
    cppNodeUsesHash(node: CodeNode): boolean;
    cppScanFnsForHash(fns: Array<RangerAppFunctionDesc>): boolean;
    cppProgramUsesHash(ctx: RangerAppWriterContext): boolean;
    cppNodeNamesType(node: CodeNode, tn: string): boolean;
    cppScanFnsForType(fns: Array<RangerAppFunctionDesc>, tn: string): boolean;
    cppProgramUsesType(tn: string, ctx: RangerAppWriterContext): boolean;
    cppNodeHasOptionalUnion(node: CodeNode, ctx: RangerAppWriterContext): boolean;
    cppScanFnsForOptionalUnion(fns: Array<RangerAppFunctionDesc>, ctx: RangerAppWriterContext): boolean;
    cppProgramUsesOptionalUnion(ctx: RangerAppWriterContext): boolean;
    cppProgramHasWeakField(ctx: RangerAppWriterContext): boolean;
    writeCppWeakHelper(wr: CodeWriter): void;
    writeCppHeaderVar(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter, do_initialize: boolean): void;
    ...
    cppMutatedValueClassParam(arg: RangerAppParamDesc, ctx: RangerAppWriterContext): boolean;
    ...
    writeNewCall(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    writeArrayLiteral(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    cppUsesThisValue(body: CodeNode): boolean;
    cppNeedsSharedFromThis(node: CodeNode): boolean;
    writeClassHeader(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    CreateUnions(parser: RangerFlowParser, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    writeClass(node: CodeNode, ctx: RangerAppWriterContext, orig_wr: CodeWriter): void;
}
...
export declare class RangerRustClassWriter extends RangerGenericClassWriter {
    ...
    rust_trait_self_is_handle: boolean;
    ...
    rustClippyNoteWritten: boolean;
    rustIdentNames: {
        [key: string]: boolean;
    };
    rustIdentNamesDone: boolean;
    rustEnums: EnumNativeAnalysis;
    rust_in_cell_assign: boolean;
    rust_last_recv_tmp: string;
    rust_selfrc_path_only: boolean;
    rust_computing_family_mut: boolean;
    constructor();
    lineEnding(): string;
    rustFillIdentNames(ctx: RangerAppWriterContext): void;
    rustCollectFnNames(fns: Array<RangerAppFunctionDesc>): void;
    rustCollectVrefs(node: CodeNode): void;
    rustSnakeCase(n: string): string;
    adjustIdentOrType(n: string, ctx: RangerAppWriterContext): string;
    adjustType(tn: string): string;
    adjustReserved(tn: string): string;
    WriteScalarValue(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    rustStaticStrRead(node: CodeNode): boolean;
    rustWriteStaticStrValue(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    ...
    CreateLambdaCall(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    CreateLambda(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    writeRustLambdaType(expression_value: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    ...
    WriteVRef(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    writeRustFnClose(variant: RangerAppFunctionDesc, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    writeRustReturnType(variant: RangerAppFunctionDesc, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    rustExprReadsThrough(node: CodeNode, name: string): boolean;
    rustRhsReadsSharedCell(node: CodeNode): boolean;
    rustFoldedInitIsWritable(node: CodeNode, ctx: RangerAppWriterContext): boolean;
    rustNeedsNonSnakeCase(): boolean;
    rustNeedAllow(name: string, wr: CodeWriter): void;
    rustNeedClippyAllow(name: string, wr: CodeWriter): void;
    rustNmIsName(seg: string, a: string, b: string): boolean;
    rustLocalNeedsMut(srcName: string, cName: string, typeName: string, ctx: RangerAppWriterContext): boolean;
    rustLocalPathUseAfter(srcName: string, cName: string, minLine: number, ctx: RangerAppWriterContext): boolean;
    rustFoldedLocalIsWrittenAgain(srcName: string, cName: string, node: CodeNode, ctx: RangerAppWriterContext): boolean;
    rustWriteFoldedInit(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    writeVarDef(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    writeArgsDef(fnDesc: RangerAppFunctionDesc, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    ...
    rustStaticCallClassName(cc: RangerAppClassDesc, fc: CodeNode, ctx: RangerAppWriterContext): string;
    rustStaticCallBlockedByHandle(fc: CodeNode, ctx: RangerAppWriterContext): boolean;
    ...
    rustExtractSelfCallConflicts(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    rustTailBorrowsLocal(node: CodeNode): boolean;
    walkRustFnBody(fnB: CodeNode, sCtx: RangerAppWriterContext, wr: CodeWriter): void;
    rustIsSelfCallNode(n: CodeNode): boolean;
    opWritesOwnParens(opName: string, node: CodeNode, ctx: RangerAppWriterContext): boolean;
    rustWriteBitOperand(o: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    rustWriteIDivLeft(o: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    ...
    rustWriteCmpOperand(o: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    rustWriteCastOperandF64(o: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    ...
    writeRustFormatOps(ops: Array<CodeNode>, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    rustPlainScalarPath(n: CodeNode): boolean;
    rustTryCompoundAssign(left: CodeNode, right: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): boolean;
    rustArgNeedsCellWrap(arg: RangerAppParamDesc, nVal: CodeNode, ctx: RangerAppWriterContext): boolean;
    rustWalkOperand(n: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    ...
    beforeOperatorStatement(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    CustomOperator(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    rustWriteOperand(arg: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    rustTreeMentions(node: CodeNode, name: string): boolean;
    rustForItemIsCopyScalar(item: CodeNode): boolean;
    rustForItemClassName(item: CodeNode, ctx: RangerAppWriterContext): string;
    rustClassFieldIsCopy(clName: string, field: string, ctx: RangerAppWriterContext): boolean;
    rustForBodyOnlyReadsCopyFields(body: CodeNode, itemName: string, clName: string, ctx: RangerAppWriterContext): boolean;
    rustForBodyTouchesCollection(body: CodeNode, coll: CodeNode): boolean;
    rustForCollBorrows(coll: CodeNode, ctx: RangerAppWriterContext): boolean;
    rustWriteForLoop(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    ...
    writeStructFieldType(p: RangerAppParamDesc, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    writeStructFieldTypeInner(p: RangerAppParamDesc, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    rustSegThroughTrait(node: CodeNode, idx: number, ctx: RangerAppWriterContext): boolean;
    writeStructField(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    writeTraitFieldAccessorDecls(cl: RangerAppClassDesc, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    writeTraitFieldAccessorImpls(cl: RangerAppClassDesc, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    rustClassIsEmitted(cl: RangerAppClassDesc): boolean;
    rustIfWithoutElse(n: CodeNode): boolean;
    rustFnBodyHasCollapsibleIf(m: RangerAppFunctionDesc): boolean;
    rustNeedsCollapsibleIf(ctx: RangerAppWriterContext): boolean;
    rustParamIsVecRef(pa: RangerAppParamDesc): boolean;
    rustNeedsPtrArg(ctx: RangerAppWriterContext): boolean;
    rustFnHasDeadStore(m: RangerAppFunctionDesc, ctx: RangerAppWriterContext): boolean;
    rustNeedsUnusedAssignments(ctx: RangerAppWriterContext): boolean;
    rustNeedsUnusedVariables(ctx: RangerAppWriterContext): boolean;
    rustNeedsTooManyArgs(ctx: RangerAppWriterContext): boolean;
    rustNeedsRcHeader(ctx: RangerAppWriterContext): boolean;
    ...
    rustTraitFamilyIsReentrant(cl: RangerAppClassDesc, ctx: RangerAppWriterContext): boolean;
    rustClassInReentrantTrait(cl: RangerAppClassDesc, ctx: RangerAppWriterContext): boolean;
    ...
    rustSelfRcArgBorrows(node: CodeNode, fc: CodeNode, ctx: RangerAppWriterContext): boolean;
    writeSelfRcReceiverHoist(node: CodeNode, fc: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): boolean;
    closeSelfRcReceiverHoist(opened: boolean, wr: CodeWriter): void;
    ...
    writeSingletonAccessor(cl: RangerAppClassDesc, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    rustLibraryMode(ctx: RangerAppWriterContext): boolean;
    rustPubPrefix(ctx: RangerAppWriterContext): string;
    rustTraitIsInterface(name: string, ctx: RangerAppWriterContext): boolean;
    rustEnumNameOf(node: CodeNode, ctx: RangerAppWriterContext): string;
    rustEnumIsNative(enumName: string, ctx: RangerAppWriterContext): boolean;
    rustNodeUsesHash(node: CodeNode): boolean;
    rustNodeUsesStringIndex(node: CodeNode): boolean;
    rustHeaderHelperNeeds(ctx: RangerAppWriterContext): number;
    rustScanFnList(fns: Array<RangerAppFunctionDesc>, resIn: number): number;
    writeClass(node: CodeNode, ctx: RangerAppWriterContext, orig_wr: CodeWriter): void;
    ...
    rustWriteUnionValue(targetTypeName: string, nVal: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): boolean;
    ...
    rustWriteUnionArg(arg: RangerAppParamDesc, nVal: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): boolean;
    rustWriteScrutineeType(scrut: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    rustWriteUnionCase(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    rustWriteCaseBody(body: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
}
export declare class RangerKotlinClassWriter extends RangerGenericClassWriter {
    compiler?: LiveCompiler;
    kotlin_unions_written: boolean;
    ktTraits: TraitInterfaceAnalysis;
    ktLoops: ForLoopShape;
    kotlin_traits_written: boolean;
    ktEnums: EnumNativeAnalysis;
    kotlin_enums_written: boolean;
    kotlin_package_written: boolean;
    constructor();
    ktEnumIsNative(enumName: string, ctx: RangerAppWriterContext): boolean;
    writeKotlinNativeEnums(ctx: RangerAppWriterContext, wr: CodeWriter): void;
    writeKotlinTraitInterfaces(ctx: RangerAppWriterContext, wr: CodeWriter): void;
    ...
    getTypeString(type_string: string): string;
    writeLambdaType(expression_value: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    writeTypeDef(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    CreateLambda(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    WriteVRef(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    EncodeString(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): string;
    ...
    kotlinWriteMutableParamCopies(fnDesc: RangerAppFunctionDesc, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    writeArgsDef(fnDesc: RangerAppFunctionDesc, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    writeFnCall(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    writeNewCall(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    writeClass(node: CodeNode, ctx: RangerAppWriterContext, orig_wr: CodeWriter): void;
    ktWriteOperand(arg: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    ktWriteForLoop(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    ktWriteSwitch(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    CustomOperator(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
}
export declare class RangerDartClassWriter extends RangerGenericClassWriter {
    ...
    dartTraits: TraitInterfaceAnalysis;
    dartLoops: ForLoopShape;
    dart_traits_written: boolean;
    dartEnums: EnumNativeAnalysis;
    dart_enums_written: boolean;
    constructor();
    dartEnumIsNative(enumName: string, ctx: RangerAppWriterContext): boolean;
    writeDartNativeEnums(ctx: RangerAppWriterContext, wr: CodeWriter): void;
    writeDartTraitInterfaces(ctx: RangerAppWriterContext, wr: CodeWriter): void;
    ...
    writeFnCall(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    writeNewCall(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    writePubspec(node: CodeNode, ctx: RangerAppWriterContext, orig_wr: CodeWriter): void;
    writeClass(node: CodeNode, ctx: RangerAppWriterContext, orig_wr: CodeWriter): void;
    dartWriteOperand(arg: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    dartWriteForLoop(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    CustomOperator(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
}
export declare class RangerCSharpClassWriter extends RangerGenericClassWriter {
    ...
    csTraits: TraitInterfaceAnalysis;
    csLoops: ForLoopShape;
    csharp_traits_written: boolean;
    csEnums: EnumNativeAnalysis;
    csharp_enums_written: boolean;
    constructor();
    adjustType(tn: string): string;
    csEnumIsNative(enumName: string, ctx: RangerAppWriterContext): boolean;
    writeCSharpNativeEnums(ctx: RangerAppWriterContext, wr: CodeWriter): void;
    writeCSharpTraitInterfaces(ctx: RangerAppWriterContext, wr: CodeWriter): void;
    ...
    writeClass(node: CodeNode, ctx: RangerAppWriterContext, orig_wr: CodeWriter): void;
    csWriteOperand(arg: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    csWriteForLoop(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    CustomOperator(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
}
export declare class RangerScalaClassWriter extends RangerGenericClassWriter {
    compiler?: LiveCompiler;
    init_done: boolean;
    scalaTraits: TraitInterfaceAnalysis;
    scala_traits_written: boolean;
    scEnums: EnumNativeAnalysis;
    scala_enums_written: boolean;
    scLoops: ForLoopShape;
    constructor();
    getObjectTypeString(type_string: string, ctx: RangerAppWriterContext): string;
    getTypeString(type_string: string): string;
    writeTypeDef(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    writeTypeDefNoOption(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    WriteVRef(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    writeVarDef(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    writeArgsDef(fnDesc: RangerAppFunctionDesc, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    writeFnEnd(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    writeFnStart(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    CustomOperator(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    CreateLambda(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    writeArrayLiteral(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    scEnumIsNative(enumName: string, ctx: RangerAppWriterContext): boolean;
    writeScalaNativeEnums(ctx: RangerAppWriterContext, wr: CodeWriter): void;
    writeScalaTraitDefs(ctx: RangerAppWriterContext, wr: CodeWriter): void;
    writeClass(node: CodeNode, ctx: RangerAppWriterContext, orig_wr: CodeWriter): void;
}
export declare class RangerGolangClassWriter extends RangerGenericClassWriter {
    ...
    goTraits: TraitInterfaceAnalysis;
    goLoops: ForLoopShape;
    did_write_nullable: boolean;
    did_write_sseclient: boolean;
    go_unions_written: boolean;
    goEnums: EnumNativeAnalysis;
    go_enums_written: boolean;
    httpServerWriter: RangerGolangHttpServerWriter;
    constructor();
    goEnumIsNative(enumName: string, ctx: RangerAppWriterContext): boolean;
    goEnumMemberName(enumName: string, memberName: string, ctx: RangerAppWriterContext): string;
    writeGoNativeEnums(ctx: RangerAppWriterContext, wr: CodeWriter): void;
    ...
    goWriteUnionValue(targetTypeName: string, nVal: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): boolean;
    goWriteUnionArg(arg: RangerAppParamDesc, nVal: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): boolean;
    WriteScalarValue(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    ...
    WriteVRef(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    WriteSetterVRef(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    goExtractAssign(value: CodeNode, p: RangerAppParamDesc, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    ...
    CreateLambdaCall(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    CreateLambda(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    goWriteForLoop(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    goWriteOperand(arg: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    goWriteIndexForLoop(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    CustomOperator(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    writeInterface(cl: RangerAppClassDesc, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    writeSingletonAccessor(cl: RangerAppClassDesc, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    goNodeUsesOptional(node: CodeNode): boolean;
    goScanFnsForOptional(fns: Array<RangerAppFunctionDesc>): boolean;
    goProgramUsesOptional(ctx: RangerAppWriterContext): boolean;
    writeGoTraitInterfaces(ctx: RangerAppWriterContext, wr: CodeWriter): void;
    writeClass(node: CodeNode, ctx: RangerAppWriterContext, orig_wr: CodeWriter): void;
}
...
export declare class RangerJavaScriptHttpServerWriter {
    constructor();
    isHttpServerClass(cl: RangerAppClassDesc): boolean;
    getRouteMethod(fnDesc: RangerAppFunctionDesc): string;
    getRoutePath(fnDesc: RangerAppFunctionDesc): string;
    methodNameOf(fnDesc: RangerAppFunctionDesc): string;
    writeAdapter(wr: CodeWriter, hasSSE: boolean): void;
    writeServerStart(serverArg: CodeNode, node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter, writer: RangerJavaScriptClassWriter): void;
}
export declare class RangerPHPClassWriter extends RangerGenericClassWriter {
    compiler?: LiveCompiler;
    thisName: string;
    wrote_header: boolean;
    phpEnums: EnumNativeAnalysis;
    php_enums_written: boolean;
    phpLoops: ForLoopShape;
    constructor();
    phpWriteForLoop(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    CustomOperator(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    phpEnumIsNative(enumName: string, ctx: RangerAppWriterContext): boolean;
    writePhpNativeEnums(ctx: RangerAppWriterContext, wr: CodeWriter): void;
    adjustType(tn: string): string;
    EncodeString(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): string;
    WriteScalarValue(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    WriteVRef(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    writeVarInitDef(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    writeVarDef(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    disabledVarDef(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    CreateMethodCall(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    CreatePropertyGet(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    CreateLambdaCall(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    CreateLambda(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    phpTypeNameFor(tn: string, ctx: RangerAppWriterContext): string;
    phpPropertyType(nn: CodeNode, ctx: RangerAppWriterContext): string;
    phpPropertyDefault(pt: string): string;
...
export declare class RangerPythonClassWriter extends RangerGenericClassWriter {
    ...
    thisName: string;
    wrote_header: boolean;
    pyEnums: EnumNativeAnalysis;
    python_enums_written: boolean;
    pyTraits: TraitInterfaceAnalysis;
    python_traits_written: boolean;
    pyLoops: ForLoopShape;
    pyHoistWriters: Array<CodeWriter>;
    pyLambdaCounter: number;
    constructor();
    writePythonTraitProtocols(ctx: RangerAppWriterContext, wr: CodeWriter): void;
    pyEnumIsNative(enumName: string, ctx: RangerAppWriterContext): boolean;
    writePythonNativeEnums(ctx: RangerAppWriterContext, wr: CodeWriter): void;
    emptyBlockFiller(): string;
    adjustType(tn: string): string;
    EncodeString(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): string;
    WriteScalarValue(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    ...
    pyWalkBody(body: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    writeVarInitDef(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    writeVarDef(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    CreateMethodCall(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    CreatePropertyGet(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    ...
    writeTypeDef(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    pyTypeNameFor(tn: string, ctx: RangerAppWriterContext): string;
    pyCollectionTypeFor(tn: string, ctx: RangerAppWriterContext): string;
    pyAnnotationOf(node: CodeNode, ctx: RangerAppWriterContext): string;
    writeClassVarDef(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    writeArgsDef(fnDesc: RangerAppFunctionDesc, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    pyWriteReturnAnnotation(variant: RangerAppFunctionDesc, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    writeArrayLiteral(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    writeFnCall(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    writeNewCall(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    CreateCallExpression(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    writeClass(node: CodeNode, ctx: RangerAppWriterContext, orig_wr: CodeWriter): void;
    pyWriteOperand(arg: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    pyWriteForLoop(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    CustomOperator(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
}
...
export declare class RangerJavaScriptClassWriter extends RangerGenericClassWriter {
    httpServerWriter: RangerJavaScriptHttpServerWriter;
    jsLoops: ForLoopShape;
    compiler?: LiveCompiler;
    thisName: string;
    wrote_header: boolean;
    target_flow: boolean;
    ...
    target_esm: boolean;
    tsTraits: TraitInterfaceAnalysis;
    ts_traits_written: boolean;
    tsEnums: EnumNativeAnalysis;
    ts_enums_written: boolean;
    constructor();
    jsWriteOperand(arg: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    jsWriteForLoop(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    CustomOperator(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    lineEnding(): string;
    writeTsOptionalReturnSuffix(variant: RangerAppFunctionDesc, wr: CodeWriter): void;
    adjustType(tn: string): string;
    CreateTsUnions(parser: RangerFlowParser, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    ...
    writeVarInitDef(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    writeVarDef(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    tsEnumIsNative(enumName: string, ctx: RangerAppWriterContext): boolean;
    writeTsNativeEnums(ctx: RangerAppWriterContext, wr: CodeWriter): void;
    writeTsTraitInterfaces(ctx: RangerAppWriterContext, wr: CodeWriter): void;
    writeClassVarDef(p: RangerAppParamDesc, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    writeNewCall(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    CreateLambdaCall(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    CreateLambda(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    writeArgsDef(fnDesc: RangerAppFunctionDesc, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    ...
    CreatePages(parser: RangerFlowParser, ctx: RangerAppWriterContext, orig_wr: CodeWriter): void;
    CreatePage(parser: RangerFlowParser, node: CodeNode, ctx: RangerAppWriterContext, orig_wr: CodeWriter): void;
    writeNpmPackage(node: CodeNode, ctx: RangerAppWriterContext, orig_wr: CodeWriter): void;
}
...
export declare class LiveCompiler {
    ...
    constructor();
    rustWritesTraitClass(cl: RangerAppClassDesc, ctx: RangerAppWriterContext): boolean;
    ...
    installFile(filename: string, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    findOpCode(op: CodeNode, node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    findOpTemplate(op: CodeNode, node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): CodeNode | undefined;
    localCall(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): boolean;
    finishWalkNode(wr: CodeWriter): void;
    WalkNode(node: CodeNode, in_ctx: RangerAppWriterContext, wr: CodeWriter): void;
    typeNameToIdentifier(ss: string): string;
    tmplDelimitedSlot(cmdList: CodeNode, idx: number): boolean;
    walkCommandList(cmd: CodeNode, node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    isMutatingOpName(n: string): boolean;
    walkCommand(cmd: CodeNode, node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
...
export declare class PkgFetch {
    ...
    install(env: InputEnv, manDir: string): boolean;
    oneDep(env: InputEnv, pending: Array<PkgFetchWork>, w: PkgFetchWork, d: PkgDep): boolean;
...
export declare class RangerDocGenerator {
    constructor();
    writeTypeDef(item: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    writeArgDefs(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    createClassDoc(node: CodeNode, ctx: RangerAppWriterContext, orig_wr: CodeWriter): void;
    writeOpDesc(item: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    writeTypeDoc(list: Array<RangerAppOperatorDesc>, tester: (item: RangerAppOperatorDesc) => boolean, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    createOperatorDoc(node: CodeNode, ctx: RangerAppWriterContext, orig_wr: CodeWriter): void;
}
export declare class StaticAnalyzer {
    ...
    currentClass?: RangerAppClassDesc;
    debug: boolean;
    strictStringsTotal: number;
    strictStringsRisky: number;
    strictStringsSafe: number;
    strictStringsGuards: number;
    strictStringsNotes: Array<string>;
    strictStringsFiles: {
        [key: string]: number;
    };
    strictStringsFileOrder: Array<string>;
    mutatingOps: {
        [key: string]: boolean;
    };
    ...
    unionMemberIsMutatedName(v: string): boolean;
    nodeRootHasType(node: CodeNode, typeName: string): boolean;
    treeMutatesType(node: CodeNode, typeName: string): boolean;
    unionMemberIsMutated(typeName: string): boolean;
    scanFnsForMutation(fns: Array<RangerAppFunctionDesc>, typeName: string): boolean;
    ...
    nodeNamesLocal(node: CodeNode, name: string): boolean;
    nameUsedAfter(node: CodeNode, name: string, afterSp: number): boolean;
    nameIsAliased(node: CodeNode, name: string): boolean;
    countDefsOfName(node: CodeNode, name: string): number;
    findDefOfName(node: CodeNode, name: string): CodeNode | undefined;
    nearestBlockSp(node: CodeNode): number;
    storeIsLocalMove(body: CodeNode, storeNode: CodeNode, stored: CodeNode, name: string, fnCtx: RangerAppWriterContext): boolean;
    walkForSharing(body: CodeNode, node: CodeNode, fnCtx: RangerAppWriterContext, fnName: string, mutated: Array<string>): void;
    ...
    foldObjectInitsAll(): void;
    foldObjectInitsFn(fn: RangerAppFunctionDesc): void;
    foldObjectInitsIn(body: CodeNode, node: CodeNode, fnCtx: RangerAppWriterContext): void;
    countNameUses(node: CodeNode, name: string): number;
    plainNewClassName(value: CodeNode): string;
    classTakesFoldedInit(cl: RangerAppClassDesc): boolean;
    nodeHasEffect(node: CodeNode): boolean;
    classHasField(cl: RangerAppClassDesc, name: string): boolean;
    tryFoldObjectInit(body: CodeNode, block: CodeNode, idx: number, fnCtx: RangerAppWriterContext): void;
    ...
    fnUsesThisValue(fn: RangerAppFunctionDesc): boolean;
    fnSelfCallsNeedy(fn: RangerAppFunctionDesc): boolean;
    stringIsAsciiOnly(s: string): boolean;
    strictStringsOpKind(name: string): string;
    ssSubjectName(n: CodeNode): string;
    ssUnwrap(n: CodeNode): CodeNode;
    ssIsIntLiteral(n: CodeNode): boolean;
    ssIsZeroLiteral(n: CodeNode): boolean;
    ssIsCodePointLabel(lbl: string): boolean;
    ssMergeTaint(a: string, b: string): string;
    ssIsCountOp(name: string): boolean;
    ssIsArith(name: string): boolean;
    ssIsCompare(name: string): boolean;
    ssTaintOf(nRaw: CodeNode, taint: {
        [key: string]: string;
    }): string;
    ssTieBound(vRaw: CodeNode, bound: CodeNode, taint: {
        [key: string]: string;
    }): void;
    ssTaintWalk(node: CodeNode, taint: {
        [key: string]: string;
    }, indexed: {
        [key: string]: boolean;
    }, zeroTested: {
    ...
    ssCollectVars(n: CodeNode, out: {
        [key: string]: boolean;
    }): void;
    ssRefsAny(n: CodeNode, names: {
        [key: string]: boolean;
    }): boolean;
    ssCollectLabels(n: CodeNode, taint: {
        [key: string]: string;
    }, out: {
        [key: string]: boolean;
    }): void;
    ssIndexVarWalk(node: CodeNode, idxVars: {
        [key: string]: boolean;
    }): void;
    ssBoundWalk(node: CodeNode, taint: {
        [key: string]: string;
    }, idxVars: {
        [key: string]: boolean;
    }, indexed: {
    ...
    ssNoteGuard(v: CodeNode, lit: CodeNode, taint: {
        [key: string]: string;
    }, zeroTested: {
        [key: string]: boolean;
    }, litGuarded: {
    ...
    ssReportWalk(node: CodeNode, fnName: string, taint: {
        [key: string]: string;
    }, indexed: {
        [key: string]: boolean;
    }, zeroTested: {
        ...
        [key: string]: boolean;
    }, sizeTest: boolean, fnUnits: boolean): void;
    strictStringsScanFns(fns: Array<RangerAppFunctionDesc>, clName: string): void;
    analyzeStringIndexing(): void;
    computeTraitReentrancy(): void;
    treeCallsThroughField(node: CodeNode, fieldName: string): boolean;
    classCallsThroughField(cl: RangerAppClassDesc, fieldName: string): boolean;
    classHasBackReference(cl: RangerAppClassDesc): boolean;
    computeSelfRcNeeds(): void;
...
export declare class viewbuilder_Android {
    ...
    elWithText(name: string, node: CodeNode, wr: CodeWriter): void;
    WalkNode(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    writeClass(node: CodeNode, ctx: RangerAppWriterContext, orig_wr: CodeWriter): void;
}
export declare class viewbuilder_Web {
    constructor();
    _attr(wr: CodeWriter, name: string, value: string): void;
    tagAttrs(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    tagText(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    tag(name: string, node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    WalkNode(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
    CreateViews(ctx: RangerAppWriterContext, wr: CodeWriter): void;
    writeClass(node: CodeNode, ctx: RangerAppWriterContext, wr: CodeWriter): void;
}
...
export declare class VirtualCompiler {
    ...
    run(env: InputEnv): CompilerResults;
