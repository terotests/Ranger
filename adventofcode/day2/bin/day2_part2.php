...
$total = 0;
$row = array();
operatorsOf::forEach_2($list, (function ($item, $index) use ( &$value,  &$n_idx,  &$row,  &$total,  &$max,  &$min) {
...
class operatorsOf { 
  ...
  public static function forEach_2( $__self , $cb ) {
    for ( $i = 0; $i < count($__self); $i++) {
      $it = $__self[$i];
      call_user_func($cb, $it, $i);
    }
  }
  public static function forEach_3( $__self , $cb ) {
    for ( $i_1 = 0; $i_1 < count($__self); $i_1++) {
      $it_1 = $__self[$i_1];
      call_user_func($cb, $it_1, $i_1);
    }
  }
}
