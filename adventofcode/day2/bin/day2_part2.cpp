...
typedef mpark::variant<std::shared_ptr<day_two_part_two>, int, std::string, bool, double>  r_union_Any;
...
// header definitions
class day_two_part_two : public std::enable_shared_from_this<day_two_part_two>  { 
  public :
    /* class constructor */ 
    day_two_part_two( );
    /* static methods */ 
...
class operatorsOf : public std::enable_shared_from_this<operatorsOf>  { 
  public :
    ...
    static void forEach_3( std::vector<int> __self , std::function<void(int, int)> cb );
};

int __g_argc;
char **__g_argv;
day_two_part_two::day_two_part_two( ) {
}
...
void  operatorsOf::forEach_3( std::vector<int> __self , std::function<void(int, int)> cb ) {
