...
2.80 GHz. Median of three, after a warm-up run. Both renderings emit the same
bytes — the comparison is only meaningful because the outputs are identical.

| Rendering | Self-compile |
| --- | ---: |
| C++ (`g++ -O2`) | **3.65 s** |
| JavaScript (node) | 9.9 s |

Building the compiler *binary* is the slow part, not running it: generating the
C++ takes ~13 s and `g++ -O2` another ~175 s.
...
1. **A lambda copied a by-reference parameter** (C++ only). The writer captures
   ...
   finished compiling the compiler after **775 s**.
2. **Seven dead source-line lookups.** `WalkNode` — the flow parser's main tree
   ...
   once per operator match. Deleting the seven bindings was worth most of what
   is left above: it took the C++ binary from 29 s to under 4, and node from
   14 s to 10.
