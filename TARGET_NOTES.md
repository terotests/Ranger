...
### The library inside the binary

A packaged compiler carries `Lang.rgr`, `stdops.rgr` and every `lib/*.rgr`
**inside the executable**, so it runs with no `RANGER_LIB` and no `.rgr` file
anywhere. This is what makes a distribution package trivial: one file in
`/usr/bin`, no wrapper script, no library path to get right, nothing to set in
a shell profile.

The hook is one place. Every file the compiler reads — `Lang.rgr`,
`stdlib.rgr`, and every `Import` (`mergeImports` in `ng_RangerFlowParser.rgr`
goes through `TFiles.searchEnv` and then `read_file env`) — passes through the
two `InputEnv` operators in `compiler/InputFileSystem.rgr`. They now consult
`RangerEmbeddedLib` when the real file is missing:

```ranger
fn file_exists:boolean (env:InputEnv path:string name:string) {
  if(env.use_real) {
    if( file_exists path name ) { return true }
    return (RangerEmbeddedLib.hasFile(name))
  }
  ...
```

**Disk is looked at first and always wins.** A project that keeps its own
`Lang.rgr` or a patched `JSON.rgr` in the working directory (or names one
through `RANGER_LIB`) keeps using it; the built-in copy is the fallback, not an
override. The regression test writes an unparseable `Lang.rgr` next to the
source and requires the compile to fail — if the embedded copy won, the
override would be a lie.

`compiler/EmbeddedLib.rgr` is generated by `scripts/generate-embedded-lib.mjs`
and holds each file as `push parts "…"` chunks of ~4 kB, split **on line
boundaries** so a chunk can never end inside a multi-byte character or an
escape. It is committed as a **stub** (no files, `hasFile` always false): an
ordinary build stays the size it was, and only `--embed-lib` fills it in, with
a trap that restores the stub however the build exits. The cost when filled:
`bin/output.js` 2.8 MB → 3.7 MB, the C++ translation unit 2.7 MB → 3.5 MB, the
stripped binary 5.8 MB → 6.6 MB. Link time and compile speed are unchanged.

Two properties are worth stating, because both were measured rather than
assumed. The native binary reading **only its embedded library** compiles the
whole compiler to JavaScript that is byte-identical to the Node build's — 807 kB
of library survives the trip through Ranger string literals, the C++ writer's
escaping and `std::string` intact. And the packaged smoke test compiles the same
`hello.rgr` twice, once with the library on disk and once with nothing on disk,
and requires the same bytes out.

No target-specific work was needed: string constants are string constants
everywhere, so the same source gives a self-contained compiler on any backend
that can compile the compiler.
