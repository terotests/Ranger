...
| ~~**P3 — rivikirjasto**~~ **tehty** | 13 rivityyppiä shapessa ja jokainen matriisin familia päätyy johonkin niistä: 14 life-familiaa tekstiriveiksi kuten referenssissä, `Circuit` otsikkona ja riveinä sen alla, `Text` piirtää life-labelin ja numerot omina osinaan, tags/emojis/derived suodatettu pois. Korpus **55 casea, kaikki täsmäävät**. Näkymä `document` piirtää koko dokumentin ja **vierittyy** (EVG:n `scrollTop` leikatulla laatikolla, layout clampaa). | `rt:l0` 55/55, `rt:check` ja `rt:frame` vihreät | MONSTER.compact renderöityy kokonaan |
