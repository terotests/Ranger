...
| **P3 — rivikirjasto** *(kesken)* | Tehty: 11 familiaa shapessa; `Text` piirtää life-labelin ja numerot omina osinaan; tags/emojis/derived suodatettu pois riveistä kuten referenssissä; korpus 32 casea, **31 täsmää**; **viides näkymä `document`** piirtää koko dokumentin familia kerrallaan, rail reititetty globaalisti. Jäljellä: 14 life-familian fallback-teksti, `Circuit`, `Interval`-renderöijä, vieritys pitkälle dokumentille | MONSTER.compact renderöityy kokonaan |
