...
| **P3 — rivikirjasto** *(kesken)* | Tehty: `Summary`, `Phase`, `Custom`, `Duration`, `Unknown` shapeen; `Text` piirtää life-labelin ja numerot omina osinaan; tags/emojis/derived suodatettu pois riveistä kuten referenssissä; korpus 32 casea, **31 täsmää**. Jäljellä: 14 life-familian fallback-teksti, `Circuit`, `Interval`-renderöijä, session-näkymä listaksi `ScrollAreaCtl`illa | MONSTER.compact renderöityy kokonaan |
