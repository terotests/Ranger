...
- The **Go self-host binary does not finish a large source**: `tmp/selfhost-go/rangerc` compiles `tests/fixtures/hello.rgr` in ~0.4 s but had not finished `compiler/ng_Compiler.rgr` after 30 minutes on an otherwise idle 4-core machine, sitting in `[2/5] Analyzing code...` at ~2 GB RSS (30 minutes of wall clock, 50 minutes of CPU, then killed by `timeout`). The C++ build does the same job in 3.1 s and Node in 7.3 s, so this is not a constant factor — it looks like a complexity blow-up in the generated Go (found while benchmarking the native builds for the release binaries). Codegen and `go build` are unaffected, which is why the CI gate does not see it: `selfhost:check:go` only builds the binary, it never runs it on anything big.
- **`-d=<absolute path>` is resolved against the working directory** on both the Node and the C++ builds: `-d=/tmp/x/out` writes to `<cwd>/tmp/x/out` and reports the doubled path as the output location, with a success exit code. Scripts have to pass a relative output directory (`scripts/build-native-compiler.sh` does).
