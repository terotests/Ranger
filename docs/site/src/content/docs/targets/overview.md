...
| Rust | `-l=rust` | `.rs` | A class becomes a plain `struct`, and the writer adds `.clone()` where the value would move. `Rc` and `RefCell` appear for a `weak` field only. Test the output: some operators have no Rust template. |
| Python | `-l=python` | `.py` | |
