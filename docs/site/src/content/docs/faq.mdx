...
- The [operator reference](/Ranger/docs/reference/operators/statements/) holds
  the operators of `Lang.rgr`. Every program can use them.
- A program can import more operator files. The
  [collections file](/Ranger/docs/reference/libraries/stdlib/) adds `map`,
  `filter`, `reduce`, `any`, `all` and `slice`.
- [JSON](/Ranger/docs/reference/libraries/json/) and the other imported files
  work the same way.
