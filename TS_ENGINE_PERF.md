...
> **Update (2026-08-09): Kotlin joins the run gate.** The engine compiles to
> **Go, Kotlin, Python, C#, Swift 6 and Dart** as well as C++ and Rust. Each of
> those six builds with its own toolchain (when present) and answers the Node
> benchmark cases exactly as Node does.
>
> `tests/ts-engine-targets.test.ts` guards the set: it compiles the engine to
> each target, and builds and runs the Go, Python, C#, Dart, Swift and Kotlin
> results wherever those toolchains exist. It has its own config for the reason
