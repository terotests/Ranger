    ...
    (function (_myTrait_) {
      ...
      _myTrait_.initDOMNamespace = function (t) {
          ...
          if (node.value === null) {
            ...
            if(node.value.type=="Literal") {
              if (typeof node.value.value == "string") {
                if(!isNaN(node.value.value) && node.value.value) {
                  this.out(node.value.value);
                } else {
        ...
        _myTrait_.DOMJSXOpeningElement = function (node, ctx) {
          ...
          if (!this._compNs && !this._noPredefinedComponents && !objName && _elemNamesList.indexOf(elemName) >= 0) {
            ...
            if (node.attributes && node.attributes.length) {
              for (var i = 0; i < node.attributes.length; i++) {
                ...
                if (attrName && attrName.substring(0, 3) == "fn_") {
                  var fName = attrName.slice(3).toLowerCase();
                  var valueNode = node.attributes[i].value;
            ...
            // the structural code is run imperatively by the user of the JSX results
            if(this._structural) {
              ctx.__openParent._isStructural = true;
              this.out("{ init : function() { ", true);
              this.indent(1);
              this.out(" return ");
            }
        ...
        _myTrait_.childNodesToArray = function(node, ctx) {
          // console.log("childNodesToArray ", node);
          this.out("[");
          if (node.children) {
        ...
        _myTrait_.DOMJSXElement = function (node, ctx) {
          ...
          var old_cust = ctx.__custom;
          var old_open_parent = ctx.__openParent;

          var old_ns =  this._compNs;
          var old_structural = this._structural;
          // Then, look for the element NS

          // namespace:X

          if(node.openingElement &&  node.openingElement.attributes && node.openingElement.attributes.length) {
            var aList = node.openingElement.attributes;
            for(var i=0; i<aList.length; i++) {
              var attr = aList[i];
              if (attr.name.type == "JSXNamespacedName") {
                if( attr.name.name.name == "namespace") {
                    this._compNs = attr.value.value;
                }
                if( attr.name.name.name == "structural") {
                    this._structural = attr.value.value;
                }
              } 
            }
          }
          ...
          if(ctx.__custom) {
             // console.log("--> opening of custom element");
          } else {
            ...
            if (node.children) {
              ...
              for (var i = 0; i < node.children.length; i++) {
                ...
                if (child.type == "JSXExpressionContainer") {
                  ...
                  this.out("var __ins = function(expr) {");
                  this.indent(1);
                    ...
                    this.indent(1);
                    this.out("expr.forEach(function(a){__ins(a)});", true);
                    this.indent(-1);
                    this.out("} else { ", true);
          ...
          ctx.__custom = old_cust;
          ctx.__openParent = old_open_parent;

          this._compNs = old_ns;
          this._structural = old_structural;
        ...
        _myTrait_.DOMJSXClosingElement = function (node, ctx) {
          if(ctx.__openParent._hasClosure) {
            ...
            if(ctx.__openParent._isStructural) {
              this.out("}} ", true);
              this.indent(-1);
            }              
  ...
  setTimeout(function () {
    if (typeof espree != "undefined") {
      ...
      for (var i = 0; i < list.length; i++) {
        ...
        walker._structural = scriptElem.getAttribute("structural");
