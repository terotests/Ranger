...
CompilerInterface.displayCompilerErrors = function(appCtx) {
  ...
  for ( let i_3 = 0; i_3 < appCtx.compilerErrors.length; i_3++) {
    var e = appCtx.compilerErrors[i_3];
    const line_index = e.node.getLine();
...
CompilerInterface.displayParserErrors = function(appCtx) {
  ...
  for ( let i_4 = 0; i_4 < appCtx.parserErrors.length; i_4++) {
    var e_1 = appCtx.parserErrors[i_4];
    const line_index_1 = e_1.node.getLine();
    console.log((e_1.node.getFilename() + " Line: ") + (1 + line_index_1))
    console.log(e_1.description)
    console.log(e_1.node.getLineString(line_index_1))
  }
}
/* static JavaSript main routine */
function __js_main() {
  const allowed_languages = ["es6","go","scala","java7","swift3","cpp","php","ranger"];
  ...
  const the_file = process.argv[ 2 + process.execArgv.length + 0];
  const the_lang_file = process.argv[ 2 + process.execArgv.length + 1];
  const the_lang = process.argv[ 2 + process.execArgv.length + 2];
  const the_target_dir = process.argv[ 2 + process.execArgv.length + 3];
  const the_target = process.argv[ 2 + process.execArgv.length + 4];
  console.log("language == " + the_lang)
  ...
  const c = (require('fs').readFileSync( process.cwd() + '/' + "." + '/' + the_file , 'utf8'));
  const code = new SourceCode(c);
  ...
  try {
    ...
    const lang_str = (require('fs').readFileSync( process.cwd() + '/' + "." + '/' + the_lang_file , 'utf8'));
    const lang_code = new SourceCode(lang_str);
    ...
    const wr_1 = file.getWriter();
    let staticMethods;
    const importFork = wr_1.fork();
    ...
    const import_list = wr_1.getImports();
