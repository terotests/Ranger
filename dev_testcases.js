	...
	*/
	...
	process {
		...
		process ( spawn : (if (> x 10 )) ) {
			...
			play_audio(from:"http://...")

			// could you be able to create a process in the lisp code
			(if (> x 10)
				;; do something...
				<div/>
			)
		...
		<View as=(if (> x 10) "red" "green")>
			<Label color="white">
				The name is (x) or something </Label>
			(
				(def test_div <div class=(if ( > x 10) "big" "small" ) />)
				([] 
					<div width=(+ x 20) heigth=(x) /> 
					<div/>
				)
			)
		</View>
	...
	ctx["_xx_"] = 1000;
	var view_node = process_ast.children[4];
	var xml_n = xml_node_eval( view_node, ctx );

	console.log(JSON.stringify(xml_n, null, 2));

	console.log(ctx.test_div);


	var process_ast2 = JMLParser(`
		<View as=(if (> x 10) "red" "green")>
			<Label color="white">
				The name is (x) or something </Label>
			(
	...
	console.log(xml_node_eval(process_ast2, ctx));


}

function only_xml_test() {
	var ctx = new Jinx.CreateContext();
	...
	var process_ast2 = JMLParser(`
		(
			...
			(defn obj_to_html (obj)
				(

					(if (== (. obj "charNode") true)
						(. obj "cdata")
						(
							(def attrs (. obj "attributes"))
							(join "<" (. obj "name") 
									(join (map 
											(=> (name)
												(join " " name '= "' (. attrs name) '" ')
											)
											(keys attrs) )
										)
								">" 
									(join (map obj_to_html
											(. obj "children")
									))
								"</" (. obj "name") ">"
							)
			...
			(def x 100)
			(def base_style "padding:20px;color:white;")
			(def style1 "background-color:#58a;font-family:arial;")
			(def style2 "background-color:#48a;font-family:arial;")

			(def sample_data ([] 1 2 3 4 5))

			(def result <div style=( join base_style (if (> x 10) style1 style2))>
							<div>
								The value of x == (x) </div>

								<h3>Create a list of sample_data's (length sample_data) items </h3>
								<ul>
									(map ( => (v) <li>Test item (v)</li> )
										sample_data
									)
								</ul>
						</div>)
			(console result)
			(console (obj_to_html result))
			(add_to_dom "main" (obj_to_html result))
		)
	`);

	var r = run_lisp(process_ast2, ctx);
	console.log(r);
