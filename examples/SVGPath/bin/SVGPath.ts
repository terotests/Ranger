type union_Any = Vec2|Mat2|PathExecutor|PathCollector|PathSegment|EVGBezierPath|EVGPathParser|number|string|boolean|number;
export class Vec2  {
  x: number;
  y: number;
  constructor() {
...
export class Mat2  {
  ...
  setRotation (v : number) : void  {
    const cs : number  = Math.cos(v);
    const sn : number  = Math.sin(v);
    this.m0 = cs;
    this.m1 = sn;
    this.m2 = -1.0 * sn;
    this.m3 = cs;
  ...
  multiply (b : Mat2) : void  {
    const t0 : number  = (this.m0 * b.m0) + (this.m1 * b.m2);
    const t2 : number  = (this.m2 * b.m0) + (this.m3 * b.m2);
    const t4 : number  = ((this.m4 * b.m0) + (this.m5 * b.m2)) + b.m4;
  ...
  inverse () : Mat2  {
    let invdet : number  = (this.m0 * this.m3) - (this.m2 * this.m1);
    const det : number  = invdet;
    if ( (det > -0.0001) && (det < 0.0001) ) {
      this.toIdentity();
      return this;
    }
    invdet = 1.0 / det;
    const inv : Mat2  = new Mat2();
...
export class PathCollector  extends PathExecutor {
...
export class EVGPathParser  {
  ...
  __xformPoint (point : Vec2, seg : PathSegment) : Vec2  {
    const res : Vec2  = Vec2.CreateNew((((point.x * seg.t0) + (point.y * seg.t2)) + seg.t4), (((point.x * seg.t1) + (point.y * seg.t3)) + seg.t5));
    return res;
  };
  ...
  __vecang (u : Vec2, v : Vec2) : number  {
    let r : number  = this.__vecrat(u, v);
    if ( r < -1.0 ) {
      r = -1.0;
    }
    if ( r > 1.0 ) {
  ...
  scanNumber () : boolean  {
    ...
    let fc : number  = s.charCodeAt(this.i );
    let c : number  = fc;
    let sp : number  = 0;
    let ep : number  = 0;
  ...
  pathArcTo (callback : PathExecutor, cp : Vec2, args : PathSegment, rel : boolean) : Vec2  {
    let rx : number  = 0.0;
    let ry : number  = 0.0;
    let rotx : number  = 0.0;
    let x1 : number  = 0.0;
    ...
    let sa : number  = 0.0;
    let sb : number  = 0.0;
    /** unused:  const ux : number  = 0.0   **/ 
    /** unused:  const uy : number  = 0.0   **/ 
    /** unused:  const vx : number  = 0.0   **/ 
    /** unused:  const vy : number  = 0.0   **/ 
    let a1 : number  = 0.0;
    let da : number  = 0.0;
    let x : number  = 0.0;
    let y : number  = 0.0;
    ...
    let px : number  = 0.0;
    let py : number  = 0.0;
    let ptanx : number  = 0.0;
    let ptany : number  = 0.0;
    const t : PathSegment  = new PathSegment();
    let sinrx : number  = 0.0;
    let cosrx : number  = 0.0;
    let fa : number  = 0.0;
    let fs : number  = 0.0;
    let i_1 : number  = 0;
    let ndivs : number  = 0;
    ...
    const cpx : number  = cp.x;
    const cpy : number  = cp.y;
    rx = Math.abs(args.t0);
    ry = Math.abs(args.t1);
    ...
    const u : Vec2  = Vec2.CreateNew(((x1p - cxp) / rx), ((y1p - cyp) / ry));
    const v : Vec2  = Vec2.CreateNew((((-1.0 * x1p) - cxp) / rx), (((-1.0 * y1p) - cyp) / ry));
    const unitV : Vec2  = Vec2.CreateNew(1.0, 0.0);
    ...
    while (i_1 <= ndivs) {
      ...
      dx = Math.cos(a);
      dy = Math.sin(a);
      const trans : Vec2  = this.__xformPoint(Vec2.CreateNew((dx * rx), (dy * ry)), t);
      x = trans.x;
      y = trans.y;
      const v_trans : Vec2  = this.__xformVec(Vec2.CreateNew((((-1.0 * dy) * rx) * kappa), ((dx * ry) * kappa)), t);
      tanx = v_trans.x;
      tany = v_trans.y;
      if ( i_1 > 0 ) {
    ...
    const rv : Vec2  = Vec2.CreateNew(x2, y2);
    return rv;
  };
  parsePath (path : string, callback : PathExecutor) : void  {
    this.i = 0;
    this.buff = path;
    const s : string  = this.buff;
    this.__len = s.length;
    /** unused:  const buff_1 : string  = path   **/ 
    let cmd : number  = 76;
    /** unused:  const path_1 : EVGBezierPath  = new EVGBezierPath()   **/ 
    const args : PathSegment  = new PathSegment();
    let require_args : number  = 2;
    let arg_cnt : number  = 0;
    const QPx : PathSegment  = new PathSegment();
    const QPy : PathSegment  = new PathSegment();
    const CPx : PathSegment  = new PathSegment();
    const CPy : PathSegment  = new PathSegment();
    let cx : number  = 0.0;
    let cy : number  = 0.0;
    let cx2 : number  = 0.0;
    let cy2 : number  = 0.0;
        ...
        if ( arg_cnt >= require_args ) {
          switch (cmd ) { 
            ...
            case 116 : 
              ...
              break;
            case 97 : 
              const res : Vec2  = this.pathArcTo(callback, Vec2.CreateNew(cx, cy), args, true);
              cx = res.x;
              cy = res.y;
              cx2 = cx;
              cy2 = cy;
              break;
            case 65 : 
              const res_1 : Vec2  = this.pathArcTo(callback, Vec2.CreateNew(cx, cy), args, false);
              cx = res_1.x;
              cy = res_1.y;
              cx2 = cx;
              cy2 = cy;
