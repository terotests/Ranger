...
T‰m‰ dokumentti kuvaa, miten `Lang.rgr`:n array/map-operaattorit voidaan tuoda Low IR / WASM -backendiin. Nykyinen `IntList` / `IntMap` -demo k‰ytt‰‰ **luokkia + `Mem`-intrinskej‰**; Lang-syntaksi (`def a:[int]`, `set`, `get`, Ö) on erillinen polku.

## Miksi Lang-templatet eiv‰t riit‰

`-l=llvm` k‰ytt‰‰ `RangerLLVMPipeline` ? `LowIRBuilderPass` ? LLVM/WAT. Se **ei** laajenna `Lang.rgr`:n `templates { es6 Ö }` -blokkeja kuten ES6-backend. `RangerLLVMClassWriter.writeClass` on tyhj‰ ó koko moduuli rakennetaan Low IR:st‰.
...
K‰yt‰nnˆss‰: **lowering `ng_LowIRBuilder.rgr`:‰‰n + pieni runtime (`RtArray` / `RtMap` tai `Mem`-laajennus)**.
...
| Luokat `IntList` / `IntMap` | ó | kyll‰ (`llvm_collections.rgr`) |
| `Mem.alloc/load/store` | ó | kyll‰ (intrinsic) |
...
```text
offset 0: data   (i32)  ó osoite elementtidataan
offset 4: len    (i32)
offset 8: cap    (i32)
```
...
Myˆhemmin: `RtArray_push` ? Lang `push`.

## Toteutusvaiheet

### Vaihe 1 ó `[int]` ja `[int:int]` (ei stringej‰)
...
- `compiler/ng_LowIRBuilder.rgr` ó lowering
- `compiler/ng_LowIR.rgr` ó `emit*` + intrinsiset
- `compiler/ng_WATWriter.rgr` / `ng_LLVMIRWriter.rgr`
- `tests/fixtures/llvm_collections.rgr` ó luokkapohjainen referenssi
- `compiler/Lang.rgr` ó operaattorim‰‰rittelyt (rivit ~1111, 1881, 2939, 3442ñ4051)
