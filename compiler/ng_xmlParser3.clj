...
class XMLParser:void {
  ...
  def last_parent_safe:CodeNode
  def curr_node@(weak):CodeNode
  def tag_depth:int 0
  ...
  fn parse_attributes:boolean () {
    ...
    while (i < len) {
      ...
      if (c == 34) {
        ...
        ep = i
        if ((i < len) && (ep > sp)) {
          def new_attr@(lives):CodeNode (new CodeNode)
  ...
  fn parse:void () {
    ...
    while (i < len) {
      ...
      if (((ccode "<") == cc1) && (cc2 == (ccode "/"))) {
        ...
        def last_parent:CodeNode (itemAt parents (p_cnt - 1))
      ...
      if (cc1 == (ccode "<")) {
        ...
        if (null? curr_node) {
          def new_rnode:CodeNode (new CodeNode )
          new_rnode.vref = new_tag
          new_rnode.value_type = RangerNodeType.CodeNode
          rootNode = new_rnode
          push parents new_rnode
          curr_node = new_rnode
        } {
          def new_node:CodeNode (new CodeNode )
          new_node.vref = new_tag
          new_node.value_type = RangerNodeType.CodeNode
