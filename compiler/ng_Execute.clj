...
class Execute {
  ...
  sfn hello@(main):void () {
    ...
    print "Arg cnt == " + (shell_arg_cnt)
    if ( (shell_arg_cnt) < 1  ) {
      print "Ranger Execute Env, version 0.0.1"
      print "usage <file>"
      print "PS. run.clj was the first example file used here..."
      return 
    }    

    def the_file:string (shell_arg 0)
    def the_lang_file:string "ScriptLang.clj"
        ...
        for appCtx.definedClassList cName:string i {
            ...
            for cl.defined_variants fnVar:string i {
                ...
                for mVs.variants variant:RangerAppFunctionDesc i {
                    ...
                    def pr:JinxProcess (new JinxProcess())
                    pr.stdCommands = flowParser.stdCommands
                    
                    timer "run.." {
                        pr.start(body RunCtx)
                    }
