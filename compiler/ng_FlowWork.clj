...
class RangerFlowParser {
  ...
  fn walkNodeArguments:NodeWalkResult ( arg_eval_start:int args:CodeNode callArgs:CodeNode codeDef:CodeNode ctx:RangerAppWriterContext blockCtx:RangerAppWriterContext wr:CodeWriter ) {

    def res (new NodeWalkResult)

    for args.children arg@(lives):CodeNode i {
      if( i < arg_eval_start) {
        continue
      }
      res.arg_eval_start = i

      if( (array_length callArgs.children) <= (i + 1)) {
        res.not_enough_args = true
        break
      }
      ...
      if( arg.value_type == RangerNodeType.ExpressionType ) {
        if(codeDef.is_block_node == false) {
          def later@(lives) (new WalkLater)
          later.arg = arg
          later.callArg = callArg
          push res.walk_later later              
        }  
      } {
        if(arg.type_name == "block" || (arg.hasFlag("block"))) {
          ; NOTE: blockCtx is used to walk the given block, necessary for example macros
          this.WalkNode(callArg blockCtx wr)
          res.last_was_block = true
        } {
  ...
  fn stdParamMatch:boolean (callArgs@(lives):CodeNode inCtx:RangerAppWriterContext wr:CodeWriter require_all_match:boolean) {
    ...
    def origCallArgs (callArgs.copy())
...
;          }
        ...
        def WalkA (this.walkNodeArguments( arg_eval_start args callArgs codeDef ctx ctx wr))

        not_enough_args = WalkA.not_enough_args
        last_was_block = WalkA.last_was_block
        walk_later = WalkA.walk_later        
        last_walked = WalkA.last_walked
...
;            print "Args sig for " + nSig + " == " + (argsSig)
        ...
        if all_matched {
          ...
          if is_macro {
            ...
            ; create new macro so that block is evaluated in the ranger context
;            def macroArgs (origCallArgs.copy())
;            def mCtx (ctx.fork())
;            mCtx.targetLangName = "ranger"
;            def WalkA (this.walkNodeArguments( 0 args macroArgs codeDef ctx mCtx wr))
