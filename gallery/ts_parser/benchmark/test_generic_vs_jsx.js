...
function testCode(code, name, tsxMode = true) {
  ...
  try {
    ...
    console.log(
      `Has JSX: ${hasJSX}, Has Arrow: ${hasArrow}, Has TypeParameter: ${hasTypeParam}`
    );
  } catch (e) {
...
testCode(
  ...
  "Ambiguous <T> (should be JSX in TSX mode)",
  true
);
...
testCode(
  ...
  "Generic with extends {} (should be generic)",
  true
);
...
testCode(
  ...
  "Generic with extends unknown (should be generic)",
  true
);
...
testCode(
  'const genericFn = <T>() => { return "test"; }',
  "Without TSX mode (should be generic)",
  false
);
