  ...
  for (const key of Object.keys(node)) {
    ...
    if (val && typeof val === "object") {
      if (Array.isArray(val)) {
        val.forEach((item, i) =>
          getAllTypes(item, types, `${path}.${key}[${i}]`)
        );
      } else {
...
// Test cases
testCode(
  'const genericFn = <T>() => { return "test"; }',
  "1. Ambiguous <T> - should be JSX in TSX mode",
  true
);
testCode(
  'const genericFn = <T extends {}>() => { return "test"; }',
  "2. Generic with extends {} - should be generic",
  true
);
testCode(
  'const genericFn = <T extends unknown>() => { return "test"; }',
  "3. Generic with extends unknown - should be generic",
  true
);
testCode(
  'const genericFn = <T>() => { return "test"; }',
  "4. Without TSX mode - should be generic",
  false
);
