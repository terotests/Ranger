...
describe('Type Declarations', () => {
  it('Interface Declaration', () => {
    ...
    expect(findNode(ast, n => 
      n.nodeType === 'InterfaceDeclaration' || n.nodeType === 'TSInterfaceDeclaration'
    )).toBe(true);
  });
  ...
  it('Type Alias', () => {
    ...
    expect(findNode(ast, n => 
      n.nodeType === 'TypeAliasDeclaration' || n.nodeType === 'TSTypeAliasDeclaration'
    )).toBe(true);
  });
  ...
  it('Declare Module', () => {
    const ast = parse('declare module "lodash" { export function chunk<T>(arr: T[]): T[][]; }');
...
describe('Basic Types', () => {
  it('Primitive Types', () => {
    ...
    expect(findNode(ast, n => 
      n.nodeType?.includes('TypeReference') || n.nodeType?.includes('Keyword') || n.name === 'string'
    )).toBe(true);
  });
  ...
  it('Array Type (Array<T>)', () => {
    ...
    expect(findNode(ast, n => 
      n.nodeType?.includes('TypeReference') && n.name === 'Array'
    )).toBe(true);
  });
  ...
  it('Intersection Type', () => {
    ...
    expect(findNode(ast, n => n.nodeType?.includes('IntersectionType'))).toBe(true);
  });
  ...
  it('Type Literal (Object Type)', () => {
    ...
    expect(findNode(ast, n => n.nodeType?.includes('TypeLiteral'))).toBe(true);
  });

  it('Function Type', () => {
    const ast = parse('type Fn = (x: number) => string;');
    expect(findNode(ast, n => n.nodeType?.includes('FunctionType'))).toBe(true);
  });
});
...
describe('Generics', () => {
  it('Generic Interface', () => {
    ...
    expect(findNode(ast, n => n.nodeType?.includes('TypeParameter'))).toBe(true);
  });
  ...
  it('Generic Constraint', () => {
    const ast = parse('function fn<T extends object>(arg: T): T { return arg; }');
    expect(findNode(ast, n => n.nodeType?.includes('TypeParameter'))).toBe(true);
  });
...
describe('Classes', () => {
  ...
  it('Public/Private/Protected', () => {
    const ast = parse('class C { public a: number; private b: string; protected c: boolean; }');
    expect(findNode(ast, n => n.nodeType === 'PropertyDefinition')).toBe(true);
  });
  ...
  it('Static Members', () => {
    ...
    expect(findNode(ast, n => 
      n.nodeType === 'PropertyDefinition' || n.nodeType === 'MethodDefinition'
    )).toBe(true);
  });
  ...
  it('Constructor Parameter Properties', () => {
    const ast = parse('class C { constructor(public x: number, private y: string) {} }');
    expect(findNode(ast, n => n.nodeType === 'ClassDeclaration')).toBe(true);
  });
});
...
describe('Functions', () => {
  it('Function Declaration', () => {
    ...
    expect(findNode(ast, n => n.nodeType === 'FunctionDeclaration')).toBe(true);
  });

  it('Arrow Function', () => {
    const ast = parse('const fn = (x: number): number => x * 2;');
    expect(findNode(ast, n => n.nodeType === 'ArrowFunctionExpression')).toBe(true);
  });
  ...
  it('Rest Parameters', () => {
    ...
    expect(findNode(ast, n => n.nodeType === 'RestElement' || n.kind === 'rest')).toBe(true);
  });

  it('Function Overloads', () => {
    const ast = parse('function fn(x: string): string;\nfunction fn(x: number): number;\nfunction fn(x: any): any { return x; }');
    expect(findNode(ast, n => n.nodeType === 'FunctionDeclaration')).toBe(true);
  });

  it('Async Function', () => {
    const ast = parse("async function fetchData(): Promise<string> { return ''; }");
    expect(findNode(ast, n => n.nodeType === 'FunctionDeclaration')).toBe(true);
  });
});
...
describe('Statements', () => {
  it('Variable Declaration', () => {
    ...
    expect(findNode(ast, n => n.nodeType === 'VariableDeclaration')).toBe(true);
  });
...
describe('Expressions', () => {
  it('Type Assertion (as)', () => {
    ...
    expect(findNode(ast, n => n.nodeType?.includes('AsExpression'))).toBe(true);
  });

  it('Type Assertion (<T>)', () => {
    const ast = parse('const x = <string>value;');
    expect(findNode(ast, n => n.nodeType?.includes('TypeAssertion'))).toBe(true);
  });

  it('Non-Null Assertion', () => {
    const ast = parse('const x = value!;');
    expect(findNode(ast, n => n.nodeType?.includes('NonNullExpression'))).toBe(true);
  });

  it('Satisfies Expression', () => {
    const ast = parse('const x = { a: 1 } satisfies Record<string, number>;');
    expect(findNode(ast, n => n.nodeType?.includes('SatisfiesExpression'))).toBe(true);
  });
  ...
  it('Nullish Coalescing', () => {
    ...
    expect(findNode(ast, n => 
      (n.nodeType === 'BinaryExpression' || n.nodeType === 'LogicalExpression') && n.value === '??'
    )).toBe(true);
  });
});
...
describe('Modules', () => {
  ...
  it('Export Named', () => {
    ...
    expect(findNode(ast, n => 
      n.nodeType?.includes('ExportDeclaration') || n.nodeType?.includes('ExportNamed')
    )).toBe(true);
  });

  it('Export Default', () => {
    const ast = parse('export default function() {}');
    expect(findNode(ast, n => n.nodeType?.includes('ExportDefault'))).toBe(true);
  });
...
describe('Advanced Types', () => {
  it('Conditional Type', () => {
    ...
    expect(findNode(ast, n => n.nodeType?.includes('ConditionalType'))).toBe(true);
  });
  ...
  it('Indexed Access Type', () => {
    ...
    expect(findNode(ast, n => n.nodeType?.includes('IndexedAccessType'))).toBe(true);
  });

  it('Keyof Type', () => {
    const ast = parse('type Keys = keyof Person;');
    expect(findNode(ast, n => n.nodeType?.includes('TypeOperator') || n.value === 'keyof')).toBe(true);
  });

  it('Typeof Type', () => {
    const ast = parse('type T = typeof obj;');
    expect(findNode(ast, n => n.nodeType?.includes('TypeQuery') || n.value === 'typeof')).toBe(true);
  });
  ...
  it('Template Literal Type', () => {
    ...
    expect(findNode(ast, n => n.nodeType?.includes('TemplateLiteralType'))).toBe(true);
  });
});
...
describe('JavaScript (ES6+)', () => {
  it('Generator Function', () => {
    ...
    expect(findNode(ast, n => n.nodeType === 'FunctionDeclaration' && n.generator === true)).toBe(true);
  });
  ...
  it('For-Await-Of', () => {
    ...
    expect(findNode(ast, n => n.nodeType === 'ForOfStatement' && n.await === true)).toBe(true);
  });
  ...
  it('Private Field', () => {
    ...
    expect(findNode(ast, n => 
      n.nodeType === 'PrivateIdentifier' || n.nodeType === 'PropertyDefinition'
    )).toBe(true);
  });
  ...
  it('Logical Assignment (&&=)', () => {
    const ast = parse('x &&= y;');
    expect(findNode(ast, n => n.nodeType === 'AssignmentExpression' && n.value === '&&=')).toBe(true);
  });
  ...
  it('Exponentiation Operator', () => {
    ...
    expect(findNode(ast, n => n.nodeType === 'BinaryExpression' && n.value === '**')).toBe(true);
  });

  it('Numeric Separators', () => {
    const ast = parse('const x = 1_000_000;');
    expect(findNode(ast, n => 
      n.nodeType === 'Literal' || n.nodeType === 'NumberLiteral' || n.nodeType === 'NumericLiteral'
    )).toBe(true);
  });

  it('BigInt Literal', () => {
    const ast = parse('const x = 123n;');
    expect(findNode(ast, n => n.nodeType === 'BigIntLiteral' || n.value === '123n')).toBe(true);
  });
  ...
  it('Import Meta', () => {
    ...
    expect(findNode(ast, n => n.nodeType === 'MetaProperty' || n.name === 'import')).toBe(true);
  });

  it('Object Shorthand', () => {
    const ast = parse('const obj = { x, y };');
    expect(findNode(ast, n => n.nodeType === 'Property' && n.shorthand === true)).toBe(true);
  });

  it('Computed Property', () => {
    const ast = parse('const obj = { [key]: value };');
    expect(findNode(ast, n => n.nodeType === 'Property' && n.computed === true)).toBe(true);
  });

  it('Getter', () => {
    const ast = parse('const obj = { get x() { return 1; } };');
    expect(findNode(ast, n => 
      (n.nodeType === 'Property' || n.nodeType === 'MethodDefinition') && n.kind === 'get'
    )).toBe(true);
  });
  ...
  it('Setter', () => {
    ...
    expect(findNode(ast, n => 
      (n.nodeType === 'Property' || n.nodeType === 'MethodDefinition') && n.kind === 'set'
    )).toBe(true);
  });
  ...
  it('Tagged Template', () => {
    ...
    expect(findNode(ast, n => n.nodeType === 'TaggedTemplateExpression')).toBe(true);
  });
});
...
describe('JSX', () => {
  ...
  it('JSX Self-Closing', () => {
    ...
    expect(findNode(ast, n => n.nodeType === 'JSXElement' && n.left?.kind === 'self-closing')).toBe(true);
  });

  it('JSX Expression', () => {
    const ast = parse('<div>{value}</div>', true);
    expect(findNode(ast, n => n.nodeType === 'JSXExpressionContainer')).toBe(true);
  });
  ...
  it('JSX Spread Attribute', () => {
    ...
    expect(findNode(ast, n => n.nodeType === 'JSXSpreadAttribute')).toBe(true);
  });

  it('Ambiguous <T> as JSX in TSX mode', () => {
    const ast = parse('const fn = <T>() => {}', true);
    expect(findNode(ast, n => n.nodeType === 'JSXElement' || n.nodeType === 'JSXOpeningElement')).toBe(true);
  });

  it('Generic <T extends {}> not JSX', () => {
    const ast = parse('const fn = <T extends {}>() => { return "test"; }', true);
    expect(findNode(ast, n => n.nodeType === 'ArrowFunctionExpression')).toBe(true);
    expect(findNode(ast, n => n.nodeType === 'JSXElement')).toBe(false);
  });

  it('Generic <T extends unknown> not JSX', () => {
    const ast = parse('const fn = <T extends unknown>() => { return "test"; }', true);
    expect(findNode(ast, n => n.nodeType === 'ArrowFunctionExpression')).toBe(true);
    expect(findNode(ast, n => n.nodeType === 'JSXElement')).toBe(false);
  });
});
...
describe('Tricky Cases', () => {
  ...
  it('Type Predicate', () => {
    const ast = parse("function isString(x: unknown): x is string { return typeof x === 'string'; }");
    expect(findNode(ast, n => n.nodeType === 'TSTypePredicate')).toBe(true);
  });

  it('Assertion Function', () => {
    const ast = parse('function assert(value: unknown): asserts value { if (!value) throw new Error(); }');
    expect(findNode(ast, n => 
      n.nodeType === 'TSTypePredicate' || n.nodeType === 'TSAssertsThisTypePredicate'
    )).toBe(true);
  });
  ...
  it('Nested Conditional Type', () => {
    const ast = parse("type Check<T> = T extends string ? 'str' : T extends number ? 'num' : 'other';");
  ...
  it('Named Tuple Elements', () => {
    ...
    expect(findNode(ast, n => n.nodeType === 'TSNamedTupleMember')).toBe(true);
  });

  it('Rest in Tuple Type', () => {
    const ast = parse('type Tail<T extends any[]> = T extends [any, ...infer Rest] ? Rest : never;');
    expect(findNode(ast, n => n.nodeType === 'TSRestType')).toBe(true);
  });

  it('Override Modifier', () => {
    const ast = parse('class Child extends Parent { override method() { return 1; } }');
    expect(findNode(ast, n => n.nodeType === 'MethodDefinition' && n.name === 'method')).toBe(true);
  });

  it('Accessor Keyword', () => {
    const ast = parse('class Foo { accessor x: number = 0; }');
    expect(findNode(ast, n => 
      n.nodeType === 'PropertyDefinition' || n.nodeType === 'AccessorProperty'
    )).toBe(true);
  });
});
