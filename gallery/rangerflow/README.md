...
Mermaid answers to thirty-eight header keywords and this reads thirty-five of
them. The second one was already here before the reading started: a `classDiagram` is the UML
...
charts — has a reader of its own further down this page. **Thirty-five of
Mermaid's thirty-eight header keywords are drawn**, and the other three —
`treemap`, `treeView-beta`, `eventmodeling` — are recognised and refused rather
than guessed at. The table is in
...
made of the same text. Currently **228/228 checks over 49 examples**, including
