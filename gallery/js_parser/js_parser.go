...
func (this *JSPrinter) emitLine (text string) () {
  var ind string= this.getIndent();
  this.output = ((this.output + ind) + text) + "\n"; 
}
...
func (this *JSPrinter) printVariableDeclarator (node *JSNode) () {
  if  node.id.has_value {
    var id *JSNode= node.id.value.(*JSNode);
    this.printNode(id);
  }
  if  node.init.has_value {
    this.emit(" = ");
    this.printNode(node.init.value.(*JSNode));
  }
}
...
func (this *JSPrinter) printClassDeclaration (node *JSNode) () {
  ...
  if  node.superClass.has_value {
    var sc *JSNode= node.superClass.value.(*JSNode);
    this.emit(" extends " + sc.name);
  }
  this.emit(" ");
  if  node.body.has_value {
...
func (this *JSPrinter) printMethodDefinition (node *JSNode) () {
  ...
  if  node.key.has_value {
    var keyNode *JSNode= node.key.value.(*JSNode);
    this.emit(keyNode.name + "(");
  } else {
    this.emit("(");
  }
  if  node.body.has_value {
...
func JSParserMain_static_processFile(inputFile string, outputFile string) () {
  ...
  var stmtCount int64= int64(len(program.children));
  var output string= (printer).print(program);
  r_write_text_file(".", outputFile, output)
  fmt.Println( (("Parsed " + inputFile) + " -> ") + outputFile )
  fmt.Println( (strings.Join([]string{ "  ",strconv.FormatInt(stmtCount, 10) }, "")) + " statements processed" )
}
