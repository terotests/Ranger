 ...
 *
 *   node gallery/datagrid/web/standalone/smoke.mjs [--port 8898] [--keep]
 */
...
async function main() {
  if (!fs.existsSync(path.join(DIST, "datagrid_web.js"))) {
    console.error("no build in " + DIST + " — run: npm run datagrid:web");
    process.exit(1);
  }
  ...
  try {
    ...
    const shot = process.argv.includes("--shot")
      ? path.resolve(HERE, "../../artifacts/19_browser_only.png")
      : "";
    ...
    if (!(cmds > 40)) problems.push("the scene has almost nothing in it (" + cmds + " commands)");
    ...
    console.log("\nthe editor ran in a browser with no host behind it");
  } finally {
    server.close();
  }
}
