...
echo "==> JavaScript"
node dist/rgrc.js -es6 "$SRC" -d=gallery/datagrid/bin -o=TableRefTest.js -nodecli > "$OUT/js.log" 2>&1 || {
...
echo
echo "==> C++ ($CXX)"
node dist/rgrc.js -l=cpp "$SRC" -nodecli -d="$OUT" -o=TableRefTest.cpp > "$OUT/cpp.log" 2>&1 || {
