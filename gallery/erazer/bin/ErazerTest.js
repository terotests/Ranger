...
ErazerPaint.blank = function(w, h, r, g, b) {
  const img = new ImageBuffer();
  img.init(w, h);
  ErazerFont.fillRect(img, 0, 0, w, h, r, g, b);
  return img;
};
ErazerPaint.paintLabel = function(img, x, y, text) {
  ErazerFont.paintText(img, x, y, 2, 17, 24, 39, text);
};
ErazerPaint.paintCentered = function(img, x, y, w, h, text, r, g, b) {
  const tw = ErazerFont.textWidth(text, 2);
  const th = ErazerFont.textHeight(2);
  const tx = x + (((w - tw) / 2) | 0);
  const ty = y + (((h - th) / 2) | 0);
  ErazerFont.paintText(img, tx, ty, 2, r, g, b, text);
};
ErazerPaint.form = function() {
  const img = ErazerPaint.blank(320, 240, 232, 236, 240);
...
ErazerPaint.tabs = function() {
  const img = ErazerPaint.blank(320, 80, 229, 231, 235);
...
ErazerPaint.menu = function() {
  const img = ErazerPaint.blank(220, 160, 229, 231, 235);
...
ErazerPaint.iconRow = function() {
  const img = ErazerPaint.blank(200, 64, 255, 255, 255);
...
ErazerPaint.button = function() {
  const img = ErazerPaint.blank(160, 64, 232, 236, 240);
  ErazerFont.fillRoundRect(img, 24, 16, 112, 32, 6, 37, 99, 235);
  ErazerPaint.paintCentered(img, 24, 16, 112, 32, "Login", 255, 255, 255);
  return img;
};
ErazerPaint.chipRow = function() {
  const img = ErazerPaint.blank(560, 80, 232, 236, 240);
...
ErazerPaint.chipStrip = function() {
  const img = ErazerPaint.blank(540, 80, 232, 236, 240);
...
ErazerPaint.paintSliderRow = function(img, x, y, trackW, fillW) {
...
ErazerPaint.slider = function() {
  const img = ErazerPaint.blank(400, 56, 255, 255, 255);
  ErazerPaint.paintSliderRow(img, 16, 16, 280, 160);
  return img;
};
ErazerPaint.sliders = function() {
  const img = ErazerPaint.blank(400, 160, 255, 255, 255);
...
ErazerPaint.paintSwitch = function(img, x, y, on) {
  if ( on ) {
    ErazerFont.fillRoundRect(img, x, y, 50, 30, 15, 52, 199, 89);
    ErazerFont.fillRoundRect(img, x + 22, y + 2, 26, 26, 13, 255, 255, 255);
  } else {
    ErazerFont.fillRoundRect(img, x, y, 50, 30, 15, 233, 233, 235);
    ErazerFont.fillRoundRect(img, x + 2, y + 2, 26, 26, 13, 255, 255, 255);
  }
};
ErazerPaint.paintGroup = function(img, y, labels) {
  const n = labels.length;
  const rowH = 44;
  ErazerFont.fillRoundRect(img, 16, y, 328, n * rowH, 10, 255, 255, 255);
  let i = 0;
  while (i < n) {
    const ry = y + i * rowH;
    ErazerPaint.paintLabel(img, 32, ry + 15, labels[i]);
    ErazerPaint.paintSwitch(img, 282, ry + 7, i - ((i / 2) | 0) * 2 == 0);
    if ( i < n - 1 ) {
      ErazerFont.fillRect(img, 32, (ry + rowH) - 1, 312, 1, 198, 198, 200);
    }
    i = i + 1;
  };
};
ErazerPaint.groupedList = function() {
  const img = ErazerPaint.blank(360, 400, 242, 242, 247);
  ErazerFont.fillRoundRect(img, 120, 10, 120, 6, 3, 17, 17, 17);
  ErazerPaint.paintGroup(img, 40, ["Grid", "Level", "Mirror"]);
  ErazerPaint.paintGroup(img, 200, ["Scan", "Burst", "Text"]);
  return img;
};
ErazerPaint.frostedList = function() {
  const img = ErazerPaint.blank(360, 400, 200, 202, 214);
  let x = 0;
  while (x < 360) {
    const r = 200 - (((x * 60) / 360) | 0);
    const g = 202 - (((x * 58) / 360) | 0);
    const b = 214 - (((x * 14) / 360) | 0);
    ErazerFont.fillRect(img, x, 0, 1, 400, r, g, b);
    x = x + 1;
  };
  ErazerFont.fillRoundRect(img, 120, 10, 120, 6, 3, 17, 17, 17);
  ErazerPaint.paintGroup(img, 40, ["Grid", "Level", "Mirror"]);
...
class ErazerTest  {
  ...
  testAtlas (t) {
    const abc = ErazerFont.alphabet();
    const data = ErazerFont.atlas();
    t.eqInt("atlas covers the alphabet", data.length, abc.length * 35);
    const a = ErazerFont.bitsFor("A");
    t.eqInt("glyph A is 35 bits", a.length, 35);
    t.eqBool("A has a top bar", ErazerFont.cellOn(a, 1, 0), true);
  };
  testOcr (t) {
    const img = ErazerPaint.blank(140, 40, 255, 255, 255);
    ErazerFont.paintText(img, 10, 8, 2, 17, 24, 39, "OK");
    const doc = Erazer.analyze(img);
    const hit = doc.root.findText("OK");
    t.ok("ocr reads OK", (typeof(hit) === "undefined") == false);
    if ( typeof(hit) === "undefined" ) {
    } else {
      const n = hit;
  ...
  testBlank (t) {
    const img = ErazerPaint.blank(64, 48, 232, 236, 240);
    const doc = Erazer.analyze(img);
  ...
  testButton (t) {
    ...
    const hit = doc.root.findRole("button");
    if ( typeof(hit) === "undefined" ) {
  ...
  testForm (t) {
    const img = ErazerPaint.form();
    const doc = Erazer.analyze(img);
    const fields = doc.root.roleCount("textfield");
    const buttons = doc.root.roleCount("button");
    const checks = doc.root.roleCount("checkbox");
    const labels = doc.root.roleCount("label");
    ...
    const name = doc.root.findText("Name");
    t.ok("form reads Name", (typeof(name) === "undefined") == false);
    const email = doc.root.findText("Email");
    t.ok("form reads Email", (typeof(email) === "undefined") == false);
    if ( (fields < 2 || (typeof(email) === "undefined")) || (typeof(name) === "undefined") ) {
    }
  };
  testTabs (t) {
    const img = ErazerPaint.tabs();
    const doc = Erazer.analyze(img);
    const tabs = doc.root.roleCount("tab");
    t.ok("three tabs", tabs >= 3);
    const file = doc.root.findText("File");
    t.ok("tab File", (typeof(file) === "undefined") == false);
    const edit = doc.root.findText("Edit");
    t.ok("tab Edit", (typeof(edit) === "undefined") == false);
    if ( (tabs < 3 || (typeof(file) === "undefined")) || (typeof(edit) === "undefined") ) {
    }
  };
  testMenu (t) {
    const img = ErazerPaint.menu();
    const doc = Erazer.analyze(img);
    const items = doc.root.roleCount("menuitem");
  ...
  testIcon (t) {
    const img = ErazerPaint.iconRow();
    const doc = Erazer.analyze(img);
    const icons = doc.root.roleCount("icon");
    t.ok("plus is an icon", icons >= 1);
    const hit = doc.root.findRole("icon");
    if ( typeof(hit) === "undefined" ) {
    } else {
      const n = hit;
      t.ok("icon has an SVG", n.iconSvg.length > 20);
      t.ok("icon SVG is a path", n.iconSvg.indexOf("<path") >= 0);
    }
    const add = doc.root.findText("Add");
    t.ok("icon row reads Add", (typeof(add) === "undefined") == false);
  };
  testChipRow (t) {
    const img = ErazerPaint.chipRow();
    const doc = Erazer.analyze(img);
    const buttons = doc.root.roleCount("button");
    t.ok("three sibling chips are buttons", buttons >= 3);
    const rest = doc.root.findText("Rest");
    t.ok("chip Rest", (typeof(rest) === "undefined") == false);
    const sick = doc.root.findText("Sick");
    t.ok("chip Sick", (typeof(sick) === "undefined") == false);
    const ready = doc.root.findText("Ready");
    t.ok("wide chip Ready", (typeof(ready) === "undefined") == false);
    t.ok("wide chips are not tabs", doc.root.roleCount("tab") == 0);
    if ( ((buttons < 3 || (typeof(rest) === "undefined")) || (typeof(sick) === "undefined")) || (typeof(ready) === "undefined") ) {
    }
  };
  testChipStrip (t) {
    const img = ErazerPaint.chipStrip();
    const doc = Erazer.analyze(img);
    const buttons = doc.root.roleCount("button");
    const tabs = doc.root.roleCount("tab");
  ...
  testSlider (t) {
    const img = ErazerPaint.slider();
    const doc = Erazer.analyze(img);
    const sliders = doc.root.roleCount("slider");
    t.ok("slider track found", sliders >= 1);
    const hit = doc.root.findRole("slider");
    if ( typeof(hit) === "undefined" ) {
    } else {
      const n = hit;
  ...
  testGroupedList (t) {
    const img = ErazerPaint.groupedList();
    const doc = Erazer.analyze(img);
    t.eqInt(
      "a thin bar alone is not a slider",
      doc.root.roleCount("slider"),
      0
    );
    t.eqInt("six switches", doc.root.roleCount("switch"), 6);
    t.eqInt(
      "hairlines split the cards into six rows",
      doc.root.roleCount("listitem"),
      6
    );
    t.eqInt("each card is a list", doc.root.roleCount("list"), 2);
    const hit = doc.root.findText("Level");
    t.ok("a row label is read", (typeof(hit) === "undefined") == false);
    const root = doc.root;
    const boxes = ErazerLayoutNet.boxesFromTree(root, doc.width, doc.height);
    const net = ErazerLayoutNet.shared();
    const guesses = net.scan(boxes);
    let tall = 0;
    let i = 0;
  ...
  crossings (n) {
    ...
    while (i < n.kids.length) {
      ...
      if ( a.role != "underlay" ) {
        let j = i + 1;
        while (j < n.kids.length) {
          const b = n.kids[j];
          if ( (b.role != "underlay" && a.w * a.h >= 400) && b.w * b.h >= 400 ) {
            const ox = a.x < b.maxX() && b.x < a.maxX();
            const oy = a.y < b.maxY() && b.y < a.maxY();
            if ( ox && oy ) {
              c = c + 1;
            }
          }
  ...
  testFrostedList (t) {
    const img = ErazerPaint.frostedList();
    const doc = Erazer.analyze(img);
    const root = doc.root;
    t.eqInt(
      "a gradient backdrop stays one surface",
      root.roleCount("backdrop"),
      0
    );
    ...
    t.ok("the gradient reaches the JSON", doc.json.indexOf("gradient-from") >= 0);
  };
  testCornersAndRows (t) {
    const img = ErazerPaint.groupedList();
    const doc = Erazer.analyze(img);
    const root = doc.root;
    const hit = root.findRole("list");
    if ( typeof(hit) === "undefined" ) {
    ...
    let rows = 0;
    let at = doc.json.indexOf("erazer-listitem");
    while (at >= 0) {
      let line = doc.json.substring(at, at + 200 );
      ...
      const rest = doc.json.substring(at + 10, doc.json.length );
      const nx = rest.indexOf("erazer-listitem");
      if ( nx >= 0 ) {
        at = (at + 10) + nx;
      } else {
        at = 0 - 1;
  ...
  textInSwitch (n, inside) {
    let c = 0;
    if ( inside && (n.role == "text" || n.role == "label") ) {
      c = 1;
    }
    const deeper = inside || n.role == "switch";
    let i = 0;
    while (i < n.kids.length) {
      const k = n.kids[i];
  ...
  wordAt (o, text, x, y, line) {
    o.addLineWord(
      text,
      x,
      y,
      ErazerFont.textWidth(text, 2),
      ErazerFont.textHeight(2),
      95,
      line
    );
  };
  testOutsideWords (t) {
    const img = ErazerPaint.blank(340, 150, 242, 242, 247);
    ErazerFont.fillRoundRect(img, 20, 20, 300, 110, 10, 255, 255, 255);
    ErazerPaint.paintLabel(img, 36, 36, "Name");
    const vw = ErazerFont.textWidth("Value", 2);
    ErazerPaint.paintLabel(img, 304 - vw, 36, "Value");
    const tw = ErazerFont.textWidth("Title", 2);
    ErazerPaint.paintLabel(img, 20 + (((300 - tw) / 2) | 0), 90, "Title");
    ErazerPaint.paintSwitch(img, 250, 70, true);
    const o = ErazerOptions.defaults();
    this.wordAt(o, "Nimi", 36, 36, 1);
    this.wordAt(o, "Arvo", 304 - vw, 36, 1);
    this.wordAt(o, "Otsikko", 20 + (((300 - tw) / 2) | 0), 90, 2);
    o.addLineWord("J", 274, 72, 26, 26, 90, 3);
    o.addLineWord("zzz", 40, 100, 20, 10, 20, 4);
    const doc = Erazer.fromImageBuffer(img, o);
    const root = doc.root;
    const a = root.findText("Nimi");
    const b = root.findText("Arvo");
    const c = root.findText("Otsikko");
    t.ok("OCR text replaces the 5x7 reading", (typeof(a) === "undefined") == false);
    t.ok("the old reading is gone", typeof(root.findText("Name")) === "undefined");
    t.ok("words split at a wide gap", (typeof(b) === "undefined") == false);
    t.ok("a knob read as J is not text", typeof(root.findText("J")) === "undefined");
    t.ok("a low-confidence word is ignored", typeof(root.findText("zzz")) === "undefined");
    if ( ((typeof(a) === "undefined") == false && (typeof(b) === "undefined") == false) && (typeof(c) === "undefined") == false ) {
      const na = a;
      const nb = b;
      const nc = c;
      t.eqStr("label hugging the left", na.align, "left");
      t.eqStr("value at the right edge", nb.align, "right");
      t.eqStr("title in the middle", nc.align, "center");
      t.ok("font size is CSS-sized", nc.fontSize > 14);
    }
    t.ok("alignment reaches the JSON", doc.json.indexOf("\"text-align\":\"right\"") >= 0);
  };
  testSliders (t) {
    const img = ErazerPaint.sliders();
    const doc = Erazer.analyze(img);
    const sliders = doc.root.roleCount("slider");
  ...
  testJson (t) {
    const img = ErazerPaint.button();
    const doc = Erazer.analyze(img);
    t.ok("json is an EVG document", doc.json.indexOf("\"evg\":1") >= 0);
    t.ok("json names the button class", doc.json.indexOf("erazer-button") >= 0);
    t.ok("overlay is an SVG", doc.overlaySvg.indexOf("<svg") == 0);
    t.ok("outline names the button", doc.outline.indexOf("button") >= 0);
    t.ok("layout json has boxes", doc.layoutJson.indexOf("\"boxes\"") >= 0);
  };
  testLayoutFeatures (t) {
    const net = new ErazerLayoutNet();
    const list = net.synthList(4, 16.0);
    const feat = ErazerLayoutFeat.features(list);
    t.eqInt("feature vector is 40", feat.length, 40);
    t.ok("list is all labels", feat[18] > 0.99);
    t.ok("list is same column", feat[14] > 0.99);
    t.ok("list vertical regularity is high", feat[12] > 0.95);
    t.ok("list pair-pattern is off", feat[25] < 0.05);
    const rows = net.synthRows(16.0);
    const rf = ErazerLayoutFeat.features(rows);
    t.ok("property rows have checkboxes", rf[19] > 0.3);
    t.ok("property rows repeat a pair", rf[25] > 0.9);
    const scaled = ErazerLayoutFeat.scaleAll(list, 1.25);
    const sf = ErazerLayoutFeat.features(scaled);
    const dLab = ErazerLayoutFeat.absD((feat[18] - sf[18]));
    const dReg = ErazerLayoutFeat.absD((feat[12] - sf[12]));
    t.ok("em+scale leaves label fraction", dLab < 0.0001);
    t.ok("em+scale leaves vertical regularity", dReg < 0.0001);
    const augs = ErazerLayoutFeat.augment(list);
    t.ok("one label becomes many samples", augs.length >= 10);
    let gapBoxes = [];
    const a = ErazerLayoutBox.of("label", 0.0, 0.0, 80.0, 16.0, 16.0);
    const b = ErazerLayoutBox.of("label", 0.0, 40.0, 80.0, 16.0, 16.0);
    a.index = 0;
    b.index = 1;
    gapBoxes.push(a);
    gapBoxes.push(b);
    const g = ErazerLayoutFeat.describe(gapBoxes, "list", 1.0);
  ...
  testLayoutLearn (t) {
    ...
    const list = net.synthList(4, 16.0);
    const rows = net.synthRows(16.0);
    const grid = net.synthGrid(16.0);
    let groups = [];
    let labels = [];
    groups.push(ErazerLayoutSet.of(list));
    labels.push("list");
    groups.push(ErazerLayoutSet.of(rows));
    labels.push("property_row");
    groups.push(ErazerLayoutSet.of(grid));
    labels.push("grid");
    const aList = ErazerLayoutFeat.augment(list);
    const aRows = ErazerLayoutFeat.augment(rows);
    const aGrid = ErazerLayoutFeat.augment(grid);
    let i = 0;
    ...
    let r = 0;
    while (r < aRows.length) {
      groups.push(aRows[r]);
      labels.push("property_row");
      r = r + 1;
    };
    let g = 0;
    while (g < aGrid.length) {
      groups.push(aGrid[g]);
      labels.push("grid");
      g = g + 1;
    };
    net.fitAll(groups, labels, 8);
    const gl = net.predict(list);
    const gr = net.predict(rows);
    const gg = net.predict(grid);
    t.eqStr("stacked labels train as list", gl.type, "list");
    t.ok("list confidence is high", gl.confidence > 0.55);
    t.eqStr(
      "label+checkbox rows train as property_row",
      gr.type,
      "property_row"
    );
    ...
    const cands = ErazerLayoutFeat.candidates(list);
    t.ok("heuristics propose a list candidate", cands.length >= 1);
    net.ensureClass("inspector-group");
    net.teach(list, "inspector-group", 4);
    const custom = net.predict(list);
    t.eqStr("new concept can be taught", custom.type, "inspector-group");
  };
  testLayoutStructure (t) {
    const net = new ErazerLayoutNet();
    const list = net.synthList(5, 16.0);
    const g = ErazerLayoutFeat.describe(list, "list", 0.9);
    ...
    const tb = net.synthToolbar(4, 16.0);
    const tg = ErazerLayoutFeat.describe(tb, "toolbar", 0.9);
    t.eqStr("toolbar axis is horizontal", tg.axis, "horizontal");
    const img = ErazerPaint.menu();
    const doc = Erazer.analyze(img);
    t.ok("menu analyze writes layout json", doc.layoutJson.length > 20);
    t.ok("overlay can mark a primitive", doc.overlaySvg.indexOf("data-box=") >= 0);
  };
  testLayoutWeights (t) {
    const net = new ErazerLayoutNet();
    const list = net.synthList(4, 16.0);
    net.teach(list, "list", 4);
    const dump = net.dumpWeights();
    const copy = new ErazerLayoutNet();
    t.ok("a dump loads back", copy.loadWeights(dump));
    const a = net.predict(list);
    const b = copy.predict(list);
    t.eqStr("the reloaded net names the same class", b.type, a.type);
    t.ok("exponent notation survives", ErazerLayoutFeat.absD(copy.parseNum("8.4e-7") - 8.4e-7) < 1e-10);
...
function __js_main() {
  const test = new ErazerTest();
  const t = RgTest.forSuite("gallery/erazer");
  test.testAtlas(t);
  test.testOcr(t);
  test.testBlank(t);
  test.testButton(t);
