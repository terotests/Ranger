...
A small gallery study. Nine Ranger programs try to express the Rust a fluent
...
bash gallery/friendly/rust/compile.sh
```
...
Two holes I walked into while writing the study, which are writer bugs rather
than missing language:

1. An optional **string parameter** becomes `Option<Option<String>>` and does
   not rustc. Optional *locals* and optional *returns* are fine.
2. `try` / `throw` compiles, emits `panic!`, and **drops the catch block**.
   The binary dies. Documented in the target notes; still the spelling a
   Ranger programmer reaches for first.
...
Do **not** rely on `try` / `throw` on this target. Do **not** put
`@(optional):string` in a parameter list. Do **not** `return x` from an
optional function when `x` is non-optional — bind `def found@(optional):T`
and return that. Do **not** expect a Ranger `Enum` to become a Rust `enum`.
...
**Writer bug.** `fn shown:string (maybe@(optional):string)` — a perfectly
ordinary Ranger signature — emits `fn shown(maybe : Option<Option<String>>)`
and does not rustc. See [`attempts/02_optional_string_param.rgr`](attempts/02_optional_string_param.rgr).
I removed the parameter and used `??` on locals. That is a workaround, not
an idiom.
...
**Could it be done?** Not as Rust traits. Ranger traits are a source-level
reuse tool. They do emit a `dyn` trait object when the *same* Ranger trait
is used as a polymorphic type in larger programs (see `TARGET_NOTES.md`),
but that is an internal vtable with `rgf_*` field accessors, not
`impl Display` / `impl From` / `impl Iterator`. There is no spelling for
“implement this Rust standard trait.”
...
are no bounds, no `where`, no lifetime on the peek. A `_optionalInt`
helper sits on `Stack` so the C++ sibling emits `r_optional_primitive`;
Rust ignores it.
...
**Could it be done?** Only by not using `try`. A first-class `Result` in
the language, lowered to Rust `Result` and to exceptions or optionals on
the other targets, is the missing piece.
...
Ranked by how much they would change the studies above.
...
9. **`snake_case` names.** Every file still carries `#![allow(non_snake_case)]`.
   `PLAN_RUST_IDIOMATICITY.md` already ranks this. It is mechanical and it
...
```
gallery/friendly/rust/
  README.md                 this study
  compile.sh                Ranger → rustc → run, fails on [FAIL] or rustc
  ../src/0N_*.rgr           the nine programs (shared with the sibling studies)
  generated/0N_*.rs         snapshots of the writer output
  generated/0N_*.out        what the binary printed
  attempts/                 patterns that do not rustc, or panic
```

Related: [`PLAN_RUST_IDIOMATICITY.md`](../../../PLAN_RUST_IDIOMATICITY.md),
...
The sibling studies live under [`gallery/friendly/`](../README.md).
