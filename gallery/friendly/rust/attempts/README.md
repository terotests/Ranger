...
| [`09_throw_panics.rgr`](09_throw_panics.rgr) | `try` / `catch` | Compiles. `throw` is `panic!`. The catch block is a comment. The binary dies on the error path. Same file is the throw probe on the other targets: recover/except/catch on Go/Python/C++; runs on JavaScript, Dart, Java, and C#; rejected by `kotlinc`; would not `swiftc`. |
