...
The writer only defines that class when `str2int` (or a sibling) appears,
so the shared source now has a `_optionalInt` helper. That is a writer
bug, not a language gap. Monomorphization, not `template<class T>`.
...
2. Always emit `r_optional_primitive` (or use `std::optional`) when any
   optional scalar exists.
