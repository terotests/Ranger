...
function sizeOfWindow() {
  return [Math.max(900, Math.floor(window.innerWidth)), Math.max(600, Math.floor(window.innerHeight))];
}
...
canvas.addEventListener("pointerup", () => app.setPressed(""));
...
function showPicked() {
  ...
  status.textContent = "";
  if (v === "new2" || v === "new1") {
    src.value = v === "new2" ? TEMPLATE_V2 : TEMPLATE_V1;
...
document.getElementById("code-run").addEventListener("click", () => {
  ...
  status.textContent = app.appError(index) || (version === app.modeNow() ? "running" : `loaded — switch the emulator to UIX v${version} mode to run it`);
});
