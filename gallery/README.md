...
| [`uast`](uast/README.md) | Unified analysis model for CodeGraph: language plugins (Ranger now, TypeScript next) → UAST → semantic CodeModel → the existing CodeGraph view. Plan in [`uast/PLAN_UAST.md`](uast/PLAN_UAST.md) |
...
| [`ui`](ui/README.md) | EVG controllers measured against Radix: a behaviour-parity harness that runs the same spec through Ranger's own controllers and real `@radix-ui` components, and diffs what a user can observe. [Live at /ui/](https://terotests.github.io/Ranger/ui/) — twenty tree-literal demos, and Radix vs Ranger at [/ui/web/](https://terotests.github.io/Ranger/ui/web/) |
...
| [`evg/apple`](../lib/evg/apple/README.md) | The EVG painter and the CoreGraphics surface the Apple ports draw through — one backend for UIKit, SwiftUI and watchOS |
| [`firesim`](firesim/README.md) | A **Firebase, simulated**: Firestore, the Identity Toolkit, `firestore.rules` parsed and run, and a model that streams a word at a time — answering Google's own REST shapes, so switching an app to a real project is a base URL. Runs in Node, on a socket for a device simulator, or [wholly inside a browser tab](https://terotests.github.io/Ranger/firesim/), where the workbench page is a database browser over it — collections, documents, a document's fields with their types, a query builder and a live permission grid, drawn by EVG and controlled by `gallery/ui`'s own controllers |
...
| [`evg`](../lib/evg/README.md) — now `lib/evg`, **MIT** | **The layout engine** everything above draws through: the CSS-shaped box model, flex, grid, units, the stylesheet with `@media` and pseudo-classes, text measurement, transitions, hit testing, the accessibility tree, and the display list the painters read |
...
| [`rustfriendly`](rustfriendly/README.md) | A small study: write idiomatic Rust *in Ranger*, compile `-l=rust`, and record what the writer emits — what works, what cannot be said, and how the language could close the gap |
