#!/usr/bin/env bash
# Compile and run wasm_pong_runner_demo (headless, no SDL window).
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/../../.." && pwd)"
SOURCE="$ROOT/gallery/game_engine/scripting/wasm_pong_runner_demo.rgr"
OUT_DIR="$ROOT/tmp/wasm-demo"
CPP_FILE="$OUT_DIR/wasm_pong_demo.cpp"
BIN_FILE="$OUT_DIR/wasm_pong_demo"
...
RANGER_LIB="$ROOT/compiler/Lang.rgr:$ROOT/lib/stdops.rgr" node "$ROOT/bin/output.js" \
  -l=cpp "$SOURCE" -nodecli -d="tmp/wasm-demo" -o="wasm_pong_demo.cpp"
