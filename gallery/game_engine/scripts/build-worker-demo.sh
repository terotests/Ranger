#!/usr/bin/env bash
# Build + run the Ranger2D streaming worker demo: worker.wasm (culling + asset
# load policy) driven by a C host harness over the wasm3 bridge.
#
# Usage:
#   ./gallery/game_engine/scripts/build-worker-demo.sh          # build wasm + harness, run
#   ./gallery/game_engine/scripts/build-worker-demo.sh --no-run # build only
...
HARNESS="$CRATE/host_demo.c"
OUT_DIR="$ROOT/tmp/worker-demo"
BIN="$OUT_DIR/worker_demo"
...
echo "==> 2/2 build host harness (wasm3 bridge)"
CC="${CC:-gcc}"
