...
| the compiler itself (`ng_Compiler.rgr`, 76 files, 5 MB) | OOM | ~9 s | **does not fit in memory** |

Loading the 2.6 MB bundle takes ~1 s; the rest is the compile.

### The compiler compiling itself

This does not complete here. Three attempts, all ending in the same place:

| Heap cap | Died after |
|---|---|
| 12 GB | ~6 min |
| 13.6 GB | ~10 min |
| 13 GB (with the `return ";"` fix in) | ~45 min |

V8 spends ~96 % of its time in GC before it dies. The parser fix let the run
go much further — 45 minutes against 6 — which says the earlier attempts were
also failing work, not just failing faster; but it did not change the
outcome. The engine holds the program AST, 5 MB of source and the compiler's
whole working set as interpreter objects at once, and that multiplier is what
does not fit, not any single allocation.
