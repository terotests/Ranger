...
| Mesh editor (authoring tool) | in the gate: 7 module smokes + 72 JS↔Ranger parity checks + a 34-assert real-browser e2e (`mesh_editor/tests/run.sh`). **Tessellation kernels are now Ranger** — `SplineLathe.latheOnSpine` / `torusOnSpine` / `buildSpineFrames` / `torusUnitFitScale` in `spline_lathe.rgr` are the live path the editor calls (flat primitive arrays, so they also lower to native/wasm32); the JS originals survive only as the parity reference | **texture rasterizer still browser-only JS** — `eyeTexture.js` / `eyeEmotion.js` (~2k lines), so the README's "params only, rasterized at runtime" still has no runtime off the browser; `latheTessellate.js` part assembly still JS; preview host bypasses `RgRegistryBridge`; no asset export to `games/` |
...
| WASM32 / Rust | `RgWasmGameHost` runs a wasm32 game like a `.tsx` game (+ host→guest **input channel**); **`games/ylos3d_wasm` is now a faithful 1:1 Rust port of the full `ylos3d`** (split-screen LPC climber, moving platforms, enemies, 3D diamonds, super/HUD/finish) emitting the same bridge commands; `wasm_mem_str` implemented on native (string/asset args); launcher routes `engine=wasm`→`runWasm` | on-device SDL run (macOS/Pi) to eyeball parity |
