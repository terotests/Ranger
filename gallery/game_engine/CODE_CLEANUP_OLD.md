...
This file holds the inventory, vertex walkthrough, file-system map, entity/bridge
chapters, and other exploratory notes that used to live in `CODE_CLEANUP.md`.

**Active design direction** is only in
[`CODE_CLEANUP.md`](./CODE_CLEANUP.md) — *Architecture decisions (binding — from
design review)*. Prefer that document when deciding what to implement.

The material below is kept for archaeology and evidence; it is not the
...
| **Worked example (camera + physics mesh)** | Line-by-line target path for script-driven camera and object positions. |
...
For a full camera + physics-mesh walkthrough see **Worked example — camera
position and physics-driven mesh position** (W.1–W.7).
