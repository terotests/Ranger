...
//
// Shared Ylos 2 platformer logic — used by index.tsx (day) and night.tsx.

import { soundEvent, particleEvent, rumbleEvent, musicScoreEvent, stopMusicEvent } from "game_helpers";
import { drawDayLevelBackground } from "./ylos2_level_day";
import { drawNightLevelBackground } from "./ylos2_level_night";
...
let activeLevel;
let carryScores = false;

export function setupYlos2Game(level, options) {
  activeLevel = level;
  carryScores = options != null && options.carryScores === true;
}

function worldH() {
  return activeLevel.worldH;
}

function movingPlatSpeed() {
  if (activeLevel.movingPlatSpeed != null) {
    return activeLevel.movingPlatSpeed;
  }
  return 0.06;
}

function platColorBody() {
  return activeLevel.theme.platColorBody;
}

function platColorTop() {
  return activeLevel.theme.platColorTop;
}

function platColorBottom() {
  return activeLevel.theme.platColorBottom;
}

function goalPlatformDef() {
  const plats = activeLevel.platforms;
  return plats[plats.length - 1];
}

function p1StartX() {
  return activeLevel.p1StartX;
}

function p2StartX() {
  return activeLevel.p2StartX;
}
...
function buildPlatforms() {
  const out = [];
  let i = 0;
  while (i < activeLevel.platforms.length) {
    out.push(scalePlatform(activeLevel.platforms[i]));
    i = i + 1;
  }
  return out;
}
...
function buildEnemyDefs() {
  const out = [];
  let i = 0;
  while (i < activeLevel.enemyDefs.length) {
    const d = activeLevel.enemyDefs[i];
    out.push({
      x: scaleX(d.x),
      y: d.y,
      dir: d.dir,
...
function buildFruitDefs() {
  const out = [];
  let i = 0;
  while (i < activeLevel.fruitDefs.length) {
    const d = activeLevel.fruitDefs[i];
...
function buildDiamondDefs() {
  const out = [];
  let i = 0;
  while (i < activeLevel.diamondDefs.length) {
    const d = activeLevel.diamondDefs[i];
    out.push({ x: scaleX(d.x), y: d.y });
    i = i + 1;
  }
  return out;
...
function makeMovingPlatforms() {
  const out = [];
  let i = 0;
  while (i < activeLevel.movingPlatforms.length) {
    const d = activeLevel.movingPlatforms[i];
    const x = scaleX(d.x);
    out.push({
      x: x,
      prevX: x,
...
function updateMovingPlatforms(platforms, dt, staticPlats) {
  ...
  while (i < platforms.length) {
    ...
    const prevX = mp.x;
    let x = mp.x + mp.dir * movingPlatSpeed() * dt;
    let dir = mp.dir;
    if (x < mp.minX) {
      x = mp.minX;
      dir = 1;
...
function floorY() {
  return activeLevel.platforms[0].y;
}
...
export function staticLevelHeight() {
  return worldH();
}
...
function movingPlatformSprites(mpi, w, h) {
  ...
  const top = platColorTop();
  const body = platColorBody();
  const bottom = platColorBottom();
  return [
    movingPlatformLayerSprite("mp" + mpi + "t", w, PLAT_EDGE_H, top),
    movingPlatformLayerSprite("mp" + mpi + "m", w, bodyH, body),
    movingPlatformLayerSprite("mp" + mpi + "b", w, PLAT_EDGE_H, bottom)
  ];
}
...
function drawActiveBackground() {
  if (activeLevel.id == "night") {
    drawNightLevelBackground();
  } else {
    drawDayLevelBackground();
  }
}
...
export function createStaticBg() {
  ...
  while (i < platforms.length) {
    const p = platforms[i];
    bgFillRect(p.x, p.y, p.w, p.h, body.r, body.g, body.b);
    bgFillRect(p.x, p.y, p.w, PLAT_EDGE_H, top.r, top.g, top.b);
    bgFillRect(p.x, p.y + p.h - PLAT_EDGE_H, p.w, PLAT_EDGE_H, bottom.r, bottom.g, bottom.b);
    i = i + 1;
  }

  const flagX = scaleX(240);
  bgFillRect(flagX - 8, 80, 8, 90, theme.flagPole.r, theme.flagPole.g, theme.flagPole.b);
  bgFillRect(flagX, 88, 36, 20, theme.flagRed.r, theme.flagRed.g, theme.flagRed.b);
  bgFillRect(flagX, 96, 30, 8, theme.flagGold.r, theme.flagGold.g, theme.flagGold.b);
}
...
function diamondSprite(id) {
  ...
  let eg = 255;
  let eb = 255;
  if (theme.diamondBright) {
    br = theme.diamondBright.r;
    bg = theme.diamondBright.g;
    bb = theme.diamondBright.b;
  }
  if (theme.diamondEdge) {
    er = theme.diamondEdge.r;
    eg = theme.diamondEdge.g;
    eb = theme.diamondEdge.b;
  }
  return {
    id: id,
    kind: "bitmap",
...
export function sprites() {
  ...
  let f = 0;
  const fruit = activeLevel.theme.fruitColor;
  while (f < MAX_FRUITS) {
    list.push({ id: "f" + f, kind: "circle", rad: 6, r: fruit.r, g: fruit.g, b: fruit.b });
    f = f + 1;
  }
  let d = 0;
  ...
  while (mpi < MAX_MOVING_PLATFORMS) {
    const def = activeLevel.movingPlatforms[mpi];
...
function respawnMarkedDiamonds(prev) {
  ...
  let i = 0;
  while (i < MAX_DIAMONDS) {
    const def = activeLevel.diamondDefs[i];
    let taken = 0;
    if (prev) {
      if (prev[i]) {
        taken = prev[i].taken;
...
function tryStartSummitMusic(summitMusicStarted, events) {
  ...
  events.push(musicScoreEvent(activeLevel.summitMusic, false));
  return 1;
}

export function initState() {
  const slots = playerSlots();
  const cameraY = worldH() - VIEW_H;
  let score1 = 0;
  let score2 = 0;
  if (carryScores) {
  ...
  const p2 = makePlayerOnFloor(p2StartX(), -1);
...
function respawnPlayer(which) {
  ...
  if (which == 1) {
    return makePlayerOnFloor(p1StartX(), 1);
  }
  return makePlayerOnFloor(p2StartX(), -1);
}
...
export function hud(props) {
  ...
  let msg = activeLevel.title + "! " + activeLevel.hudIntro;
  if (victory == 0) {
    if (isSplitPane()) {
      if (paneIndex == 0) {
  ...
  if (victory == 1) {
    victoryLine = "Voitto!";
    msg = activeLevel.victoryRestartText;
  }
  return (
...
export function update(props) {
  ...
  if (showVictoryBanner({ playerSlots: slots, p1: p1, p2: p2 })) {
    if (wantsRestart(props, dual)) {
      const nextPath = activeLevel.nextLevelPath;
      if (nextPath != null && nextPath.length > 0) {
        saveGameData({ score1: s.score1, score2: s.score2, level: activeLevel.id });
        pushGame(nextPath);
        return {
          showNet: 0,
          playerSlots: slots,
          ...
          events: [stopMusicEvent(), soundEvent("win")]
        };
      }
