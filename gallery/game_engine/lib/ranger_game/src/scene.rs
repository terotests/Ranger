...
impl Scene {
    pub const fn new() -> Self {
        Self {
            ...
            sun: (Vec3::new(-1.0, -1.0, -1.0), Color::WHITE, 0.9),
            camera: Camera {
                projection: Projection::Perspective,
                eye: Vec3::new(0.0, 0.0, 4.0),
                target: Vec3 {
    ...
    fn entity(&mut self) -> Option<Entity> {
        for i in 0..MAX_NODES {
            if !self.nodes[i].alive {
                ...
                return Some(Entity {
                    id: i as u32 + 1,
                    generation: self.nodes[i].generation,
                });
            }
        }
        None
    ...
    fn node_index(&self, e: Entity) -> Option<usize> {
        ...
        (i < MAX_NODES && self.nodes[i].alive && self.nodes[i].generation == e.generation)
            .then_some(i)
    }
    ...
    pub fn spawn_mesh(&mut self, mesh: MeshAsset, material: MaterialId) -> Option<Entity> {
        let e = self.entity()?;
        if self.instance_count == MAX_NODES {
            return None;
        }
    ...
    pub fn node(&mut self, entity: Entity) -> NodeHandle<'_> {
        NodeHandle {
            scene: self,
            entity,
        }
    }
    pub fn light(&mut self, entity: Entity) -> LightHandle<'_> {
        LightHandle {
            scene: self,
            entity,
        }
    ...
    pub fn spawn_point_light(
        ...
        self.lights[self.light_count] = PointLight {
            node: e,
            colour,
            intensity,
            range,
    ...
    pub fn publish(&mut self) -> bool {
        ...
        for n in 0..self.instance_count {
            ...
            let Some(ni) = self.node_index(inst.node) else {
                continue;
            };
            if !self.nodes[ni].visible {
                continue;
            }
            ...
            for k in 0..mesh.vertices {
                ...
                let p = self.world_position(src.position, ni);
    ...
    fn world_position(&self, mut p: Vec3, mut index: usize) -> Vec3 {
        let mut depth = 0;
        loop {
            p = transform_local(p, self.nodes[index]);
            let Some(parent) = self.nodes[index].parent else {
                return p;
            };
            let Some(next) = self.node_index(parent) else {
                return p;
            };
            index = next;
            depth += 1;
...
impl Assets<'_> {
    pub fn material(&mut self, texture: u32, colour: Color, flags: u32) -> MaterialId {
        ...
        self.scene.materials[i] = Material {
            texture,
            colour,
            flags,
        };
...
impl NodeHandle<'_> {
    pub fn set_position(&mut self, p: Vec3) {
        if let Some(i) = self.scene.node_index(self.entity) {
            self.scene.nodes[i].position = p;
        }
    }
    pub fn set_rotation(&mut self, q: Quat) {
        if let Some(i) = self.scene.node_index(self.entity) {
            self.scene.nodes[i].rotation = q;
        }
    }
    pub fn set_scale(&mut self, s: Vec3) {
        if let Some(i) = self.scene.node_index(self.entity) {
            self.scene.nodes[i].scale = s;
        }
    }
    pub fn set_visible(&mut self, visible: bool) {
        if let Some(i) = self.scene.node_index(self.entity) {
            self.scene.nodes[i].visible = visible;
        }
    }
    pub fn set_parent(&mut self, parent: Option<Entity>) {
        if let Some(i) = self.scene.node_index(self.entity) {
            self.scene.nodes[i].parent = parent.filter(|p| self.scene.node_index(*p).is_some());
        }
    }
}
...
impl LightHandle<'_> {
    pub fn set_enabled(&mut self, enabled: bool) {
        for l in self.scene.lights.iter_mut() {
            if l.node == self.entity {
                l.enabled = enabled;
            }
        }
    }
...
fn transform_local(p: Vec3, n: Node) -> Vec3 {
    let q = n.rotation;
    let x = p.x * n.scale.x;
    let y = p.y * n.scale.y;
    let z = p.z * n.scale.z;
    ...
    Vec3::new(
        ix * q.w + iw * -q.x + iy * -q.z - iz * -q.y + n.position.x,
...
fn write_vertex(i: usize, p: Vec3, n: Vec3, c: Color, uv: [f32; 2]) {
    ...
    MESH.write_i32(o + 4, fixed(p.y));
    MESH.write_i32(o + 8, fixed(p.z));
    MESH.write_i32(o + 12, unit(n.x));
...
fn write_headers(v: usize, i: usize, s: usize) {
    MESH.write_u32(0, MESH_MAGIC);
    MESH.write_u32(4, 1);
    MESH.write_u32(8, MESH_SIZE as u32);
    MESH.write_u32(12, 0);
    MESH.write_u32(16, v as u32);
    MESH.write_u32(20, i as u32);
    MESH.write_u32(24, s as u32);
    MESH.write_u32(52, INDEX_OFFSET as u32);
    MESH.write_u32(56, SUBMESH_OFFSET as u32);
}
fn write_materials(s: &Scene) {
    MATERIALS.write_u32(0, MAT_MAGIC);
    MATERIALS.write_u32(4, 1);
    MATERIALS.write_u32(8, MAT_SIZE as u32);
    MATERIALS.write_u32(16, s.material_count as u32);
    for i in 0..s.material_count {
        let o = 20 + i * 16;
        let m = s.materials[i];
        MATERIALS.write_u32(o, m.texture);
        MATERIALS.write_u32(o + 4, m.colour.packed());
        MATERIALS.write_u32(o + 8, m.flags);
    }
}
fn write_camera(c: &Camera) {
    CAMERA.write_u32(0, CAM_MAGIC);
    CAMERA.write_u32(4, 2);
    CAMERA.write_u32(8, CAM_SIZE as u32);
    CAMERA.write_u32(
        16,
        match c.projection {
            Projection::Affine2d => 0,
            Projection::Ortho3d => 1,
    ...
    for (o, v) in [
        ...
        CAMERA.write_i32(o, if o < 44 { fixed(v) } else { unit(v) });
    }
    CAMERA.write_i32(56, fixed(c.fovy));
    CAMERA.write_i32(60, fixed(c.near));
    CAMERA.write_i32(64, fixed(c.far));
    CAMERA.write_i32(136, fixed(c.ortho_height));
}
fn write_lights(s: &Scene) {
    LIGHTS.write_u32(0, LIT_MAGIC);
    LIGHTS.write_u32(4, 1);
    LIGHTS.write_u32(8, LIT_SIZE as u32);
    LIGHTS.write_u32(16, s.ambient.0.packed());
    LIGHTS.write_i32(20, fixed(s.ambient.1));
    LIGHTS.write_i32(24, unit(s.sun.0.x));
    LIGHTS.write_i32(28, unit(s.sun.0.y));
    LIGHTS.write_i32(32, unit(s.sun.0.z));
    LIGHTS.write_u32(36, s.sun.1.packed());
    LIGHTS.write_i32(40, fixed(s.sun.2));
    let mut n = 0;
    for l in s.lights.iter().take(s.light_count) {
        if !l.enabled {
            continue;
        }
        ...
        let o = 48 + n * 24;
        LIGHTS.write_i32(o, fixed(s.nodes[i].position.x));
        LIGHTS.write_i32(o + 4, fixed(s.nodes[i].position.y));
        LIGHTS.write_i32(o + 8, fixed(s.nodes[i].position.z));
        LIGHTS.write_u32(o + 12, l.colour.packed());
        LIGHTS.write_i32(o + 16, fixed(l.intensity));
        LIGHTS.write_i32(o + 20, fixed(l.range));
        n += 1;
    }
...
macro_rules! scene_exports {
    () => {
        ...
        pub extern "C" fn rg_mesh_ptr() -> i32 {
            $crate::scene::mesh_ptr()
        }
        #[no_mangle]
        pub extern "C" fn rg_mesh_size() -> i32 {
            $crate::scene::mesh_size()
        }
        #[no_mangle]
