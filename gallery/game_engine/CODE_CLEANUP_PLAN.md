...
does not inherit the reconciler-era layout.
...
The existing `three/`, `scripting/`, `physics/`, and `wasm/` trees stay as the
**v1 / demo** stack until a later migration step. v2 starts empty of render
backends and only pulls in files that the early gates actually need.

---

## Principles

1. **New tree, selective moves.** Create `gallery/game_engine/v2/` first. Copy or
   move only the minimum files required for the current step’s tests. Do not
   bulk-import `three/src` or the reconciler bridge.
...
Step 0 (this change) creates every directory above with a `README.md`. Later
steps fill implementations and tests under those READMEs’ contracts.
...
| **12** | Demo migration + `RETIRE-RECONCILE` | 11 | Yes |
...
## Phase 12 — Migrate demos + retire reconciler

Point demos at v2 live path; delete `ThreeTsxBridge.reconcile` when
`RETIRE-RECONCILE` criteria in CODE_CLEANUP are met. Track retirement explicitly;
do not leave dual paths undocumented.

---

## Selective move list (guidance)

Move **into v2 only when a phase needs it**. Prefer copy-then-adapt for shared
pdf_writer pieces until v2 owns the API, then delete or thin the old path.

| Keep in v1 for now | Why |
|--------------------|-----|
| `three/tsx/*` wrapper tree + reconciler | Temporary compatibility (D-SYNC) |
| `three/src/three_gl_*.rgr`, software backend | Phase 11 only |
| Full `scripting/*` game runners | Phase 12 migration |
| Demo games under `games/` | Consume v2 later |
...
- [ ] **Phase 12** — Demo migration + `RETIRE-RECONCILE`
