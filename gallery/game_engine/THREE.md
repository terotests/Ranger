...
|---|---|---|---|
| **Math** | Vector3, Euler, Quaternion, Matrix4, Box3, Color, MathUtils ✅; Frustum (+ the Plane bits it needs) ✅ used for view-frustum culling, value-parity tested vs three.js; Vector3.setFromSphericalCoords ◐ | Vector2, Vector4, Matrix3, Sphere, Plane (public), Ray, Triangle, Spherical, Cylindrical | prerequisite for raycasting, UV/2D work |
| **Core** | Object3D (transforms/world matrix/children ⇒ also covers Group), **nested hierarchy reconciled through the interpreter** (façade `Group`/`Mesh.add` → host parenting → composed world matrices, value-parity tested vs three.js), BufferGeometry (position/normal/uv/tangent/index), Clock via ThreeTimer ✅ | Raycaster, InstancedBufferGeometry, InterleavedBuffer, BufferAttribute usage beyond the fixed set, Layers, morph attributes | picking/interaction examples, instancing |
...
| **Geometries** | BoxGeometry, TeapotGeometry ✅; Plane, Circle, Ring, Sphere, Cylinder, Cone, Torus, TorusKnot ✅ (object model ported 1:1, façade-exposed, reconciled by the bridge into the host, and value-parity tested vs real three.js through the interpreter) | Tetra/Octa/Icosa/Dodeca, Capsule, Extrude, Lathe, Tube, Shape, Text, Edges/Wireframe geometry | **most `webgl_geometry_*` examples** |
...
| **Animation** | AnimationMixer, AnimationClip, KeyframeTrack (Number/Vector/Quaternion) ✅ — linear + three.js-exact quaternion slerp, applied to a target's TRS through the interpreter, value-parity tested vs three.js | looping/clamping + Discrete/Smooth interpolation, multiple actions/weights/crossfade, morph-target & skeletal animation, AnimationObjectGroup | **animation examples**, animated glTF |
