...
class EvgBitmapTracerTest  {
      ...
      if ( isDigit ) {
        ...
        if ( ch == "." ) {
          ...
          if ( ch == "-" ) {
            ...
            if ( a >= 0 ) {
              if ( EvgBitmapTracerTest.wholeTurns(have, want) == false ) {
                bad = bad + 1;
              }
              want = a;
              have = 0;
  ...
  commandLetters (d) {
    ...
    while (i < n) {
      const ch = d.substring(i, (i + 1) );
      if ( EvgBitmapTracerTest.arityOf(ch) >= 0 ) {
        count = count + 1;
      }
      i = i + 1;
    };
  ...
  testRepeatedCommandLettersAreLeftOut (t) {
    const bm = this.makeRing(64);
    const o = EvgTraceOptions.defaults();
    const tr = EvgBitmapTracer.fromBinary(bm, o);
    tr.trace();
    const d = tr.getPathData();
    const letters = this.commandLetters(d);
    const cmds = tr.commandCount();
    t.ok("a curve run spends fewer letters than it has commands", letters < cmds);
    const miscount = this.miscountedCommands(d);
    t.eqInt("and every run is still a whole number of turns", miscount, 0);
  };
  makeRing (n) {
    const bm = EvgBinaryBitmap.create(n, n);
    const c = ((n / 2) | 0);
...
EvgBitmapTracerTest.wholeTurns = function(have, want) {
  if ( want == 0 ) {
    return have == 0;
  }
  if ( have < want ) {
  ...
  const rem = have - ((((have / want) | 0)) * want);
  return rem == 0;
};
...
function __js_main() {
  ...
  test.testRepeatedCommandLettersAreLeftOut(t);
