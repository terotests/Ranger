...
EVGBoxShorthandTest.padOf = function(el) {
  return (((((((el.box.paddingTopPx.toString())) + " ") + ((el.box.paddingRightPx.toString()))) + " ") + ((el.box.paddingBottomPx.toString()))) + " ") + ((el.box.paddingLeftPx.toString()));
};
EVGBoxShorthandTest.marginOf = function(el) {
  return (((((((el.box.marginTopPx.toString())) + " ") + ((el.box.marginRightPx.toString()))) + " ") + ((el.box.marginBottomPx.toString()))) + " ") + ((el.box.marginLeftPx.toString()));
};
EVGBoxShorthandTest.boxFor = function(decl) {
  const s = new EVGStyleSheet();
  s.parse((".b { " + decl) + " }");
  const el = EVGElement.createDiv();
  el.className = "b";
  s.applyTree(el, "");
  el.box.resolveUnits(400.0, 200.0, 16.0, 16.0);
  return el;
};
EVGBoxShorthandTest.testPadding = function(c) {
  console.log("");
  console.log("-- padding, one to four values");
  c.eqStr("one value is all four sides", EVGBoxShorthandTest.padOf(EVGBoxShorthandTest.boxFor("padding: 10px;")), "10 10 10 10");
  c.eqStr("two values are vertical then horizontal", EVGBoxShorthandTest.padOf(EVGBoxShorthandTest.boxFor("padding: 0 16px;")), "0 16 0 16");
  c.eqStr("three are top, horizontal, bottom", EVGBoxShorthandTest.padOf(EVGBoxShorthandTest.boxFor("padding: 1px 2px 3px;")), "1 2 3 2");
  c.eqStr("four are top, right, bottom, left", EVGBoxShorthandTest.padOf(EVGBoxShorthandTest.boxFor("padding: 1px 2px 3px 4px;")), "1 2 3 4");
  c.eqStr("a bare zero needs no unit", EVGBoxShorthandTest.padOf(EVGBoxShorthandTest.boxFor("padding: 0;")), "0 0 0 0");
  c.eqStr("the badge's own declaration", EVGBoxShorthandTest.padOf(EVGBoxShorthandTest.boxFor("padding: 2px 10px;")), "2 10 2 10");
  c.eqStr("extra whitespace between components", EVGBoxShorthandTest.padOf(EVGBoxShorthandTest.boxFor("padding:   4px   8px  ;")), "4 8 4 8");
  c.eqStr("a percentage resolves against the width on all four sides", EVGBoxShorthandTest.padOf(EVGBoxShorthandTest.boxFor("padding: 10%;")), "40 40 40 40");
  c.eqStr("em against the element's own font size", EVGBoxShorthandTest.padOf(EVGBoxShorthandTest.boxFor("padding: 1em 2em;")), "16 32 16 32");
};
EVGBoxShorthandTest.testMargin = function(c) {
  console.log("");
  console.log("-- margin, which may go negative");
  c.eqStr("one value is all four sides", EVGBoxShorthandTest.marginOf(EVGBoxShorthandTest.boxFor("margin: 10px;")), "10 10 10 10");
  c.eqStr("four sides in order", EVGBoxShorthandTest.marginOf(EVGBoxShorthandTest.boxFor("margin: 1px 2px 3px 4px;")), "1 2 3 4");
  c.eqStr("a negative margin survives", EVGBoxShorthandTest.marginOf(EVGBoxShorthandTest.boxFor("margin: -4px 8px;")), "-4 8 -4 8");
  c.eqStr("margin: 0 auto is accepted, and auto is zero here", EVGBoxShorthandTest.marginOf(EVGBoxShorthandTest.boxFor("margin: 0 auto;")), "0 0 0 0");
  c.eqStr("a percentage margin resolves against the width", EVGBoxShorthandTest.marginOf(EVGBoxShorthandTest.boxFor("margin: 5%;")), "20 20 20 20");
};
EVGBoxShorthandTest.afterBad = function(decl) {
  const s = new EVGStyleSheet();
...
EVGBoxShorthandTest.testRejected = function(c) {
  console.log("");
  console.log("-- declarations a browser drops whole");
  c.eqStr("five values is not a shorthand", EVGBoxShorthandTest.afterBad("padding: 1px 2px 3px 4px 5px;"), "7 7 7 7");
  c.eqStr("a component that is not a length", EVGBoxShorthandTest.afterBad("padding: 1px nonsense;"), "7 7 7 7");
  c.eqStr("a negative padding takes the whole declaration with it", EVGBoxShorthandTest.afterBad("padding: -4px;"), "7 7 7 7");
  c.eqStr("and a negative one among good ones", EVGBoxShorthandTest.afterBad("padding: 4px -4px;"), "7 7 7 7");
  c.eqStr("padding: auto is not a thing", EVGBoxShorthandTest.afterBad("padding: auto;"), "7 7 7 7");
  c.eqStr("a unit EVG does not know", EVGBoxShorthandTest.afterBad("padding: 1px 4ch;"), "7 7 7 7");
};
EVGBoxShorthandTest.testLaidOut = function(c) {
  console.log("");
  console.log("-- and it reaches the layout");
  const s = new EVGStyleSheet();
  ...
  layout.layout(row);
  c.eqStr("the first child starts one padding in", (kid.calculatedX.toString()), "16");
  c.eqStr("and the row is still its declared width", (row.calculatedWidth.toString()), "200");
};
/* static JavaSript main routine at the end of the JS file */
function __js_main() {
  const c = new BoxCheck();
  console.log("=== EVG box shorthands: one to four values ===");
