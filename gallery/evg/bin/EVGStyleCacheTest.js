...
EVGStyleCacheTest.sheetText = function() {
  return ((((((((((((("" + ".page { display: flex; flex-direction: column; width: 400px; }") + ".box { width: 100px; height: 20px; background-color: #cccccc;") + "       opacity: 1; transform: none; color: #111111; }") + ".box:hover  { background-color: #ff0000; opacity: 0.5; }") + ".box:focus  { background-color: #00ff00; transform: scale(1.2); }") + ".box:active { background-color: #0000ff; }") + ".box:disabled { opacity: 0.25; }") + ".wide { width: 220px; }") + ".tall { height: 40px; }") + ".theme-dark .box { color: #eeeeee; background-color: #222222; }") + ".plain { width: 60px; height: 10px; }") + ".plain:hover { opacity: 0.35; }") + "@media (max-width: 300px) { .box { width: 50px; } }") + ".ink { color: #333333; font-size: 13px; }";
};
EVGStyleCacheTest.box = function(i, variant, state, inline) {
  ...
  el.textContent = "n" + ((i.toString()));
  return el;
};
EVGStyleCacheTest.page = function() {
  ...
  while (i < 192) {
    const variant = i % 6;
    const state = (Math.floor( (i / 6))) % 16;
    const inline = ((Math.floor( (i / 96))) % 2) == 1;
    p.addChild(EVGStyleCacheTest.box(i, variant, state, inline));
    i = i + 1;
  };
  return p;
};
EVGStyleCacheTest.paintOf = function(root, theme, w, cached) {
...
EVGStyleCacheTest.sameBothWays = function(c, label, theme, w) {
  const a = EVGStyleCacheTest.page();
  const b = EVGStyleCacheTest.page();
  c.eqStr(label + ": the cache paints what the scan painted", EVGStyleCacheTest.paintOf(a, theme, w, true), EVGStyleCacheTest.paintOf(b, theme, w, false));
};
EVGStyleCacheTest.testFixtureParses = function(c) {
  console.log("--- the fixture is what it says it is ---");
  const s = new EVGStyleSheet();
  s.parse(EVGStyleCacheTest.sheetText());
  const n = s.getErrorCount();
  let i = 0;
  while (i < n) {
    console.log("       " + s.getError(i));
    i = i + 1;
  };
  c.eqInt("the stylesheet has no errors", n, 0);
  c.ok("and it has rules with states, themes and media in it", s.getRuleCount() >= 10);
};
EVGStyleCacheTest.testAgreement = function(c) {
  console.log("--- cached == resolved from scratch ---");
...
EVGStyleCacheTest.testViewportInvalidates = function(c) {
  console.log("--- one sheet, two surfaces ---");
  const s = new EVGStyleSheet();
  s.parse(EVGStyleCacheTest.sheetText());
  const wide = EVGStyleCacheTest.page();
  s.setViewport(400.0, 600.0, false);
  s.applyTree(wide, "");
  const wideFirst = wide.children[0];
  const wideW = (wideFirst.width.value.toString());
  const narrow = EVGStyleCacheTest.page();
  s.setViewport(280.0, 600.0, false);
  s.applyTree(narrow, "");
  const narrowFirst = narrow.children[0];
  const narrowW = (narrowFirst.width.value.toString());
  c.eqStr("the wide surface gives the base width", wideW, "100");
  c.eqStr("and the narrow one the media width", narrowW, "50");
  const wideAgain = EVGStyleCacheTest.page();
  s.setViewport(400.0, 600.0, false);
  s.applyTree(wideAgain, "");
  const againFirst = wideAgain.children[0];
  c.eqStr("and back to the base width", (againFirst.width.value.toString()), "100");
};
EVGStyleCacheTest.testThemeOnOneSheet = function(c) {
  console.log("--- one sheet, two themes ---");
  const s = new EVGStyleSheet();
  s.parse(EVGStyleCacheTest.sheetText());
  s.setViewport(400.0, 600.0, false);
  const plain = EVGElement.createDiv();
  plain.className = "box";
  s.applyTree(plain, "");
  const plainC = plain.backgroundColor;
  const dark = EVGElement.createDiv();
  dark.className = "box";
  s.applyTree(dark, "dark");
  const darkC = dark.backgroundColor;
  const plain2 = EVGElement.createDiv();
  plain2.className = "box";
  s.applyTree(plain2, "");
  const plain2C = plain2.backgroundColor;
...
EVGStyleCacheTest.testInlineWins = function(c) {
  console.log("--- and an inline attribute still outranks the sheet ---");
  const s = new EVGStyleSheet();
  s.parse(EVGStyleCacheTest.sheetText());
  s.setViewport(400.0, 600.0, false);
  const bare = EVGElement.createDiv();
  bare.className = "box";
  const inked = EVGElement.createDiv();
  inked.className = "box";
  ...
  page.addChild(inked);
  s.applyTree(page, "");
  const bareC = bare.backgroundColor;
  const inkedC = inked.backgroundColor;
  ...
  s.applyTree(page, "");
  const hovC = inked.backgroundColor;
  c.eqStr("even hovered, where a state rule would have set it", hovC.toHexString(), "#123456");
};
EVGStyleCacheTest.testStateFlips = function(c) {
  console.log("--- a state goes on, and comes off again ---");
  const s = new EVGStyleSheet();
  s.parse(EVGStyleCacheTest.sheetText());
  s.setViewport(400.0, 600.0, false);
  const el = EVGElement.createDiv();
  el.className = "box";
  s.applyTree(el, "");
  const baseC = el.backgroundColor;
  const base = baseC.toHexString();
  el.isHovered = true;
  s.applyTree(el, "");
  const hovC = el.backgroundColor;
  const hovered = hovC.toHexString();
  el.isHovered = false;
  s.applyTree(el, "");
  const backC = el.backgroundColor;
  const back = backC.toHexString();
  ...
  const pl = EVGElement.createDiv();
  pl.className = "plain";
  s.applyTree(pl, "");
  const plBase = (pl.opacity.toString());
  pl.isHovered = true;
  s.applyTree(pl, "");
  const plHover = (pl.opacity.toString());
  pl.isHovered = false;
  s.applyTree(pl, "");
  const plBack = (pl.opacity.toString());
...
EVGStyleCacheTest.testItCaches = function(c) {
  console.log("--- and it answers from the plan ---");
  const s = new EVGStyleSheet();
  s.parse(EVGStyleCacheTest.sheetText());
  s.setViewport(400.0, 600.0, false);
  const p = EVGStyleCacheTest.page();
  s.applyTree(p, "");
  console.log(((("       " + ((s.planMisses.toString()))) + " scans, ") + ((s.planHits.toString()))) + " answered from a plan");
  ...
  const before = s.planMisses;
  s.applyTree(p, "");
  c.eqInt("a second pass over the same tree scans nothing", s.planMisses - before, 0);
};
/* static JavaSript main routine at the end of the JS file */
function __js_main() {
  const c = new StyleCacheCheck();
  console.log("=== EVG resolved-style cache ===");
