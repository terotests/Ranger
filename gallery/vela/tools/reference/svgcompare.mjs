// =============================================================================
// svgcompare.mjs — two SVG documents, compared as ink.
// =============================================================================
// Shared by `render.mjs`, which holds Vela's own suite to a quarter of a pixel
// against the reference renderer, and by `coverage.mjs`, which asks the looser
// question of how much of the published Vega-Lite gallery this draws at all.
//
// Both need the same thing: an SVG turned into absolute outlines and absolute
// text anchors, so that two documents are compared by what they put on the
// page rather than by how they wrote it down. This code lived inside
// render.mjs; the only edit in moving it is the word `export`.
// =============================================================================
...
export const TEXT_TOLERANCE = 0.5;
 ...
 */
export function comparePrimitives(refs, gots) {
  const findings = [];
  const taken = new Set();
  let matched = 0;
