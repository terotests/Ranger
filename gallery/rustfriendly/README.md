...
npm run rustfriendly:test        # or: bash gallery/rustfriendly/compile.sh
```
...
| nine studies, no gate | ten studies plus the attempts, under `npm run rustfriendly:test` |
...
gallery/rustfriendly/
  ...
  compile.sh                Ranger → rustc → run, then attempts must be refused
  src/NN_*.rgr              the ten programs
  generated/NN_*.rs         snapshots of the writer output
  generated/NN_*.out        what each binary printed
  attempts/                 forms the target cannot express, each with its
                            `; EXPECT-ERROR:` line — see attempts/README.md
```

`npm run rustfriendly:test` runs all of it.
