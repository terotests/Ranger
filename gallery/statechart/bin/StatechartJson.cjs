...
StatechartJson.guardOf = function(spec) {
  ...
  if ( spec.has("is") ) {
    g.kind = "is";
    g.source = StatechartJson.valueOf(spec.get("is"));
    g.operand = ScValOps.str(spec.stringOr("equals", ""));
    return g;
  }
  if ( spec.has("not") ) {
    g.kind = "not";
