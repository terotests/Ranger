...
| **M5 New project + patterns + kit** ✅ | the new-project sheet; all six patterns (`RavePatterns.create`); Select / Tabs / Dialog / Toggle / Input / Button / Sheet / Card / Command / Sidebar bound to `gallery/ui` controllers at run time (`RaveBind`, one binding per viewport); `ButtonCtl`, `SheetCtl`, `CardCtl`, `CommandCtl`, `SidebarCtl` added to `gallery/ui` with conformance specs that match the browser (cmdk, Radix Dialog, `<button>`, shadcn markup) | **the ten-minute test** ✅ `RaveTest.tenMinutes`: sheet → login → items → settings → phone tab → keyboard, through the editor's own doors |
| **M6 Export** ✅ | `gallery/rave/export`: the runtime compiled without the editor (`RaveExport`), a host with one view the size of the window and the route in `location.hash`, `build.mjs --app FILE --out DIR`, deployed at `/rave/app/` | `rave:export:smoke`: the M0 script against `generated-host.js` — guard, keyboard sign-in, the sidebar folding at 390, the settings tabs |
