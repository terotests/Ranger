...
| **M5 New project + patterns + kit** ✅ | the new-project sheet; all six patterns (`RavePatterns.create`); Select / Tabs / Dialog / Toggle / Input bound to `gallery/ui` controllers at run time (`RaveBind`, one binding per viewport). Still open: `ButtonCtl`, `SheetCtl`, `CommandCtl`, `SidebarCtl` as new controllers with conformance specs | **the ten-minute test** ✅ `RaveTest.tenMinutes`: sheet → login → items → settings → phone tab → keyboard, through the editor's own doors |
