...
### 7.1 The document as markup

`RaveText` is `app.rave.json` without the ids, shaped like HTML: words are an
element's content, a node's rule is its `style=`, `<media below="md"/>` is the
same node at another width, `<on click="navigate" arg="/x"/>` is an action.

It is HTML-shaped for one reason found the hard way. The first version was
indentation-based, and an answer copied back through a chat window arrives
with its leading spaces stripped — at which point a format whose tree IS its
indentation has no tree, and produces one error per line. Closing tags are
robust to every whitespace accident between here and a model. A format with
no ids in it is also the strongest round-trip test the document has.
...
| **M5.6 The AI menu** ✅ | `RaveText`: the document as HTML-shaped markup with no ids in it, written and read by one grammar, so a document survives a round trip through something that has never seen this codebase. `AI…` writes a prompt — the format, the CSS the engine will and will not take, the thing as it stands, the ask — and reads back an answer over the document, the page or the selected node. ⌘V pastes one in. No service is called: a person is the wire | a document written out, read back and written again is the same markup, and markup on one line reads the same as markup laid out; an answer over a node lands as one undo step with its shared classes merged; an answer that is not the markup is refused whole, with the line and a count rather than one error per line |
