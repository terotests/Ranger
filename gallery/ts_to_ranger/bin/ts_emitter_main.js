...
class TSEmitter  {
  ...
  emitAssignment (node) {
    const lhs = this.emitAssignTarget((node.left));
    const rhs = this.emitExpr((node.right), "expr");
  ...
  emitVarDecl (node) {
    ...
    while (i < (node.children.length)) {
      ...
      const rtype = this.inferType(name, rhs);
      if ( (rhs.length) > 0 ) {
  ...
  containsSubstring (hay, needle) {
    ...
    let i = 0;
    while (i <= (hlen - nlen)) {
      const ch = hay.substring(i, (i + nlen) );
      if ( ch == needle ) {
        return true;
      }
      i = i + 1;
  ...
  emitIf (node) {
    ...
    if ( typeof(node.left) != "undefined" ) {
      cond = this.emitExpr((node.left), "expr");
    }
  ...
  emitReturn (node) {
    ...
    const expr = this.emitExpr(val, "expr");
  ...
  emitSpriteFromObject (node, varName) {
    ...
    while (i < (node.children.length)) {
      ...
      const val = this.emitPropertyValue(prop);
  ...
  emitStateReturn (node, patch) {
    ...
    while (i < (node.children.length)) {
      ...
      if ( key == "entities" ) {
        ...
        if ( (key == "vx") || (key == "vy") ) {
          const val = this.emitExpr(valNode, "expr");
          ...
          const val2 = this.emitExpr(valNode, "expr");
      ...
      if ( valNode.nodeType == "ObjectExpression" ) {
        ...
        while (j < (valNode.children.length)) {
          ...
          if ( p2.nodeType == "Property" ) {
            ...
            const v = this.emitPropertyValue(p2);
  ...
  emitPropertyValue (prop) {
    if ( typeof(prop.left) != "undefined" ) {
      return this.emitExpr((prop.left), "expr");
    }
    return "0";
  };
  propKey (prop) {
  ...
  emitMember (node) {
    ...
    const base = this.emitExpr((node.left), "expr");
  ...
  emitBinary (node) {
    let op = node.value;
    const left = this.emitExpr((node.left), "expr");
    const right = this.emitExpr((node.right), "expr");
    if ( op == "===" ) {
      op = "==";
    }
    if ( op == "!==" ) {
  ...
  emitUnary (node) {
    ...
    const arg = this.emitExpr((node.left), "expr");
    if ( op == "-" ) {
      return ("(0.0 - " + arg) + ")";
    }
    return op + arg;
