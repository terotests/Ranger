...
echo "==> JavaScript"
node dist/rgrc.js -es6 "$SRC" -d=gallery/docx_viewer/bin -o=DocxLoadSpeedTest.js -nodecli > "$OUT/js.log" 2>&1 || {
...
echo
echo "==> C++ ($CXX)"
node dist/rgrc.js -l=cpp "$SRC" -nodecli -d="$OUT" -o=DocxLoadSpeedTest.cpp > "$OUT/cpp.log" 2>&1 || {
