#!/usr/bin/env bash
# Build the RealTrainer demo, put it on an emulator (starting one if none is running),
# and open it.
#
#   npm run rt:android:run
#   npm run rt:android:run -- --avd Pixel_Tablet_API_34
#   npm run rt:android:run -- --logcat
#   npm run rt:android:run -- --no-build          # reinstall what is already built
#
...
#
# The page is the view: the demo is responsive, so a phone gets the phone
# shell with the bottom bar and a tablet the desktop one with the rail, each
# at its own size — the same page as the browser at `?page=fit`.
#
...
The port itself can be checked without an SDK:
    ...
    npm run rt:android:verify      # the facade's rules, on Node
    npm run rt:android:typecheck   # type-check the two Android-only files (kotlinc)
MSG
  exit 1
fi
