...
fs.writeFileSync(
  ...
  "// Generated by web/build.mjs — do not edit.\n" +
    ...
    // installs itself into (`lib/evg/gl/evg-measure.js`).
...
// THE STYLESHEET, ALREADY PARSED. 104 KB of CSS produces the same 786 rules
// and 3436 declarations on every load on every device, and it costs about
// 12 ms cold to find that out — before the first pixel, for an answer that
// was known when the page was built. So the build parses it, with the app's
// OWN parser so the two cannot differ, and ships what the parser kept:
// 18.0 KB gzipped becomes 12.2, because every comment, every space and every
// selector the parser did not need is gone with it.
//
// `RealTrainerDemo.init` takes either, and tells them apart by the header.
// The checks go on handing it the source.
const sheetSource = fs.readFileSync(path.join(HERE, "realtrainer.css"), "utf8");
const parsedSheet = (() => {
  const { EVGStyleSheet } = createRequire(import.meta.url)(APP_CJS);
  const sheet = new EVGStyleSheet();
  sheet.parse(sheetSource);
  if (sheet.getErrorCount() > 0) {
    console.error(`build.mjs: the stylesheet does not parse cleanly — ${sheet.getError(0)}`);
    process.exit(3);
  }
  const text = sheet.toText();
  // Read back at once, and compared: a sheet that did not survive its own
  // round trip must not be shipped, and this is the cheapest place to notice.
  const back = new EVGStyleSheet();
  if (!back.loadText(text) || back.getRuleCount() !== sheet.getRuleCount()
      || back.getVarCount() !== sheet.getVarCount()) {
    console.error("build.mjs: the written-down stylesheet does not read back as itself");
    process.exit(3);
  }
  return text;
})();
