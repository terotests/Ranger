...
#
#   bash gallery/pptx/android/scripts/build-app.sh [debug|release]
#
...
if [ -z "$ANDROID_HOME" ] && [ -z "$ANDROID_SDK_ROOT" ]; then
  ...
  echo "The port itself can be checked without one: bash gallery/pptx/android/scripts/verify-desktop.sh" >&2
  exit 1
fi
