...
log=$(node dist/rgrc.js -es6 gallery/pptx/web/pptx_web.rgr -d="$STAGE" -o=pptx_web.js 2>&1)
...
if [ "${PPTX_NO_MINIFY:-0}" != "1" ]; then
  node lib/evg/web/tools/minify.mjs --file "$STAGE/pptx_web.js" --keep PptxWeb || exit 1
fi
