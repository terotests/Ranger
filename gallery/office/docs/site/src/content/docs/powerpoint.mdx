...
The block below is the same playground as [`/office/`](/Ranger/office/),
embedded. The code on the left runs against `@ranger/pptx`. The picture on
the right is the `.pptx` that code produced, opened by the slide editor —
not a preview of the model in memory. Use **‹** / **›** to page the stack,
edit the code, and press **Run**.

<PptxExample preset="Several slides" title="Three slides from the published API" height="34rem" />
