...
echo "==> JavaScript"
node dist/rgrc.js -es6 "gallery/office/geom/tests/$T.rgr" -d=gallery/office/bin -o="$T.js" -nodecli > "$OUT/js.log" 2>&1 || {
...
node dist/rgrc.js -l=cpp "gallery/office/geom/tests/$T.rgr" -nodecli -d="$OUT" -o="$T.cpp" > "$OUT/cpp.log" 2>&1 || {
