...
echo "==> JavaScript"
node dist/rgrc.js -es6 "$SRC" -d=gallery/office/text/bin -o=OfficeTextTest.js -nodecli > "$OUT/js.log" 2>&1 || {
...
echo
echo "==> C++ ($CXX)"
node dist/rgrc.js -l=cpp "$SRC" -nodecli -d="$OUT" -o=OfficeTextTest.cpp > "$OUT/cpp.log" 2>&1 || {
