...
echo "==> JavaScript"
node dist/rgrc.js -es6 "$SRC" -d=gallery/office/text/bin -o=OfficeArabicTest.js -nodecli > "$OUT/js.log" 2>&1 || {
...
echo
echo "==> C++ ($CXX)"
node dist/rgrc.js -l=cpp "$SRC" -nodecli -d="$OUT" -o=OfficeArabicTest.cpp > "$OUT/cpp.log" 2>&1 || {
