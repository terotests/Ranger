...
class ImageInfo  {
  constructor() {
    ...
    this.loaded = false;
    this.error = "";
    this.orientation = 1;     /** note: unused */
    this.colorSpace = "RGB";     /** note: unused */
    this.bitsPerComponent = 8;     /** note: unused */
    this.createdAt = "";     /** note: unused */
    this.camera = "";     /** note: unused */
    this.hasGPS = false;     /** note: unused */
    this.gpsLatitude = 0.0;     /** note: unused */
    this.gpsLongitude = 0.0;     /** note: unused */
  }
}
class EVGResourceLoader  {
  constructor() {
    ...
    const mp = new JPEGMetadataParser();
    this.metadataParser = mp;
  }
  setBasePath (path) {
    this.basePath = path;
  };
  loadResources (root) {
    console.log("ResourceLoader: Starting resource loading...");
    this.processElement(root);
    console.log(("ResourceLoader: Loaded " + (((this.imageCache.length).toString()))) + " images");
  };
  processElement (el) {
    console.log((("ResourceLoader: processElement tagName=" + el.tagName) + " src=") + el.src);
    if ( ((el.tagName == "Image") || (el.tagName == "image")) || (el.tagName == "img") ) {
      if ( (el.src.length) > 0 ) {
        console.log("ResourceLoader: Found image: " + el.src);
        this.loadImageMetadata(el);
      }
    }
    if ( el.elementType == 2 ) {
      if ( (el.src.length) > 0 ) {
        console.log("ResourceLoader: Found image by elementType: " + el.src);
        this.loadImageMetadata(el);
      }
    }
    let i = 0;
  ...
  loadImageMetadata (el) {
    const imgPath = el.src;
    const cached = this.getCachedImage(imgPath);
    if ( cached.loaded ) {
      el.sourceWidth = cached.width;
      el.sourceHeight = cached.height;
      if ( this.debug ) {
        console.log(((((("ResourceLoader: Cache hit for " + imgPath) + " (") + ((cached.width.toString()))) + "x") + ((cached.height.toString()))) + ")");
      }
      return;
    }
    let dirPath = "";
    ...
    const lastBackslash = imgPath.lastIndexOf("\\");
    let lastSep = lastSlash;
    if ( lastBackslash > lastSep ) {
      lastSep = lastBackslash;
    }
    if ( lastSep >= 0 ) {
      dirPath = imgPath.substring(0, (lastSep + 1) );
      fileName = imgPath.substring((lastSep + 1), (imgPath.length) );
    } else {
      dirPath = "";
      fileName = imgPath;
    }
    ...
    if ( (imgPath.indexOf("/")) != 0 ) {
      if ( (dirPath.indexOf("./")) == 0 ) {
        if ( this.basePath == "./" ) {
          ...
          fullDirPath = this.basePath + (dirPath.substring(2, (dirPath.length) ));
        }
      } else {
        fullDirPath = this.basePath + dirPath;
      }
    }
    console.log(("ResourceLoader: Loading " + fullDirPath) + fileName);
    let meta = this.metadataParser.parseMetadata(fullDirPath, fileName);
    if ( meta.isValid == false ) {
      ...
      console.log(("ResourceLoader: Trying alternative path: " + altDirPath) + fileName);
      meta = this.metadataParser.parseMetadata(altDirPath, fileName);
    }
    if ( meta.isValid == false ) {
      console.log(((("ResourceLoader: ERROR - " + meta.errorMessage) + ": ") + fullDirPath) + fileName);
      const errInfo = new ImageInfo();
      errInfo.path = imgPath;
      errInfo.loaded = true;
      errInfo.error = meta.errorMessage;
      this.imageCache.push(errInfo);
      return;
    }
    ...
    el.sourceWidth = imgWidth;
    el.sourceHeight = imgHeight;
    if ( this.debug ) {
      console.log(((((("ResourceLoader: Loaded " + imgPath) + " (") + ((imgWidth.toString()))) + "x") + ((imgHeight.toString()))) + ")");
    }
    ...
    info.loaded = true;
    this.imageCache.push(info);
  };
  getCachedImage (path) {
    let i = 0;
    while (i < (this.imageCache.length)) {
      const info = this.imageCache[i];
      if ( info.path == path ) {
        return info;
      }
      i = i + 1;
    };
    const empty = new ImageInfo();
    return empty;
  };
  clearCache () {
    let empty = [];
...
class EVGHTMLRenderer  {
  constructor() {
    ...
    this.imageServerUrl = "";     /** note: unused */
    this.foundSections = [];
    this.foundPages = [];
    ...
    this.measurer = m_1;
    const rl = new EVGResourceLoader();
  ...
  setDebug (enabled) {
    ...
    this.debug = enabled;
    this.resourceLoader.debug = enabled;
  };
  ...
  render (root) {
    ...
    let uf = [];
    this.usedFontFamilies = uf;
    this.resourceLoader.setBasePath(this.baseDir);
    this.resourceLoader.loadResources(root);
  ...
  renderImageWithParent (el, elementId, depth, parentX, parentY) {
    ...
    const containerW = el.calculatedWidth;
    let containerH = el.calculatedHeight;
    let parentAvailableH = el.parent.calculatedHeight;
    ...
    if ( (((containerH <= 0.0) && (containerW > 0.0)) && (el.sourceWidth > 0.0)) && (el.sourceHeight > 0.0) ) {
      const imageAspect = el.sourceWidth / el.sourceHeight;
      containerH = containerW / imageAspect;
      if ( this.debug ) {
        console.log("Image: calculated containerH from aspect ratio: " + ((containerH.toString())));
      }
    }
    if ( (parentAvailableH > 0.0) && (containerH > parentAvailableH) ) {
      if ( this.debug ) {
        console.log((("Image: clipping containerH from " + ((containerH.toString()))) + " to parent height ") + ((parentAvailableH.toString())));
      }
    ...
    html = html + "\">\n";
    let imgW = el.sourceWidth;
    let imgH = el.sourceHeight;
    if ( imgW <= 0.0 ) {
      imgW = containerW;
    }
    if ( imgH <= 0.0 ) {
    ...
    if ( el.imageOffsetX.isSet ) {
      if ( el.imageOffsetX.unitType == 1 ) {
        ...
        const targetOffsetX = (overflowX * el.imageOffsetX.value) / 100.0;
    ...
    if ( el.imageOffsetY.isSet ) {
      if ( el.imageOffsetY.unitType == 1 ) {
        ...
        const targetOffsetY = (overflowY * el.imageOffsetY.value) / 100.0;
  ...
  generateImageStylesRelative (el, relX, relY) {
    ...
    if ( el.imageOffsetX.isSet ) {
      hasOffset = true;
      if ( el.imageOffsetX.unitType == 1 ) {
        const offsetPct = 50.0 + el.imageOffsetX.value;
        posX = ((offsetPct.toString())) + "%";
      } else {
    ...
    if ( el.imageOffsetY.isSet ) {
      hasOffset = true;
      if ( el.imageOffsetY.unitType == 1 ) {
        const offsetPct_1 = 50.0 + el.imageOffsetY.value;
        posY = ((offsetPct_1.toString())) + "%";
      } else {
    ...
    if ( el.imageViewBoxSet ) {
      hasOffset = true;
      const vbX = el.imageViewBoxX * 100.0;
      const vbY = el.imageViewBoxY * 100.0;
      posX = ((vbX.toString())) + "%";
      posY = ((vbY.toString())) + "%";
    }
    if ( hasOffset ) {
  ...
  renderContent (root) {
    ...
    if ( isMultiPage ) {
      ...
      if ( (this.foundSections.length) == 0 ) {
        this.layout.layout(root);
      }
    } else {
      this.layout.layout(root);
  ...
  transformImagePath (imgSrc) {
    if ( (imgSrc.length) >= 10 ) {
      const prefix = imgSrc.substring(0, 10 );
      if ( prefix == "../assets/" ) {
        const relativePath = imgSrc.substring(10, (imgSrc.length) );
  ...
  generateShellHTML (serverUrl) {
    ...
    html = ((html + "            min-height: ") + (((Math.floor( this.pageHeight)).toString()))) + "px;\n";
    ...
    html = html + "            width: 100%;\n";
    html = ((html + "            max-width: ") + (((Math.floor( ((this.pageWidth * 2.0) + 20.0))).toString()))) + "px;\n";
    html = html + "        }\n";
    ...
    html = html + "        }\n";
    html = html + "        \n";
    html = html + "        // Export JPEG function for individual pages\n";
    html = html + "        async function exportJPEG(pageNum, scale = 2) {\n";
    html = html + "            const btn = document.querySelector('#evg-export-jpeg-' + pageNum);\n";
    html = html + "            if (btn) {\n";
    html = html + "                btn.disabled = true;\n";
    html = html + "                btn.textContent = '⏳';\n";
    html = html + "            }\n";
    html = html + "            statusEl.textContent = 'Generating JPEG for page ' + pageNum + '...';\n";
    html = html + "            statusEl.style.color = '#ff0';\n";
    html = html + "            try {\n";
    html = html + "                const response = await fetch(serverUrl + '/export-png?page=' + pageNum + '&scale=' + scale);\n";
    html = html + "                if (!response.ok) throw new Error('JPEG generation failed');\n";
    html = html + "                const blob = await response.blob();\n";
    html = html + "                const url = window.URL.createObjectURL(blob);\n";
    html = html + "                const a = document.createElement('a');\n";
    html = html + "                a.href = url;\n";
    html = html + "                const filename = response.headers.get('Content-Disposition')?.match(/filename=\\\"(.+)\\\"/)?.[1] || ('page' + pageNum + '.jpg');\n";
    html = html + "                a.download = filename;\n";
    html = html + "                document.body.appendChild(a);\n";
    html = html + "                a.click();\n";
    html = html + "                window.URL.revokeObjectURL(url);\n";
    html = html + "                a.remove();\n";
    html = html + "                statusEl.textContent = 'JPEG exported!';\n";
    html = html + "                statusEl.style.color = '#0f0';\n";
    html = html + "            } catch (err) {\n";
    html = html + "                statusEl.textContent = 'JPEG export failed: ' + err.message;\n";
    html = html + "                statusEl.style.color = '#f00';\n";
    html = html + "            }\n";
    html = html + "            if (btn) {\n";
    html = html + "                btn.disabled = false;\n";
    html = html + "                btn.textContent = '🖼️ JPEG';\n";
    html = html + "            }\n";
    html = html + "        }\n";
    html = html + "        \n";
    html = html + "        // Helper to add export JPEG button to a page element\n";
    html = html + "        function addExportJPEGButton(pageElement, pageNum) {\n";
    ...
    html = html + "            btn.id = 'evg-export-jpeg-' + pageNum;\n";
    html = html + "            btn.textContent = '🖼️ JPEG';\n";
    html = html + "            btn.title = 'Export page ' + pageNum + ' as JPEG';\n";
    html = html + "            btn.onclick = function() { exportJPEG(pageNum); };\n";
    ...
    html = html + "                addExportJPEGButton(pages[0], 1);\n";
    html = html + "                return;\n";
    html = html + "            }\n";
    ...
    html = html + "            addExportJPEGButton(cover, 1);\n";
    ...
    html = html + "                    \n";
    html = html + "                    addExportJPEGButton(pageClones[i], i + 1);\n";
    html = html + "                    addExportJPEGButton(pageClones[i + 1], i + 2);\n";
    ...
    html = html + "                addExportJPEGButton(backCover, totalPages);\n";
