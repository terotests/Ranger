...
class EVGStyleSheet  {
  ...
  toText () {
    ...
    for ( let i = 0; i < this.rules.length; i++) {
      ...
      out = out + (line + "\n");
      for ( let j = 0; j < r.decls.length; j++) {
        var d = r.decls[j];
...
class EVGDisplayList  {
  ...
  walkOpaque (el) {
    ...
    const paintsItsBox = el.tagName != "path";
...
class EVGPatch  {
  ...
  doInsert (op) {
    ...
    const parent = hit;
    const made = EVGPatch.create(op.tag);
...
EVGDocSource.isTreeJson = function(name) {
  return EVGDocSource.hasTail(name, ".evg.json");
};
EVGDocSource.hasTail = function(s, tail) {
  const n = tail.length;
  if ( s.length < n ) {
    return false;
  }
  return s.substring((s.length - n), s.length ) == tail;
};
/**
 * Reads a document as an EVG tree, choosing the reader by extension.
 * @param {string} dir - The directory the file is in, with or without a trailing slash.
 * @param {string} name - The file name.
 * @param {number} pageWidth - Page width, for the JSX reader's own sizing.
 * @param {number} pageHeight - Page height, likewise.
 * @returns {EVGElement} The root element; a tagName of "" means it could not be read.
 * @private
 */
EVGDocSource.load = async function(dir, name, pageWidth, pageHeight) {
  if ( EVGDocSource.isTreeJson(name) ) {
    const text = await (new Promise(resolve => { require('fs').readFile(
      dir + '/' + name,
      'utf8',
    ...
    const root = EVGTreeJson.fromText(text);
...
EVGDocSource.isAnchored = function(path) {
  if ( path.length == 0 ) {
    return true;
  }
...
EVGDocSource.split = function(path) {
  let out = [];
  let cur = "";
  let i = 0;
  while (i < path.length) {
...
EVGDocSource.collapse = function(parts) {
  let out = [];
  let i = 0;
  while (i < parts.length) {
    const seg = parts[i];
...
EVGDocSource.joinParts = function(parts, absolute) {
  let out = "";
  if ( absolute ) {
    out = "/";
  }
...
EVGDocSource.rebase = function(fromDir, toDir, rel) {
  if ( EVGDocSource.isAnchored(rel) ) {
    return rel;
  }
  const fromAbs = EVGDocSource.startsAt(fromDir);
  const toAbs = EVGDocSource.startsAt(toDir);
  if ( fromAbs != toAbs ) {
    return "";
  }
  const target = EVGDocSource.collapse(EVGDocSource.split(((fromDir + "/") + rel)));
  const base = EVGDocSource.collapse(EVGDocSource.split(toDir));
  ...
  while (common < base.length) {
    if ( common >= target.length ) {
      return EVGDocSource.walkUp((base.length - common), target, common);
    }
    const a = base[common];
    const b = target[common];
    if ( a != b ) {
      return EVGDocSource.walkUp((base.length - common), target, common);
    }
    common = common + 1;
...
EVGDocSource.walkUp = function(up, target, from) {
  let out = "";
  let i = 0;
  while (i < up) {
    out = out + "../";
...
EVGDocSource.hasDotDot = function(parts) {
  let i = 0;
  while (i < parts.length) {
    if ( parts[i] == ".." ) {
      return true;
...
EVGDocSource.startsAt = function(dir) {
  if ( dir.length == 0 ) {
    return false;
  }
  return dir.charCodeAt(0 ) == 47;
};
EVGDocSource.nothing = function() {
  const el = new EVGElement();
  el.tagName = "";
  return el;
};
EVGDocSource.kindOf = function(name) {
  if ( EVGDocSource.isTreeJson(name) ) {
    return "EVG document";
  }
  return "TSX";
...
class EVGJsonTool  {
  ...
  async run () {
    ...
    while (i < argCount) {
      ...
      if ( arg == "-w" ) {
        if ( i + 1 < argCount ) {
          const w = isNaN( parseFloat(process.argv[ 2 + (i + 1)]) ) ? undefined : parseFloat(process.argv[ 2 + (i + 1)]);
          if ( typeof(w) != "undefined" ) {
            this.pageWidth = w;
          }
          i = i + 1;
      ...
      if ( arg == "-h" ) {
        if ( i + 1 < argCount ) {
          const h = isNaN( parseFloat(process.argv[ 2 + (i + 1)]) ) ? undefined : parseFloat(process.argv[ 2 + (i + 1)]);
      ...
      if ( arg == "-css" ) {
        if ( i + 1 < argCount ) {
          this.styles.addFile(process.argv[ 2 + (i + 1)]);
          i = i + 1;
        }
      }
      if ( arg == "-theme" ) {
        if ( i + 1 < argCount ) {
          this.styles.setTheme(process.argv[ 2 + (i + 1)]);
          i = i + 1;
        }
      }
      i = i + 1;
  ...
  printUsage () {
    console.log("evg-json — convert a document to the editable EVG format");
    console.log("");
    console.log("  evg-json <in.tsx|in.evg.json> <out.evg.json> [options]");
    console.log("");
    console.log("  -css <file>    stylesheet to resolve into the output");
    console.log("  -theme <name>  theme to select from the stylesheet");
    console.log("  -w <n> -h <n>  page size for the reader");
    console.log("  -strict        fail instead of warning when something is dropped");
  };
  async convert () {
    const cut = this.inputFile.lastIndexOf("/");
    let dir = "./";
    let name = this.inputFile;
    if ( cut >= 0 ) {
      dir = this.inputFile.substring(0, (cut + 1) );
      name = this.inputFile.substring((cut + 1), this.inputFile.length );
    }
    console.log(("Reading the " + EVGDocSource.kindOf(name)) + "…");
    const root = await EVGDocSource.load(
      dir,
      name,
      this.pageWidth,
      this.pageHeight
    ...
    if ( root.tagName == "" ) {
      console.log(("Error: could not read " + this.inputFile) + " as a document");
      return;
    }
    ...
    let found = [];
    this.losses = found;
    const outCutEarly = this.outputFile.lastIndexOf("/");
    let outDirEarly = ".";
    if ( outCutEarly >= 0 ) {
      outDirEarly = this.outputFile.substring(0, outCutEarly );
    }
    this.rebaseAssets(root, "0", dir, outDirEarly);
    this.audit(root, "0");
    if ( this.losses.length > 0 ) {
      console.log("");
      console.log((this.losses.length.toString()) + " thing(s) the editable format has no place for:");
      let i = 0;
      while (i < this.losses.length) {
        console.log("  " + this.losses[i]);
        i = i + 1;
      };
      if ( this.strict ) {
        console.log("");
        console.log("Refusing to write a lossy conversion (-strict).");
        return;
      }
      console.log("");
      console.log("Writing anyway — pass -strict to make this a failure.");
    }
    const text = EVGTreeJson.toText(root);
    this.proveItLandedTheSame(root, text, dir);
    const outCut = this.outputFile.lastIndexOf("/");
    let outDir = ".";
    let outName = this.outputFile;
    if ( outCut >= 0 ) {
      outDir = this.outputFile.substring(0, outCut );
      outName = this.outputFile.substring((outCut + 1), this.outputFile.length );
    }
    require("fs").writeFileSync( outDir + "/"  + outName, text);
    console.log("");
    console.log("Wrote " + this.outputFile);
    console.log(("  " + (text.length.toString())) + " bytes");
  };
  proveItLandedTheSame (root, text, dir) {
    const again = EVGTreeJson.fromText(text);
    if ( typeof(again) === "undefined" ) {
      this.losses.push("the document could not be read back at all");
      return;
    }
    const copy = again;
    const setup = new EVGFontSetup();
    setup.init(dir, "");
    const a = new EVGLayout();
    a.setMeasurer(new TTFTextMeasurer(setup.getManager()));
    a.setPageSize(this.pageWidth, this.pageHeight);
    a.layout(root);
    const b = new EVGLayout();
    b.setMeasurer(new TTFTextMeasurer(setup.getManager()));
    b.setPageSize(this.pageWidth, this.pageHeight);
    b.layout(copy);
    this.compare(root, copy, "0");
    this.comparePainting(root, copy);
  };
  comparePainting (a, b) {
    const da = new EVGDisplayList();
    da.build(a);
    const db = new EVGDisplayList();
    db.build(b);
    const na = da.cmds.length;
    const nb = db.cmds.length;
    if ( na != nb ) {
      this.losses.push((("the page draws " + (na.toString())) + " things and the converted one draws ") + (nb.toString()));
      return;
    }
    const ja = da.toJson();
    const jb = db.toJson();
    ...
    const at = EVGJsonTool.firstDifference(ja, jb);
    this.losses.push("the same boxes, drawn differently:");
    this.losses.push("    was  " + EVGJsonTool.window(ja, at));
    this.losses.push("    now  " + EVGJsonTool.window(jb, at));
  };
  compare (a, b, path) {
    if ( a.children.length != b.children.length ) {
      this.losses.push((((path + ": ") + (a.children.length.toString())) + " children became ") + (b.children.length.toString()));
      return;
    }
    const dx = a.calculatedX - b.calculatedX;
    const dy = a.calculatedY - b.calculatedY;
    const dw = a.calculatedWidth - b.calculatedWidth;
    const dh = a.calculatedHeight - b.calculatedHeight;
    if ( EVGJsonTool.off(dx, dy, dw, dh) ) {
      this.losses.push((((path + ": the box moved — ") + EVGJsonTool.box(
        a.calculatedX,
        a.calculatedY,
        a.calculatedWidth,
        a.calculatedHeight
      )) + " became ") + EVGJsonTool.box(b.calculatedX, b.calculatedY, b.calculatedWidth, b.calculatedHeight));
    }
    let i = 0;
    while (i < a.children.length) {
      const ka = a.children[i];
      const kb = b.children[i];
      let seg = (i.toString());
      if ( ka.key.length > 0 ) {
        seg = "k:" + ka.key;
      }
  ...
  rebaseAssets (el, path, fromDir, toDir) {
    if ( el.src.length > 0 ) {
      const moved = EVGDocSource.rebase(fromDir, toDir, el.src);
      if ( moved.length == 0 ) {
        this.losses.push((((path + ": the image path `") + el.src) + "` cannot be re-anchored from ") + ((fromDir + " to ") + toDir));
      } else {
        el.src = moved;
      }
    }
  ...
  audit (el, path) {
    const made = EVGPatch.create(el.tagName);
    if ( typeof(made) === "undefined" ) {
      this.losses.push(((path + ": <") + el.tagName) + "> has no equivalent — it would be read back as something else");
    }
  ...
  dropped (path, name, value) {
    ...
    this.losses.push(((path + ": `") + name) + "` has no place in the editable format");
  };
}
EVGJsonTool.firstDifference = function(a, b) {
  let n = a.length;
  if ( b.length < n ) {
    n = b.length;
  }
...
EVGJsonTool.window = function(s, at) {
  let from = at - 60;
  if ( from < 0 ) {
    from = 0;
  }
...
EVGJsonTool.off = function(dx, dy, dw, dh) {
...
EVGJsonTool.abs = function(v) {
  if ( v < 0.0 ) {
    return 0.0 - v;
  }
  return v;
};
EVGJsonTool.box = function(x, y, w, h) {
  let out = (Math.floor( x).toString());
  out = (out + ",") + (Math.floor( y).toString());
  out = (out + " ") + (Math.floor( w).toString());
  out = (out + "x") + (Math.floor( h).toString());
  return out;
};
/* static JavaSript main routine at the end of the JS file */
async function __js_main() {
  const t = new EVGJsonTool();
  await t.run();
}
__js_main();
