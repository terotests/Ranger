...
func CreateNew_EVGTool() *EVGTool {
  me := new(EVGTool)
  me.inputFile = ""
  me.outputFile = ""
  me.pageWidth = 595.0
  ...
  me.serverPort = int64(3000)
  me.watchMode = false
  me.VERSION = "1.0.0"
  return me;
}
func (this *EVGTool) run () () {
  this.parseArgs();
  if  this.showHelp {
    this.printHelp();
    return
  ...
  if  this.showVersion {
    fmt.Println( "evg_tool version " + this.VERSION )
    return
  }
  ...
  if  (int64(len([]rune(this.outputFile)))) == int64(0) {
    this.outputFile = this.replaceExtension(this.inputFile, ".html"); 
  }
  fmt.Println( "EVG Tool v1.0.0 - HTML Preview" )
  fmt.Println( "==============================" )
...
func (this *EVGTool) parseArgs () () {
  ...
  for i < argCount {
    ...
    if  (int64(strings.Index(arg, "-a="))) == int64(0) {
      this.assetPaths = string([]rune(arg)[int64(3):(int64(len([]rune(arg))))]); 
      this.useComponents = true; 
      i = i + int64(1); 
      continue;
    }
    if  (int64(strings.Index(arg, "--title="))) == int64(0) {
      this.title = string([]rune(arg)[int64(8):(int64(len([]rune(arg))))]); 
      i = i + int64(1); 
      continue;
    }
    if  (int64(strings.Index(arg, "-t="))) == int64(0) {
      this.title = string([]rune(arg)[int64(3):(int64(len([]rune(arg))))]); 
      i = i + int64(1); 
      continue;
    }
    if  (int64(strings.Index(arg, "--port="))) == int64(0) {
      var pVal *GoNullable = new(GoNullable); 
      pVal = r_str_2_i64((string([]rune(arg)[int64(7):(int64(len([]rune(arg))))])));
      if ( pVal.has_value) {
        this.serverPort = pVal.value.(int64); 
      }
      i = i + int64(1); 
      continue;
    }
    if  (int64(strings.Index(arg, "-p="))) == int64(0) {
      var pVal_1 *GoNullable = new(GoNullable); 
      pVal_1 = r_str_2_i64((string([]rune(arg)[int64(3):(int64(len([]rune(arg))))])));
      if ( pVal_1.has_value) {
        this.serverPort = pVal_1.value.(int64); 
      }
      i = i + int64(1); 
      continue;
    }
    if  (int64(strings.Index(arg, "--width="))) == int64(0) {
      var wVal *GoNullable = new(GoNullable); 
      wVal = r_str_2_d64((string([]rune(arg)[int64(8):(int64(len([]rune(arg))))])));
      if ( wVal.has_value) {
    ...
    if  (int64(strings.Index(arg, "--height="))) == int64(0) {
      var hVal *GoNullable = new(GoNullable); 
      hVal = r_str_2_d64((string([]rune(arg)[int64(9):(int64(len([]rune(arg))))])));
      if ( hVal.has_value) {
...
func (this *EVGTool) replaceExtension (filename string, newExt string) string {
  var lastDot int64= int64(strings.LastIndex(filename, "."));
  if  lastDot >= int64(0) {
    return (string([]rune(filename)[int64(0):lastDot])) + newExt
  }
  return filename + newExt
}
func (this *EVGTool) getDirectory (path string) string {
  var lastSlash int64= int64(strings.LastIndex(path, "/"));
  var lastBackslash int64= int64(strings.LastIndex(path, "\\"));
  var lastSep int64= lastSlash;
  if  lastBackslash > lastSep {
    lastSep = lastBackslash; 
  }
  if  lastSep >= int64(0) {
    return string([]rune(path)[int64(0):(lastSep + int64(1))])
  }
  return "./"
}
func (this *EVGTool) getFileName (path string) string {
  var lastSlash int64= int64(strings.LastIndex(path, "/"));
  var lastBackslash int64= int64(strings.LastIndex(path, "\\"));
  var lastSep int64= lastSlash;
  if  lastBackslash > lastSep {
  ...
  if  lastSep >= int64(0) {
    return string([]rune(path)[(lastSep + int64(1)):(int64(len([]rune(path))))])
  }
  return path
}
func (this *EVGTool) convertToHTML () () {
  ...
  var outputDir string= this.getDirectory(this.outputFile);
  var outputFileName string= this.getFileName(this.outputFile);
...
func (this *EVGTool) printTree (element *EVGElement, depth int64) () {
  var indent string= "";
  var i int64= int64(0);
  for i < depth {
    indent = indent + "  "; 
