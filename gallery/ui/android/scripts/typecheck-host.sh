...
#
#   bash gallery/ui/android/scripts/typecheck-host.sh
#
# `MainActivity.kt` and `DashboardView.kt` — and the shared
...
OUT=tmp/ui-android/typecheck

if ! command -v kotlinc >/dev/null 2>&1; then
  echo "kotlinc is not on PATH — see https://kotlinlang.org/docs/command-line.html" >&2
  exit 1
fi
if [ ! -f "$AND/generated/ui_android.kt" ]; then
  echo "generated/ui_android.kt is missing — run scripts/build-ranger.sh first" >&2
  exit 1
fi
