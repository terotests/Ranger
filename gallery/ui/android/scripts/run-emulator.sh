#!/usr/bin/env bash
# Build the dashboard, put it on an emulator (starting one if none is running),
# and open it.
#
#   npm run ui:android:run
#   npm run ui:android:run -- --avd Pixel_Tablet_API_34
#   npm run ui:android:run -- --logcat
#   npm run ui:android:run -- --no-build          # reinstall what is already built
#
...
#
# The page is a 1336-wide document that is scaled to fit whatever it lands on,
# so a phone works and a TABLET is what it was drawn for: at `Pixel Tablet` in
# landscape the scale is very nearly 1:1 and the dashboard is the size it is in
# a browser. Pinch to zoom in, double-tap to go back to the whole width.
#
