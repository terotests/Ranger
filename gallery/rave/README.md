...
| `src/RaveKit.rgr` | The Components pane: Container, Section, Heading, Text, Link, Image, Navigation, Form, List as node trees, and Button, Input, Card, Select, Tabs, Dialog, Toggle, Sheet, Command, Sidebar as *instances* the runtime binds |
| `src/RaveBind.rgr` | Kit instances bound to `gallery/ui`'s controllers at run time — Dialog, Tabs, Select, Input, Toggle, Button, Sheet, Card, Command, Sidebar — one binding per viewport, their `UiRow`s written onto the elements so the a11y tree, the focus walk and the lint see them |
...
| `tests/RaveTest.rgr` | 503 checks: the pattern, the runtime, the guard, the keyboard, three widths, and the editor stage by stage |
...
- **The kit runs.** Button, Input, Card, Select, Tabs, Dialog, Sheet, Toggle,
  Command and Sidebar are `gallery/ui` controllers at run time (`RaveBind`):
  a dialog or a sheet opens on its trigger with the document's children as
  its body, traps the focus and closes on Escape; tabs switch; a select
  opens and picks; the command menu filters as you type; an input takes
  typing; a button's variant is a class and its press runs the node's
  actions. The five that `gallery/ui` did not have — Button, Sheet, Card,
  Command, Sidebar — were added there with conformance specs measured
  against the browser (cmdk for the command menu, Radix Dialog for the
  sheet, the platform's `<button>`, and shadcn's markup for the two that
  have no library). Each viewport carries its own instances — three viewports are
