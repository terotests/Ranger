...
| **LLVM / WASM** (`-l=llvm`) | **Experimental.** Lowers to LLVM IR; optional WAT export for freestanding WASM. Native libc builds work for demos like Space Invaders; browser WASM is still rough. Another codegen path from the same sources, not a separate surface language. The compiler itself compiles for LLVM with **no errors**, and the 22 MB of IR it emits for itself passes `opt -passes=verify` and links into a native `rangerc` that runs the whole pipeline — it compiles small programs to JavaScript byte-identically to the Node build. Pointed at the compiler's own sources it still stops in the writer, so LLVM is not (yet) a self-hosting target the way the six above are. See [`TARGET_NOTES.md`](TARGET_NOTES.md#the-compiler-on-llvm-how-far-it-gets), `npm run test:llvm`, `npm run selfhost:check:llvm`, `npm run game:build:llvm`. |
