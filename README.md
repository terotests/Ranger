...
| Self-hosting | Actively used. JavaScript is the reference; the compiler also compiles **itself for C++, Dart and Python** — each of those builds accepts the result and compiles the compiler again, Dart and Python byte-for-byte identically ([`TARGET_NOTES.md`](TARGET_NOTES.md#cross-target-gate-the-compiler-itself-on-c-dart-and-python)) |
