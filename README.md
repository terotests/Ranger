...
- [`INVENTORY.md`](INVENTORY.md) — every component in `gallery/`, `lib/` and `pkg/`: which are **packages** with a `ranger.json`, which are still reached by counting `../`, which implementations live inside a demo rather than a directory of their own, and where the same code exists twice. `npm run inventory` regenerates the numbers
