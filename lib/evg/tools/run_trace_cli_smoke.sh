...
mkdir -p "$OUT"
SAMPLE="lib/evg/web/tracer/sample.png"
ARGS=(--preset broken)
status=0
...
node lib/evg/bin/evg_trace_cli.js "$SAMPLE" "$OUT/node.svg" "${ARGS[@]}" | sed 's/^/  /'
...
if command -v python3 >/dev/null 2>&1; then
  if compile python evg_trace_cli.py; then
    python3 lib/evg/bin/evg_trace_cli.py "$SAMPLE" "$OUT/python.svg" "${ARGS[@]}" | sed 's/^/  /'
...
if command -v g++ >/dev/null 2>&1; then
  if compile cpp evg_trace_cli.cpp; then
    if g++ -O2 -std=c++17 -o lib/evg/bin/evg_trace_cli \
         lib/evg/bin/evg_trace_cli.cpp 2>"$OUT/gxx.log"; then
      ./lib/evg/bin/evg_trace_cli "$SAMPLE" "$OUT/cpp.svg" "${ARGS[@]}" | sed 's/^/  /'
...
if command -v rustc >/dev/null 2>&1; then
  if compile rust evg_trace_cli.rs; then
    if rustc --edition 2021 -O -o lib/evg/bin/evg_trace_cli_rust \
         lib/evg/bin/evg_trace_cli.rs 2>"$OUT/rustc.log"; then
      ./lib/evg/bin/evg_trace_cli_rust "$SAMPLE" "$OUT/rust.svg" "${ARGS[@]}" | sed 's/^/  /'
...
for form in "SRC --colorCount 4 DST" "SRC DST --colorCount 4" "--colorCount 4 SRC DST" "SRC DST --colorCount=4"; do
  ...
  args=${args//DST/$out}
  rm -f "$out"
  if ! node lib/evg/bin/evg_trace_cli.js $args >/dev/null 2>&1 || [ ! -s "$out" ]; then
...
echo "### bad input"
if node lib/evg/bin/evg_trace_cli.js "$OUT/no-such-file.png" "$OUT/x.svg" 2>&1 | grep -q "no such file"; then
