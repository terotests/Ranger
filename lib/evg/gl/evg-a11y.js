 ...
 */
export function createA11yMirror(host, { canvas, onActivate, onFocus, canMoveFocus, scale = 1, label = "Application", tabbable = "roving" } = {}) {
  ...
  // The tab stop held open while the app names no focus — see `update`.
  let entryStop = null;
  ...
  function update(tree) {
    ...
    if (focused) {
      ...
      // `canMoveFocus` is how a host that parks the keyboard somewhere else —
      // a text-input bridge editing in its own transparent <input> — says so.
      // The tab stop still moves; only the `.focus()` is withheld, because two
      // elements calling `.focus()` for one field take it off each other.
      if (document.activeElement !== focused.el && (!canMoveFocus || canMoveFocus(focused.node))) {
        settingFocus = true;
        try {
    ...
    // THE WAY IN. An app that has not been touched yet has nothing focused,
    // and a roving tabindex read literally then makes NO element tabbable —
    // so Tab walks straight past the whole widget and there is no way to reach
    // it from the keyboard at all. That is not the roving pattern; a roving
    // pattern always has one tab stop, and before anything is chosen it is the
    // first focusable node. Once the app names a focus this does nothing.
    if (tabbable !== "all") {
      if (!focused) {
        if (entryStop && (!entryStop.isConnected || entryStop.tabIndex !== 0)) entryStop = null;
        if (!entryStop) {
          for (const node of tree.nodes) {
            if (!node.focusable || node.disabled) continue;
            const entry = els.get(node.id);
            if (!entry) continue;
            entry.el.tabIndex = 0;
            entryStop = entry.el;
        ...
        if (entryStop !== focused.el && entryStop.isConnected) entryStop.tabIndex = -1;
        entryStop = null;
      }
    }
