...
registerSurfaceEffect({
  ...
  params: {
    density: 1.2, rise: 0.05, churn: 1, wind: 0, swirl: 2.6, scale: 190,
    detail: 5, height: 0.9, softness: 0.55, shade: 0.6, angle: -60,
...
//
// AND EACH OCTAVE DRIFTS ON ITS OWN, which is what makes this smoke change
// SHAPE rather than slide past. A field that only translates is a photograph
// being dragged across the box: the same billow arrives at the top that left
// the bottom, and the eye reads it as a texture on a conveyor. Here octave k
// is offset along a direction of its own, by an amount that GROWS with k — so
// in units of its own features the fine detail moves several times faster than
// the big shapes. That is also how turbulence goes: the large eddies hold
// their form while the small ones tear and re-form inside them, and the result
// is smoke that boils while it rises.
//
// The directions are a golden-angle fan, so no two octaves ever line up and
// the whole stack never cycles back to where it was.
float smFbm(vec2 p, float ch) {
  float sum = 0.0;
  float amp = 0.5;
  ...
  for (int i = 0; i < 6; i++) {
    if (i >= n) break;
    float k = float(i);
    vec2 dir = vec2(cos(k * 2.3999), sin(k * 2.3999));
    sum += amp * smNoise(p + dir * ch * (0.35 + 0.95 * k));
...
// source would breathe rather than roll — and each of the three lookups is
// given a rate of its own off the churn: the warps are stirred faster than the
// field they bend, so the billows change while they are carried.
//
// EVERY clock in here is the churn's, and the rise is not in it at all: the
// rise moves the point this is asked about, so at churn 0 the whole field is
// one rigid translation and nothing else. That is what makes the two
// behaviours separable, and it is checked by searching for the shift that
// lines two frames up: at churn 0 one exists, with the churn on none does.
float smField(vec2 q, float ch) {
  vec2 s = vec2(p_seed * 13.7, p_seed * 7.3);
  vec2 w1 = vec2(smFbm(q + s, ch * 1.6), smFbm(q + s + vec2(5.2, 1.3), ch * 1.35));
  vec2 w2 = vec2(
    smFbm(q + 2.0 * w1 + s + vec2(1.7, 9.2) - vec2(0.0, ch * 0.6), ch * 2.1),
    smFbm(q + 2.0 * w1 + s + vec2(8.3, 2.8), ch * 1.85));
  return smFbm(q + p_swirl * w2, ch);
}
...
vec4 fxColor(vec2 p, vec2 local) {
  ...
  // RISING. Page y counts downward, so the field is sampled FURTHER DOWN as
  // the clock runs: what was at the middle is then read at the floor, which
  // puts the smoke that was at the floor at the middle. The other sign SINKS,
  // and that is what a NEGATIVE rise is for — smoke pouring down a box is a
  // picture somebody wants. It is also what this drew by accident for an
  // afternoon, because the motion was slow enough that nobody watched one
  // billow to see where it went.
  q.y += t;

  // How fast the shapes reorganise, against how fast they travel. At 0 this is
  // a rigid field sliding; at 1 a billow is visibly a different billow a second
  // later. The two are separate on purpose: smoke that crosses the box quickly
  // and smoke that boils where it stands are different pictures, and a page
  // that wants either should not have to take the other.
  float ch = uTime * p_churn * 0.22;
  float f = smField(q, ch);
  ...
  if (p_shade > 0.0) {
    ...
    float g = smField(q + dir * 0.22, ch) - f;
