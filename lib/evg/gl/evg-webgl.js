...
//   sweep-speed  crossings per second; 0 holds it still at `sweep-at`
...
registerSurfaceEffect({
  ...
  params: {
    ...
    sweep: 0, "sweep-angle": -62, "sweep-width": 0.1,
    "sweep-speed": 0, "sweep-at": 0.5, "sweep-edge": 1.6,
  },
  frag: `
vec4 fxColor(vec2 p, vec2 local) {
  ...
  if (p_sweep > 0.0) {
    ...
    // Travelling from before one edge to past the other, so the bar arrives
    // and leaves rather than appearing in the middle: -0.2 to 1.2.
    float at = p_sweep_speed > 0.0
      ? fract(uTime * p_sweep_speed) * 1.4 - 0.2
      : p_sweep_at;
    ...
    col += vec3(bar * 0.55 + core * 0.9) * p_sweep;
