...
export function registerSurfaceEffect(def) {
  ...
  const layer = def.layer === "filter" ? "filter" : "source";
...
// How much of this pixel is inside the box, corners included: the rounded
// rectangle's signed distance, softened over a pixel so a corner is not a
// staircase. Applied in main, so no plugin can paint outside the element that
// declared it — which is the whole difference between an effect on a card and
// an effect on a page.
...
// A filter REPLACES the surface, so blending is off and the pixels outside the
// box have to be handed back exactly as they arrived. A mix and not a branch:
// a branch here would make the screen-space derivative a plugin like the
// ripple takes undefined along the box's own edge.
...
function fxFragSource(def) {
  ...
  return "#version 300 es\n" + FX_PREAMBLE + decls + "\n" + def.frag +
    (def.layer === "filter" ? FX_MAIN_FILTER : FX_MAIN_SOURCE);
}
...
vec3 fxStarLayer(vec2 q, float cell, float seed, float twinkle, float glow) {
  ...
  for (int dy = -1; dy <= 1; dy++) {
    for (int dx = -1; dx <= 1; dx++) {
      ...
      float halo = glow * 0.30 * exp(-d / (3.0 * rad));
      float spike = 0.0;
      if (bright > 0.55) {
        vec2 v = (f - at) * cell;
      ...
      acc += tint * bright * (core + halo + spike);
    }
  }
  return acc;
}
...
function buildFrame(gl, doc, opts = {}) {
  ...
  const fxLive = (inst) => (inst.events && inst.events.length > 0) ||
  ...
  let target = (rippling || fxPasses.length > 0)
  ...
  const drawFxPass = (inst, srcTex, shift) => {
  ...
  for (const run of runs) {
    ...
    // element contains.
    if (run.kind === "fx") { drawFxPass(run.inst, null, curShift); continue; }
