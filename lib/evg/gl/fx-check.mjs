...
/** The preset blocks, off disk. Read here rather than imported from
 *  `effect-shots.mjs`: that file is a SCRIPT, and importing it rendered its
 *  whole contact sheet in a browser before this check had run a line — which
 *  went unnoticed until there were enough presets for it to time out. */
const readPresets = () => parsePresets(
  fs.readFileSync(path.join(HERE, "effect-presets.css"), "utf8"));
const W = 320;
const H = 220;
...
// The same smoke twice, four seconds apart, with the shapes frozen and with
// them churning. `scale: 90` and `rise: 0.05` put the frozen pair exactly 18
// page pixels apart, which is what the shift search below has to find — and
// `height: 8` fills the box, because the bar that rises with height is not
// part of the field and would not travel with it, so a low bank is never a
// translation of itself however rigid the smoke in it is.
const travel = { churn: 0, rise: 0.05, height: 8 };
SCENES.smokeRigidA = smoke(travel);
SCENES.smokeRigidB = smoke({ ...travel, __t: 4 });
SCENES.smokeLiveA = smoke({ ...travel, churn: 1 });
SCENES.smokeLiveB = smoke({ ...travel, churn: 1, __t: 4 });
...
console.log("-- smoke banks up along the floor of its box");
{
  ...
  // AND IT CHANGES SHAPE, which is a different claim from moving and the one
  // that separates smoke from a texture on a conveyor belt. Asked by SEARCHING
  // FOR THE SHIFT: if the field only travels, then one vertical offset lines
  // the later frame up with the earlier one, and with `churn: 0` that offset
  // exists and the two frames agree along it. With the churn on, no offset
  // does — the billows the later frame holds were not in the earlier one at
  // any height.
  const best = (a2, b2) => {
    let bestErr = 1e9, bestDy = 0;
    for (let dy = -30; dy <= 30; dy += 1) {
      let sum = 0, n = 0;
      for (let y = BOX[1] + 34; y < BOX[1] + BOX[3] - 34; y += 2) {
        for (let x = BOX[0] + 6; x < BOX[0] + BOX[2] - 6; x += 2) {
          sum += Math.abs(lum(at(a2.px, x, y)) - lum(at(b2.px, x, y + dy)));
          n += 1;
        }
      }
      const err = sum / Math.max(n, 1);
      if (err < bestErr) { bestErr = err; bestDy = dy; }
    }
    return { err: bestErr, dy: bestDy };
  };
  const rigid = best(await frameOf(SCENES.smokeRigidA), await frameOf(SCENES.smokeRigidB));
  const live = best(await frameOf(SCENES.smokeLiveA), await frameOf(SCENES.smokeLiveB));
  say(rigid.err < 6, `with churn off the whole frame is one shift away (${rigid.err.toFixed(1)} at ${rigid.dy}px)`,
    "even the frozen field does not line up with itself: `rise` is not a straight translation any more");
  // AND THE SHIFT IS UPWARD, by the distance `rise` asks for. The later frame
  // matched against the earlier one at a NEGATIVE offset is smoke that went
  // up; the other sign is smoke that sinks, which is what this drew for an
  // afternoon because the motion was slow enough that nobody looked.
  say(rigid.dy <= -14 && rigid.dy >= -22, `and it is 18 pixels UP (${rigid.dy})`,
    `four seconds of rise: 0.05 at scale 90 is 18 pixels of smoke, upward — this went ${rigid.dy}`);
  say(live.err > rigid.err * 2.5,
    `and with it on no shift explains it (${live.err.toFixed(1)} at best — ${(live.err / Math.max(rigid.err, 0.01)).toFixed(1)}x)`,
    "the churning smoke lines up with itself under a shift, so it is travelling rather than boiling");
}
