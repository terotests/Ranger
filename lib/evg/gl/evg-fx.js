 ...
 *
 *  An instance the host has switched off (`inst.off`) answers no to everything,
 *  so a press on it lands on whatever is under it rather than being swallowed
 *  by an effect that is not running. The painter skips the same flag, and
 *  between them "off" means off rather than "invisible but still listening". */
export function triggerHas(inst, what) {
  if (!inst || inst.off) return false;
  const on = inst.on || "";
