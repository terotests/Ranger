 ...
 *
 *   node lib/evg/gl/view-check.mjs [--write-png prefix]
 *
...
try {
  ...
  // SwiftShader has been seen to answer readPixels before the first
  // draw is in the buffer. finish() is a fence; without it the next
  // navigation can capture a blank or half-drawn canvas.
...
async function pixelsOf(cmds, view, png) {
  ...
  // Same path every time. Chromium will sometimes answer the previous
  // document for a file:// URL it already loaded, and waitForFunction
  // then sees the leftover __DONE__ and we read the last framebuffer.
  // A unique query is a different navigation. That flake is what made
  // `evg:view:check` fail CI on a landing-only pull request: one of
  // thirty-one checks, the rest pass, no FAIL line in the last 30 lines.
  loadSeq += 1;
...
for (const [name, cmds] of Object.entries(SCENES)) {
  for (const v of VIEWS) {
    const pngBaked = pngPrefix ? `${pngPrefix}-${name}-${v.name.replace(/ /g, "_")}-baked.png` : null;
    const pngCamera = pngPrefix ? `${pngPrefix}-${name}-${v.name.replace(/ /g, "_")}-camera.png` : null;
    const once = async () => {
      const baked = await pixelsOf(bake(cmds, v), { x: 0, y: 0, scale: 1 }, pngBaked);
      const camera = await pixelsOf(cmds, v, pngCamera);
      return worstDiff(baked, camera);
    };
    let d = await once();
    // A leftover framebuffer from the previous navigation is a one-shot
    // miss, not a different picture. Draw both sides again before it
    // is a failure.
    if (d.worst > 3) d = await once();
    ok(
      `${name}, ${v.name}`,
      d.worst <= 3,
...
for (const v of VIEWS.slice(1)) {
  const once = async () => {
    ...
    return {
      dw: Math.abs(baked.w - camera.w),
      dh: Math.abs(baked.h - camera.h),
      dx: Math.abs(baked.x0 - camera.x0),
      dy: Math.abs(baked.y0 - camera.y0),
    };
  };
  let b = await once();
  if (b.dw > 2 || b.dh > 2 || b.dx > 2 || b.dy > 2) b = await once();
  ok(
    `text, ${v.name}: the run is the same size in the same place`,
    b.dw <= 2 && b.dh <= 2 && b.dx <= 2 && b.dy <= 2,
    `box off by ${b.dx},${b.dy} and ${b.dw}x${b.dh}`,
  );
}
...
// each build their own frame.
{
  ...
  if (two.err) {
    ...
    let fresh = await pixelsOf(SCENES.boxes, VIEWS[1]);
    let d = worstDiff(two.b, fresh);
    if (d.worst > 3) {
      fresh = await pixelsOf(SCENES.boxes, VIEWS[1]);
      d = worstDiff(two.b, fresh);
    }
