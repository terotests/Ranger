 ...
 * the seam is there — see lib/evg/gl/README.md.
 *
...
export function renderDisplayList(target, doc, opts = {}) {
  ...
  return {
    ...
    // `doc.list.effect`, not `doc.effect`: the effect rides the LIST, the way
    // the commands do, and read off the envelope this was null on every page
    // that has ever had one — including the one it was written for. Nothing
    // read it, which is why it stayed wrong.
    unsupportedEffect: (doc.list && doc.list.effect) ? doc.list.effect.kind : null,
    // And the same for the element-scoped ones: a starfield is a shader, and a
    // backend with no render target draws the page without it. Named rather
    // than counted, because the useful question here is WHICH effect a page
    // loses when it is exported as SVG.
    unsupportedEffects: (doc.list && doc.list.effects ? doc.list.effects : []).map((e) => e.kind),
  };
}
