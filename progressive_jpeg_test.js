...
class ProgressiveJPEGDecoder  {
  ...
  parseSOS (pos, length) {
    ...
    console.log(((((("    Scan: Ss=" + ((this.scanSs.toString()))) + " Se=") + ((this.scanSe.toString()))) + " (") + scanType) + ")");
    const scanStart = pos;
    let searchPos = pos;
    while (searchPos < (this.dataLen - 1)) {
      const b = this.data._view.getUint8(searchPos);
    ...
    while (blockIdx < buf.numBlocks) {
      ...
      while (k <= this.scanSe) {
        ...
        if ( size == 0 ) {
          ...
          while ((zerosToSkip_1 > 0) && (k <= this.scanSe)) {
...
function __js_main() {
  console.log("=== Progressive JPEG Decoder Test ===");
  console.log("");
  const dirPath = "c:\\Users\\terok\\proj\\Ranger\\gallery\\pdf_writer\\";
  const ppm = new PPMImage();
  console.log("Test 1: Progressive JPEG (Example.jpg)");
  console.log("---------------------------------------");
  const decoder1 = new ProgressiveJPEGDecoder();
  const img1 = decoder1.decode(dirPath, "Example.jpg");
  console.log((("Result: " + ((img1.width.toString()))) + "x") + ((img1.height.toString())));
  ppm.save(img1, dirPath, "progressive_example_output.ppm");
  console.log("Saved: progressive_example_output.ppm");
  console.log("");
  console.log("Test 2: Baseline JPEG (Canon_40D.jpg)");
  console.log("-------------------------------------");
  const decoder2 = new ProgressiveJPEGDecoder();
  const img2 = decoder2.decode(dirPath, "Canon_40D.jpg");
  console.log((("Result: " + ((img2.width.toString()))) + "x") + ((img2.height.toString())));
  const scaled2 = img2.scale(3);
  ppm.save(scaled2, dirPath, "progressive_baseline_output.ppm");
  console.log("Saved: progressive_baseline_output.ppm");
  console.log("");
